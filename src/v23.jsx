  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:3000,
      ...
      <div style={{background:"#fff",borderRadius:18,width:"100%",maxWidth:860,
        height:"auto",maxHeight:"98vh",display:"flex",flexDirection:"column",
          ...
          <div style={{padding:"14px 16px",display:"flex",flexDirection:"column",gap:10,overflowY:"auto"}}>
            ...
            {!canConfirm&&(
              <div style={{padding:"8px 12px",background:"#FDE8E8",border:"1px solid #D94040",borderRadius:8,fontSize:11,fontWeight:700,color:"#D94040",textAlign:"center"}}>
                {method==="Cash"&&cashFloat>0?"⚠️ Cash amount less than total":method==="Both"?"⚠️ Fix split amounts":"⚠️ Enter payment amount"}
              </div>
            )}
...
function POS({items,sales,setSales,tables,setTables,promos,license,lang="en"}){
  ...
  async function confirmPayment(method,given,change,promo,totalDiscountAmt,finalTotal,finalVat,printAndSave,payInfo,manualDiscountAmt,promoDiscountAmt,isDraft=false,extraData={}){
    ...
    // ALWAYS save to sales — also save directly to LS as safety net
    try{
      setSales(prev=>{
        const updated=[...prev,inv];
        // Direct LS save as backup in case React state save fails
        try{LS.set("restopos_sales",updated);}catch(e2){}
        return updated;
      });
    }catch(e){
      // Last resort — save directly
      try{const cur=LS.get("restopos_sales")||[];LS.set("restopos_sales",[...cur,inv]);}catch(e2){}
    ...
    if(!printAndSave){
      ...
      // Normal invoice: QZ → ESC/POS → iframe → error
      let printed=false;
      let printErr="";
      const fmt=LS.get("restopos_invoice_format")||{};
      const html=buildReceiptHTML(inv,license,null,fmt);

      // 1. Try QZ Tray (best — silent, reliable)
      if(!printed&&isQZConnected()&&_qzBillPrinter){
        try{
          await printWithQZ(html,_qzBillPrinter,fmt.paperWidth||"80mm");
          printed=true;
        }catch(e){printErr=e.message;console.warn("[QZ Print]",e);}
      }
      // 2. Try ESC/POS USB
      if(!printed&&"serial" in navigator){
        try{
          if(!isPortOpen(_billPort)){
            ...
            if(ports.length>0){try{_billPort=ports[0];if(!_billPort.readable)await _billPort.open({baudRate:9600});}catch(e){}}
          }
          if(isPortOpen(_billPort)){await printReceiptEscPos(inv,license);printed=true;}
        }catch(e){printErr=e.message;console.warn("[ESC/POS]",e);}
      }
      // 3. Silent iframe fallback
      if(!printed){
        try{
      ...
      // Result
      if(printed){
        const method2=isQZConnected()?"QZ":"ESC/POS";
        setPrintBanner({msg:"✅ "+inv.id+" — Saved & Printed ("+method2+")",type:"success"});
      }else{
        setPrintBanner({msg:"⚠️ "+inv.id+" saved — Print failed. Check QZ Tray.",type:"error"});
      }
      setTimeout(()=>setPrintBanner(null),4000);
    }
  }
...
async function connectQZ() {
  try {
    ...
    // Use insecure connection (free version — client clicks Allow once)
    ...
    _qzConnected = true;
    // Get printers
    _qzPrinters = await qz.printers.find();
    // Auto-select saved printers
    if (!_qzBillPrinter && _qzPrinters.length > 0) {
...
async function printWithQZ(htmlContent, printerName, paperWidth = "80mm") {
  try {
    if (!window.qz) await loadQZ();
    if (!window.qz) throw new Error("QZ Tray not loaded");
    if (!qz.websocket.isActive()) {
      const ok = await connectQZ();
      if (!ok) throw new Error("QZ Tray not connected");
    }
    const targetPrinter = printerName || _qzBillPrinter;
    if (!targetPrinter) throw new Error("No printer selected");
    const config = qz.configs.create(targetPrinter, {
      colorType: "grayscale",
      margins: { top: 0, right: 0, bottom: 0, left: 0 },
      size: { width: parseFloat(paperWidth) || 80, height: null },
      units: "mm",
      scaleContent: true,
    });
    const data = [{
    ...
    console.warn("[QZ] Print failed:", e.message);
    throw e;
  }
}
...
function isQZConnected() {
  return _qzConnected && window.qz && qz.websocket.isActive();
}
    ...
    <div dir={dir(lang)} style={{fontFamily:lang==="ar"?"'Tajawal','Plus Jakarta Sans',sans-serif":"'Plus Jakarta Sans','Tajawal',sans-serif",background:C.bg,height:"100vh",display:"flex",flexDirection:"column",overflow:"hidden",fontSize:`${uiScale}%`}}>
      ...
      <div style={{display:"flex",alignItems:"stretch",flexShrink:0,zIndex:100,boxShadow:"0 2px 12px rgba(0,0,0,0.18)",minHeight:50,width:"100%",flexWrap:"nowrap"}}>
        ...
        <div style={{background:"linear-gradient(135deg,#1A3D2B 0%,#1F4D36 100%)",display:"flex",alignItems:"center",gap:4,padding:"0 8px",flexShrink:0,borderLeft:"1px solid rgba(255,255,255,0.1)"}}>
          ...
          {isQZConnected()&&<span style={{fontSize:9,background:"rgba(26,107,74,0.25)",color:"#7FFAB5",padding:"3px 8px",borderRadius:4,fontWeight:800,border:"1px solid rgba(26,107,74,0.4)",whiteSpace:"nowrap"}}>🖨️ QZ Ready</span>}
