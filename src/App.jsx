...
function POS({items,sales,setSales,tables,setTables,promos,license,lang="en"}){
  ...
  async function confirmPayment(method,given,change,promo,totalDiscountAmt,finalTotal,finalVat,printAndSave,payInfo,manualDiscountAmt,promoDiscountAmt,isDraft=false,extraData={}){
    ...
    if(!printAndSave){
      ...
      // Draft: canvas image → QZ (no browser popup fallback)
      try{
        const fmt=LS.get("restopos_invoice_format")||{};
        const draftHTML=await buildReceiptHTMLForPrint(inv,license,null,fmt);
        const qzPrinter=localStorage.getItem("restopos_qz_bill_printer")||_qzBillPrinter;
        if(qzPrinter){
          if(!isQZConnected())await connectQZ();
          if(isQZConnected()){
            await printWithQZ(draftHTML,qzPrinter,fmt.paperWidth||"80mm");
            setPrintBanner({msg:"📋 "+inv.id+" — Draft Printed",type:"success"});
          }else{
            setPrintBanner({msg:"💾 "+inv.id+" — Draft Saved. QZ not connected.",type:"error"});
          }
        }else{
          setPrintBanner({msg:"💾 "+inv.id+" — Draft Saved. No printer selected in Print settings.",type:"error"});
        }
      }catch(e){
        setPrintBanner({msg:"⚠️ Draft saved — Print failed: "+e.message,type:"error"});
      }
      setTimeout(()=>setPrintBanner(null),4000);
    }else{
      // Normal invoice — canvas image → QZ (primary, no dialog fallback)
      const fmt=LS.get("restopos_invoice_format")||{};
      let printed=false;
      let printMethod="";
      try{
        const html=await buildReceiptHTMLForPrint(inv,license,zatcaInvoice||null,fmt);
        const qzPrinter=localStorage.getItem("restopos_qz_bill_printer")||_qzBillPrinter;
        if(qzPrinter){
          if(!isQZConnected())await connectQZ();
          if(isQZConnected()){
            await printWithQZ(html,qzPrinter,fmt.paperWidth||"80mm");
            printed=true;printMethod="QZ Tray";
          }
        }
      }catch(e){console.warn("[QZ Print]",e.message);}

      if(printed){
        setPrintBanner({msg:"✅ "+inv.id+" — Saved & Printed ("+printMethod+")",type:"success"});
      }else{
        setPrintBanner({msg:"💾 "+inv.id+" — Saved. Print failed — check Print & Receipt settings in Advanced tab.",type:"error"});
      }
  ...
  async function printKOT(){
    ...
    const lic=LS.get("restopos_license_v2")||{};
    const qzPrinter=localStorage.getItem("restopos_qz_kitchen_printer")||_qzKitchenPrinter;
    if(qzPrinter){
      try{
        if(!isQZConnected())await connectQZ();
        if(isQZConnected()){
          const kotHTML=buildKOTHTMLForPrint(cart,orderType,selectedTable,newKot,lic,kfmt);
          await printWithQZ(kotHTML,qzPrinter,kfmt.kotPaper||"80mm");
          showN("KOT #"+newKot+" sent to kitchen printer");
          return;
        }
      }catch(e){console.warn("[QZ KOT]",e.message);}
    }
    showN("KOT #"+newKot+" saved. No kitchen printer set — go to Advanced > Print & Receipt.");
  }
  ...
  const posActiveCats=useMemo(()=>savedCats.filter(c=>items.some(i=>i.active&&i.category===c)),[savedCats,items]);
  const cats=useMemo(()=>["ALL",...posActiveCats],[posActiveCats]);
  const filteredItems=useMemo(()=>items.filter(i=>i.active&&(activeCat==="ALL"||i.category===activeCat)),[items,activeCat]);
  return(
...
// ═══════════════════════════════════════════════════════════════════
// CANVAS-BASED ARABIC RECEIPT RENDERER
// Renders receipt to PNG — solves Arabic font issue on thermal printers
// ═══════════════════════════════════════════════════════════════════
function loadImageAsync(src){
  return new Promise((resolve,reject)=>{
    const img=new Image();img.crossOrigin="anonymous";
    img.onload=()=>resolve(img);img.onerror=()=>reject(new Error("Image load failed: "+src));
    img.src=src;
  });
}

async function buildReceiptImageDataUrl(order,license,zatca,fmt={}){
  const paperWidth=fmt.paperWidth||"80mm";
  const PW=paperWidth==="58mm"?400:576;
  const PAD=20;const CW=PW-PAD*2;
  const FS=fmt.fontSize||13;
  const isDraft=!zatca;
  const arabicFont=fmt.arabicFont||"Tajawal";
  const separator=fmt.separator||"dashed";
  try{await document.fonts.load(`bold ${FS*2}px ${arabicFont}`);await document.fonts.load(`${FS}px ${arabicFont}`);}catch(_){}
  const canvas=document.createElement("canvas");canvas.width=PW;canvas.height=99999;
  const ctx=canvas.getContext("2d");
  ctx.fillStyle="#ffffff";ctx.fillRect(0,0,PW,99999);
  let y=PAD;
  const setFont=(size,weight="normal")=>{ctx.font=`${weight} ${size}px ${arabicFont}, 'Courier New', monospace`;};
  const drawHR=(thick=false)=>{
    y+=5;ctx.strokeStyle="#000";ctx.lineWidth=thick?2:1;
    ctx.setLineDash(separator==="dashed"?[4,4]:[]);
    ctx.beginPath();ctx.moveTo(PAD,y);ctx.lineTo(PAD+CW,y);ctx.stroke();
    ctx.setLineDash([]);y+=8;
  };
  const drawCenter=(text,size,weight="normal",color="#000")=>{
    if(!text)return;setFont(size,weight);ctx.fillStyle=color;ctx.textAlign="center";
    ctx.fillText(text,PW/2,y);y+=size+5;
  };
  const drawRow=(rightText,leftText,size=FS,rw="normal",lw="normal")=>{
    ctx.fillStyle="#000";
    if(rightText){setFont(size,rw);ctx.textAlign="right";ctx.fillText(String(rightText),PAD+CW,y);}
    if(leftText){setFont(size,lw);ctx.textAlign="left";ctx.fillText(String(leftText),PAD,y);}
    y+=size+6;
  };
  const items=order.items||[];
  // HEADER
  if(fmt.logoUrl){try{const logo=await loadImageAsync(fmt.logoUrl);const lh=60,lw=(logo.width/logo.height)*lh;ctx.drawImage(logo,(PW-lw)/2,y,lw,lh);y+=lh+8;}catch(_){}}
  drawCenter(fmt.shopNameOverride||license.businessName||"المطعم",FS+9,"bold");
  if(license.businessNameEn)drawCenter(license.businessNameEn,FS+2,"bold");
  if(fmt.tagline)drawCenter(fmt.tagline,FS-1,"normal","#555");
  drawCenter(license.address||"",FS-1,"normal","#444");
  drawCenter("VAT / الرقم الضريبي: "+(license.vatNumber||""),FS-1);
  if(license.phone)drawCenter("📞 "+license.phone,FS-1);
  drawHR();
  // ORDER INFO
  drawRow(order.id||"",(order.date||"")+" "+(order.time||""),FS-1);
  if(fmt.showOrderType!==false)drawRow(order.table?"طاولة / Table "+order.table:"",order.type||"Dine-In",FS-1);
  if(fmt.showCustomer!==false&&order.customer)drawRow("العميل: "+order.customer,"Customer",FS-1);
  drawHR();
  // ITEMS HEADER
  ctx.fillStyle="#666";setFont(FS-2,"bold");
  ctx.textAlign="right";ctx.fillText("الصنف / Item",PAD+CW,y);
  ctx.textAlign="center";ctx.fillText("Qty",PW/2,y);
  ctx.textAlign="left";ctx.fillText("Amount",PAD,y);
  y+=FS+3;drawHR();
  // ITEMS
  for(const it of items){
    const amt="SAR "+(it.price*it.qty).toFixed(2);
    ctx.fillStyle="#000";ctx.textAlign="right";setFont(FS,"bold");
    ctx.fillText(it.nameAr||it.name,PAD+CW,y);
    if(it.nameAr&&fmt.showArabicName!==false){ctx.fillStyle="#444";setFont(FS-2,"normal");ctx.fillText(it.name,PAD+CW,y+FS+1);}
    ctx.fillStyle="#000";ctx.textAlign="center";setFont(FS,"bold");ctx.fillText(String(it.qty),PW/2,y);
    ctx.textAlign="left";setFont(FS,"normal");ctx.fillText(amt,PAD,y);
    if(it.notes){ctx.fillStyle="#888";setFont(FS-2,"normal");ctx.textAlign="right";ctx.fillText("↳ "+it.notes,PAD+CW,y+(it.nameAr?FS*2+4:FS+2));}
    y+=(it.nameAr&&fmt.showArabicName!==false?FS*2+8:FS+8)+(it.notes?FS:0);
  }
  drawHR();
  // TOTALS
  if(order.discount>0)drawRow("خصم / Discount","- SAR "+(order.discount||0).toFixed(2),FS);
  if(fmt.showVat!==false)drawRow("VAT 15% (incl.) / الضريبة","SAR "+(order.vat||0).toFixed(2),FS-1);
  y+=4;ctx.strokeStyle="#000";ctx.lineWidth=2;ctx.setLineDash([]);
  ctx.beginPath();ctx.moveTo(PAD,y);ctx.lineTo(PAD+CW,y);ctx.stroke();y+=7;
  const tFS=fmt.totalSize==="xl"?FS+7:fmt.totalSize==="large"?FS+4:FS+1;
  drawRow("الإجمالي / TOTAL","SAR "+(order.total||0).toFixed(2),tFS,"bold","bold");
  drawHR();
  // PAYMENT
  drawRow("الدفع / Payment",order.payMethod||"Cash",FS,"bold","bold");
  if(order.payMethod==="Cash"){drawRow("المبلغ / Cash Given","SAR "+(order.given||0).toFixed(2));drawRow("الباقي / Change","SAR "+(order.change||0).toFixed(2));}
  else if(order.payMethod==="Card")drawRow("بطاقة / Card","SAR "+(order.total||0).toFixed(2));
  else if(order.payMethod==="Both"){drawRow("نقد / Cash","SAR "+(order.cashAmount||0).toFixed(2));drawRow("بطاقة / Card","SAR "+(order.cardAmount||0).toFixed(2));}
  drawHR();
  // ZATCA / DRAFT
  if(isDraft){drawCenter("⚠ DRAFT — NOT A TAX INVOICE",FS-1,"bold","#cc0000");drawCenter("مسودة — ليست فاتورة ضريبية",FS-1,"bold","#cc0000");}
  else{
    drawCenter("ZATCA PHASE 2 COMPLIANT",FS-1,"bold");drawCenter("فاتورة إلكترونية — المرحلة الثانية",FS-1);
    if(zatca?.qr_string){const qrSize=150;const qrUrl=`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(zatca.qr_string)}`;
      try{const qrImg=await loadImageAsync(qrUrl);ctx.drawImage(qrImg,(PW-qrSize)/2,y,qrSize,qrSize);y+=qrSize+8;}catch(_){drawCenter("[QR Code]",FS-2,"normal","#888");}}
    if(order.id)drawCenter(order.id,FS-2,"normal","#555");
  }
  drawHR();
  // FOOTER
  drawCenter(fmt.footerAr||"شكراً لزيارتكم",FS+2,"bold");
  drawCenter(fmt.footer||"Thank you for your visit!",FS);
  if(fmt.website)drawCenter(fmt.website,FS-2,"normal","#777");
  if(fmt.social)drawCenter(fmt.social,FS-2,"normal","#777");
  y+=24;
  // Crop
  const out=document.createElement("canvas");out.width=PW;out.height=y;
  const octx=out.getContext("2d");octx.fillStyle="#fff";octx.fillRect(0,0,PW,y);octx.drawImage(canvas,0,0);
  return out.toDataURL("image/png");
}

async function buildReceiptHTMLForPrint(order,license,zatca,fmt={}){
  const dataUrl=await buildReceiptImageDataUrl(order,license,zatca,fmt);
  const pw=fmt.paperWidth||"80mm";
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>@page{size:${pw} auto;margin:0}*{margin:0;padding:0;box-sizing:border-box}body{width:${pw};background:#fff}img{width:100%;display:block}</style></head><body><img src="${dataUrl}"/></body></html>`;
}

function buildKOTHTMLForPrint(cart,orderType,tableId,kotNo,license,fmt={}){
  const items=cart||[];
  return `<!DOCTYPE html><html><head><meta charset="UTF-8">
<link href="https://fonts.googleapis.com/css2?family=Tajawal:wght@700;900&display=swap" rel="stylesheet">
<style>@page{size:${fmt.kotPaper||"80mm"} auto;margin:0}*{box-sizing:border-box;margin:0;padding:0}
body{font-family:'Courier New',monospace;font-size:${fmt.kotLargeText?15:13}px;color:#000;background:#fff;width:${fmt.kotPaper||"80mm"};padding:4mm}
.c{text-align:center}.b{font-weight:bold}.big{font-size:${fmt.kotLargeText?22:17}px;font-weight:900}
.hr{border:none;border-top:${fmt.kotSeparator==="solid"?"2px solid":"1px dashed"} #000;margin:5px 0}
.ar{font-family:'Tajawal',sans-serif;font-weight:700;direction:rtl;font-size:13px;color:#333}
.note{font-size:11px;color:#c00}
@media print{body{width:${fmt.kotPaper||"80mm"}}}</style></head><body>
<div class="c" style="font-size:10px;font-weight:700;letter-spacing:0.1em">KITCHEN ORDER TICKET</div>
<div class="c big">${fmt.kotTitle||"KOT"} #${kotNo}</div>
${fmt.kotTitleAr?`<div class="c ar" style="font-size:14px">${fmt.kotTitleAr}</div>`:""}
<div class="hr"></div>
<div style="display:flex;justify-content:space-between">
  <span class="b">${fmt.kotShowOrderType!==false?(orderType||"Order").toUpperCase()+(tableId&&fmt.kotShowTable!==false?" · Table "+tableId:""):""}</span>
  ${fmt.kotShowTime!==false?`<span style="font-size:11px">${new Date().toLocaleTimeString("en-SA",{hour:"2-digit",minute:"2-digit"})}</span>`:""}
</div>
${license?.businessName?`<div style="font-size:11px;color:#555">${license.businessName}</div>`:""}
<div class="hr"></div>
${items.map(it=>`<div style="display:flex;gap:8px;margin:5px 0">
  <span style="font-weight:900;min-width:28px;font-size:${fmt.kotLargeText?18:15}px">${it.qty}×</span>
  <span style="flex:1"><div style="font-weight:700">${it.name}</div>
  ${it.nameAr&&fmt.kotShowArabic!==false?`<div class="ar">${it.nameAr}</div>`:""}
  ${it.notes?`<div class="note">↳ ${it.notes}</div>`:""}</span></div>`).join("")}
<div class="hr"></div>
${fmt.kotFooter?`<div class="c" style="font-size:11px">${fmt.kotFooter}</div>`:""}
<div class="c" style="font-size:10px;color:#777">${new Date().toLocaleString("en-SA")}</div>
<br/><br/></body></html>`;
}

// ═══════════════════════════════════════════════════════════════════
// UNIFIED PRINT SETTINGS COMPONENT
// Replaces ThermalPrinterSettings + QZTraySettings
// ═══════════════════════════════════════════════════════════════════
function UnifiedPrintSettings(){
  ...
  const [innerTab,setInnerTab]=useState("printer");
  const [fmt,setFmt]=useState(()=>LS.get("restopos_invoice_format")||{
    paperWidth:"80mm",fontSize:13,separator:"dashed",arabicFont:"Tajawal",
    footer:"Thank you for your visit!",footerAr:"شكراً لزيارتكم",
    showVat:true,showArabicName:true,showCustomer:true,showOrderType:true,
    totalSize:"large",logoUrl:"",tagline:"",website:"",social:"",
    template:"modern",headerColor:"#1A6B4A",shopNameOverride:"",
    kotTitle:"KOT",kotTitleAr:"طلب المطبخ",kotPaper:"80mm",
    kotLargeText:true,kotShowArabic:true,kotShowTable:true,
    kotShowTime:true,kotShowOrderType:true,kotSeparator:"dashed",kotFooter:"",
  });
  const license=LS.get("restopos_license_v2")||{};
  const company=LS.get("restopos_company")||{};

  function addLog(msg,type="info"){
    const time=new Date().toLocaleTimeString("en-SA");
    setPrintLog(prev=>[{msg,type,time},...prev.slice(0,29)]);
  }
  function updateFmt(k,v){const updated={...fmt,[k]:v};setFmt(updated);LS.set("restopos_invoice_format",updated);}

  async function checkQZStatus(){
    setQzStatus("checking");addLog("Checking QZ Tray...");
    try{
      const loaded=await loadQZ();
      if(!loaded||!window.qz){setQzStatus("disconnected");addLog("QZ library not loaded","error");return;}
      if(!qz.websocket.isActive()){const ok=await connectQZ();if(!ok){setQzStatus("disconnected");addLog("QZ Tray not running — download from qz.io/download","error");return;}}
      setQzStatus("connected");
      const found=await qz.printers.find();
      const list=Array.isArray(found)?found:[found].filter(Boolean);
      setPrinters(list);addLog(`Connected ✓ — ${list.length} printer(s) found`,"success");
      if(!billPrinter&&list.length>0)saveBillPrinter(list[0]);
      if(!kitchenPrinter&&list.length>0)saveKitchenPrinter(list[0]);
    }catch(e){setQzStatus("disconnected");addLog("Error: "+e.message,"error");}
  }
  function saveBillPrinter(name){setBillPrinter(name);_qzBillPrinter=name;localStorage.setItem("restopos_qz_bill_printer",name);addLog("Bill printer → "+name,"success");}
  function saveKitchenPrinter(name){setKitchenPrinter(name);_qzKitchenPrinter=name;localStorage.setItem("restopos_qz_kitchen_printer",name);addLog("Kitchen printer → "+name,"success");}

  useEffect(()=>{checkQZStatus();},[]);

  const sampleOrder={id:"INV-2026-1001",date:new Date().toLocaleDateString("en-SA"),time:new Date().toLocaleTimeString("en-SA",{hour:"2-digit",minute:"2-digit"}),type:"Dine-In",table:"7",customer:"Ahmed Al-Ghamdi",items:[{name:"Grilled Chicken",nameAr:"دجاج مشوي",qty:2,price:35.00},{name:"French Fries",nameAr:"بطاطس مقلية",qty:2,price:12.00},{name:"Cola",nameAr:"كولا",qty:3,price:7.00}],subtotal:111.30,vat:16.70,total:128.00,discount:0,payMethod:"Cash",given:150,change:22};
  const sampleZatca={qr_string:"AQQMQ2xhdWRlIEluYw==",uuid:"8e6a7b5c-1234-4d56-ab78-9f0e1d2c3b4a"};
  const sampleKOT=[{qty:2,name:"Grilled Chicken",nameAr:"دجاج مشوي",notes:"extra sauce"},{qty:1,name:"Beef Burger",nameAr:"برجر لحم",notes:""},{qty:3,name:"French Fries",nameAr:"بطاطس مقلية",notes:"crispy"}];

  async function testReceiptPrint(){
    if(!billPrinter){addLog("❌ Select a bill printer first","error");return;}
    setTesting(true);addLog("Building receipt image...");
    try{const lic={...company,...license};const html=await buildReceiptHTMLForPrint(sampleOrder,lic,sampleZatca,fmt);await printWithQZ(html,billPrinter,fmt.paperWidth||"80mm");addLog("✅ Receipt printed → "+billPrinter,"success");}
    catch(e){addLog("❌ "+e.message,"error");}
    setTesting(false);
  }
  async function testDraftPrint(){
    if(!billPrinter){addLog("❌ Select a bill printer first","error");return;}
    setTesting(true);addLog("Building draft receipt...");
    try{const lic={...company,...license};const html=await buildReceiptHTMLForPrint({...sampleOrder,isDraft:true},lic,null,fmt);await printWithQZ(html,billPrinter,fmt.paperWidth||"80mm");addLog("✅ Draft printed → "+billPrinter,"success");}
    catch(e){addLog("❌ "+e.message,"error");}
    setTesting(false);
  }
  async function testKOTPrint(){
    if(!kitchenPrinter){addLog("❌ Select a kitchen printer first","error");return;}
    setTesting(true);addLog("Sending KOT...");
    try{const kotNo=Math.floor(Math.random()*900)+100;const html=buildKOTHTMLForPrint(sampleKOT,"Dine-In",7,kotNo,license,fmt);await printWithQZ(html,kitchenPrinter,fmt.kotPaper||"80mm");addLog("✅ KOT printed → "+kitchenPrinter,"success");}
    catch(e){addLog("❌ "+e.message,"error");}
    setTesting(false);
  }
  ...
  const statusLabel={connected:"QZ Connected — Silent printing active",disconnected:"QZ Disconnected — Install & run QZ Tray",checking:"Checking QZ Tray..."};
  const innerTabs=[["printer","🖨️ Printer"],["receipt","🧾 Receipt Format"],["kot","🍽️ KOT Format"]];

  const Toggle=({value,onChange,label})=>(
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
      <span style={{fontSize:13,color:C.text}}>{label}</span>
      <div onClick={()=>onChange(!value)} style={{width:40,height:22,borderRadius:11,cursor:"pointer",transition:"background 0.2s",background:value?C.success:C.border,position:"relative"}}>
  ...
  const Field=({label,children})=>(
    <div style={{marginBottom:12}}>
      <div style={{fontSize:11,fontWeight:700,color:C.textMid,marginBottom:5,textTransform:"uppercase",letterSpacing:"0.04em"}}>{label}</div>
      {children}
    </div>
  );
  const inp={width:"100%",padding:"9px 12px",background:C.bg,border:`1.5px solid ${C.border}`,borderRadius:8,fontSize:13,fontFamily:"inherit",color:C.text,outline:"none"};
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      {/* QZ Status Bar */}
      <Card style={{border:`2px solid ${statusColor[qzStatus]}33`}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <span style={{fontSize:22}}>{statusIcon[qzStatus]}</span>
            <div>
              <div style={{fontSize:15,fontWeight:800,color:statusColor[qzStatus]}}>{statusLabel[qzStatus]}</div>
              <div style={{fontSize:12,color:C.textMid,marginTop:2}}>
                {qzStatus==="connected"?`${printers.length} printer(s) available · No dialogs, silent printing`:"QZ Tray must be installed and running on this Windows PC"}
              </div>
            </div>
          </div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            <Btn onClick={checkQZStatus} variant="outline" size="sm">🔄 Refresh</Btn>
            {qzStatus==="connected"&&<Btn onClick={async()=>{await disconnectQZ();setQzStatus("disconnected");}} color={C.danger} variant="outline" size="sm">Disconnect</Btn>}
            {qzStatus==="disconnected"&&<a href="https://qz.io/download" target="_blank" rel="noreferrer" style={{padding:"7px 14px",background:C.primary,color:"#fff",borderRadius:8,fontSize:12,fontWeight:700,textDecoration:"none"}}>⬇️ Download QZ Tray</a>}
          </div>
        </div>
      </Card>
      ...
      {/* Inner Tabs */}
      <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
        {innerTabs.map(([id,lbl])=>(
          <button key={id} onClick={()=>setInnerTab(id)} style={{padding:"7px 16px",borderRadius:8,fontFamily:"inherit",fontSize:12,fontWeight:600,cursor:"pointer",border:`1.5px solid ${innerTab===id?C.primary:C.border}`,background:innerTab===id?C.primaryLight:"#fff",color:innerTab===id?C.primary:C.textMid}}>{lbl}</button>
        ))}
      </div>
      ...
      {innerTab==="printer"&&(
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
          <Card>
            <div style={{fontSize:13,fontWeight:700,marginBottom:14}}>🖨️ Printer Selection</div>
            {qzStatus!=="connected"?(
              <div style={{fontSize:12,color:C.danger,padding:"10px 0"}}>Connect QZ Tray first to see available printers.</div>
            ):(
              <>
                <Field label="Bill Printer (Receipts & Invoices)">
                  <select value={billPrinter} onChange={e=>saveBillPrinter(e.target.value)} style={sel}>
                    <option value="">— Select Bill Printer —</option>
                    {printers.map(p=><option key={p} value={p}>{p}</option>)}
                  </select>
                  {billPrinter&&<div style={{fontSize:11,color:C.success,fontWeight:600,marginTop:4}}>✓ {billPrinter}</div>}
                </Field>
                <Field label="Kitchen Printer (KOT Tickets)">
                  <select value={kitchenPrinter} onChange={e=>saveKitchenPrinter(e.target.value)} style={sel}>
                    <option value="">— Select Kitchen Printer —</option>
                    {printers.map(p=><option key={p} value={p}>{p}</option>)}
                  </select>
                  {kitchenPrinter&&<div style={{fontSize:11,color:C.success,fontWeight:600,marginTop:4}}>✓ {kitchenPrinter}</div>}
                </Field>
              </>
            )}
          </Card>
          <Card>
            <div style={{fontSize:13,fontWeight:700,marginBottom:14}}>🧪 Test Prints</div>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              <Btn onClick={testReceiptPrint} disabled={testing||!billPrinter} color={C.success} style={{width:"100%",justifyContent:"center"}}>🧾 Print Test Receipt</Btn>
              <Btn onClick={testDraftPrint} disabled={testing||!billPrinter} color={C.warning} style={{width:"100%",justifyContent:"center"}}>📋 Print Draft Invoice</Btn>
              <Btn onClick={testKOTPrint} disabled={testing||!kitchenPrinter} color={C.primary} style={{width:"100%",justifyContent:"center"}}>🍽️ Print KOT Test</Btn>
            </div>
            {qzStatus==="disconnected"&&(
              <div style={{marginTop:14}}>
                <div style={{fontSize:12,fontWeight:700,marginBottom:8}}>How to set up QZ Tray:</div>
                {[["1","Download","Click 'Download QZ Tray' above — free, ~15MB"],["2","Install & Run","Runs silently in your system tray"],["3","Allow Connection","Browser asks 'Allow this site to print?' → Allow + tick Remember"],["4","Done","Silent printing every time, no dialogs"]].map(([n,t,d])=>(
                  <div key={n} style={{display:"flex",gap:10,marginBottom:8,padding:8,background:C.bg,borderRadius:8}}>
                    <div style={{width:24,height:24,background:C.primary,color:"#fff",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:800,flexShrink:0}}>{n}</div>
                    <div><div style={{fontSize:12,fontWeight:700}}>{t}</div><div style={{fontSize:11,color:C.textMid}}>{d}</div></div>
                  </div>
                ))}
              </div>
            )}
          ...
          <Card style={{gridColumn:"1 / -1"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              <div style={{fontSize:13,fontWeight:700}}>📋 Print Log</div>
              <Btn onClick={()=>setPrintLog([])} variant="outline" size="sm">Clear</Btn>
            </div>
            <div style={{background:C.bg,borderRadius:8,padding:10,height:160,overflowY:"auto",fontFamily:"monospace",fontSize:11}}>
              {printLog.length===0?<div style={{color:C.textMid}}>No activity yet. Run a test print.</div>
                :printLog.map((l,i)=>(
                  <div key={i} style={{padding:"2px 0",borderBottom:`1px solid ${C.border}`,color:l.type==="success"?C.success:l.type==="error"?C.danger:l.type==="warning"?C.warning:C.textMid}}>
                    <span style={{color:C.textMid,marginRight:8}}>[{l.time}]</span>{l.msg}
                  </div>
                ))}
            </div>
          </Card>
      ...
      {innerTab==="receipt"&&(
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <Card>
              <div style={{fontSize:13,fontWeight:700,marginBottom:14}}>📐 Paper & Font</div>
              <Field label="Paper Width"><select value={fmt.paperWidth||"80mm"} onChange={e=>updateFmt("paperWidth",e.target.value)} style={sel}><option value="80mm">80mm (Standard)</option><option value="58mm">58mm (Small)</option></select></Field>
              <Field label="Font Size"><select value={fmt.fontSize||13} onChange={e=>updateFmt("fontSize",parseInt(e.target.value))} style={sel}>{[10,11,12,13,14,15,16].map(s=><option key={s} value={s}>{s}px</option>)}</select></Field>
              <Field label="Arabic Font"><select value={fmt.arabicFont||"Tajawal"} onChange={e=>updateFmt("arabicFont",e.target.value)} style={sel}>{["Tajawal","Cairo","Amiri","Noto Naskh Arabic"].map(f=><option key={f} value={f}>{f}</option>)}</select></Field>
              <Field label="Divider Style"><select value={fmt.separator||"dashed"} onChange={e=>updateFmt("separator",e.target.value)} style={sel}><option value="dashed">Dashed  - - - -</option><option value="solid">Solid  ————</option></select></Field>
              <Field label="Total Size"><select value={fmt.totalSize||"large"} onChange={e=>updateFmt("totalSize",e.target.value)} style={sel}><option value="normal">Normal</option><option value="large">Large</option><option value="xl">Extra Large</option></select></Field>
            </Card>
            <Card>
              <div style={{fontSize:13,fontWeight:700,marginBottom:14}}>🏪 Branding</div>
              <Field label="Shop Name Override"><input value={fmt.shopNameOverride||""} onChange={e=>updateFmt("shopNameOverride",e.target.value)} style={inp} placeholder={license.businessName||"Leave blank to use license name"}/></Field>
              <Field label="Tagline"><input value={fmt.tagline||""} onChange={e=>updateFmt("tagline",e.target.value)} style={inp} placeholder="e.g. Freshly Made Daily"/></Field>
              <Field label="Website"><input value={fmt.website||""} onChange={e=>updateFmt("website",e.target.value)} style={inp} placeholder="www.yourrestaurant.com"/></Field>
              <Field label="Social"><input value={fmt.social||""} onChange={e=>updateFmt("social",e.target.value)} style={inp} placeholder="@yourrestaurant"/></Field>
              <Field label="Footer (English)"><input value={fmt.footer||""} onChange={e=>updateFmt("footer",e.target.value)} style={inp} placeholder="Thank you for your visit!"/></Field>
              <Field label="Footer (Arabic)"><input value={fmt.footerAr||""} onChange={e=>updateFmt("footerAr",e.target.value)} style={{...inp,direction:"rtl"}} placeholder="شكراً لزيارتكم"/></Field>
            </Card>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <Card>
              <div style={{fontSize:13,fontWeight:700,marginBottom:14}}>⚙️ Show / Hide</div>
              <Toggle value={fmt.showVat!==false} onChange={v=>updateFmt("showVat",v)} label="Show VAT row"/>
              <Toggle value={fmt.showArabicName!==false} onChange={v=>updateFmt("showArabicName",v)} label="Show Arabic item names"/>
              <Toggle value={fmt.showCustomer!==false} onChange={v=>updateFmt("showCustomer",v)} label="Show customer name"/>
              <Toggle value={fmt.showOrderType!==false} onChange={v=>updateFmt("showOrderType",v)} label="Show order type / table"/>
            </Card>
            <Card>
              <div style={{fontSize:13,fontWeight:700,marginBottom:10}}>👁️ Preview</div>
              <Btn onClick={testReceiptPrint} disabled={testing||!billPrinter} color={C.success} style={{width:"100%",justifyContent:"center",marginBottom:12}}>🖨️ Print Test with This Format</Btn>
              <div style={{background:"#fff",border:`1px dashed ${C.border}`,borderRadius:8,padding:12,maxWidth:fmt.paperWidth==="58mm"?160:210,margin:"0 auto",fontFamily:`'${fmt.arabicFont||"Tajawal"}', monospace`,fontSize:fmt.fontSize||13,color:"#000",lineHeight:1.5}}>
                <div style={{textAlign:"center",fontWeight:900,fontSize:(fmt.fontSize||13)+4}}>{fmt.shopNameOverride||license.businessName||company.businessName||"Restaurant"}</div>
                <div style={{textAlign:"center",fontSize:9}}>VAT: {license.vatNumber||"300000000000003"}</div>
                <div style={{borderTop:fmt.separator==="solid"?"1px solid #000":"1px dashed #000",margin:"5px 0"}}/>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:9}}><span>INV-2026-1001</span><span>Today</span></div>
                <div style={{borderTop:fmt.separator==="solid"?"1px solid #000":"1px dashed #000",margin:"5px 0"}}/>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:2}}>
                  <span style={{direction:"rtl",fontFamily:`'${fmt.arabicFont||"Tajawal"}'`}}>دجاج مشوي</span><span>SAR 70.00</span>
                </div>
                <div style={{fontSize:9,color:"#555"}}>Grilled Chicken × 2</div>
                <div style={{borderTop:"2px solid #000",marginTop:5,paddingTop:4,display:"flex",justifyContent:"space-between",fontWeight:900,fontSize:fmt.totalSize==="xl"?(fmt.fontSize||13)+6:fmt.totalSize==="large"?(fmt.fontSize||13)+3:(fmt.fontSize||13)}}>
                  <span style={{direction:"rtl"}}>الإجمالي</span><span>SAR 128.00</span>
                </div>
                <div style={{borderTop:fmt.separator==="solid"?"1px solid #000":"1px dashed #000",margin:"5px 0"}}/>
                <div style={{textAlign:"center",fontWeight:700}}>{fmt.footerAr||"شكراً لزيارتكم"}</div>
                <div style={{textAlign:"center",fontSize:9}}>{fmt.footer||"Thank you!"}</div>
              </div>
            </Card>
          </div>
        </div>
      ...
      {innerTab==="kot"&&(
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
          <Card>
            <div style={{fontSize:13,fontWeight:700,marginBottom:14}}>🍽️ KOT Settings</div>
            <Field label="KOT Title (English)"><input value={fmt.kotTitle||"KOT"} onChange={e=>updateFmt("kotTitle",e.target.value)} style={inp}/></Field>
            <Field label="KOT Title (Arabic)"><input value={fmt.kotTitleAr||""} onChange={e=>updateFmt("kotTitleAr",e.target.value)} style={{...inp,direction:"rtl"}} placeholder="طلب المطبخ"/></Field>
            <Field label="KOT Paper Width"><select value={fmt.kotPaper||"80mm"} onChange={e=>updateFmt("kotPaper",e.target.value)} style={sel}><option value="80mm">80mm</option><option value="58mm">58mm</option></select></Field>
            <Field label="KOT Divider"><select value={fmt.kotSeparator||"dashed"} onChange={e=>updateFmt("kotSeparator",e.target.value)} style={sel}><option value="dashed">Dashed</option><option value="solid">Solid</option></select></Field>
            <Field label="Footer Text"><input value={fmt.kotFooter||""} onChange={e=>updateFmt("kotFooter",e.target.value)} style={inp} placeholder="e.g. Rush order!"/></Field>
            <Toggle value={fmt.kotLargeText!==false} onChange={v=>updateFmt("kotLargeText",v)} label="Large text (easier to read)"/>
            <Toggle value={fmt.kotShowArabic!==false} onChange={v=>updateFmt("kotShowArabic",v)} label="Show Arabic item names"/>
            <Toggle value={fmt.kotShowTable!==false} onChange={v=>updateFmt("kotShowTable",v)} label="Show table number"/>
            <Toggle value={fmt.kotShowTime!==false} onChange={v=>updateFmt("kotShowTime",v)} label="Show time"/>
            <Toggle value={fmt.kotShowOrderType!==false} onChange={v=>updateFmt("kotShowOrderType",v)} label="Show order type"/>
            <Btn onClick={testKOTPrint} disabled={testing||!kitchenPrinter} color={C.primary} style={{width:"100%",justifyContent:"center",marginTop:8}}>🖨️ Print KOT Test</Btn>
          </Card>
          <Card>
            <div style={{fontSize:13,fontWeight:700,marginBottom:10}}>👁️ KOT Preview</div>
            <div style={{background:"#fff",border:`1px dashed ${C.border}`,borderRadius:8,padding:12,maxWidth:fmt.kotPaper==="58mm"?160:210,margin:"0 auto",fontFamily:"'Courier New', monospace",fontSize:fmt.kotLargeText?14:12,color:"#000",lineHeight:1.6}}>
              <div style={{textAlign:"center",fontWeight:900,fontSize:fmt.kotLargeText?20:16}}>{fmt.kotTitle||"KOT"} #42</div>
              {fmt.kotTitleAr&&<div style={{textAlign:"center",direction:"rtl",fontFamily:`'${fmt.arabicFont||"Tajawal"}'`,fontSize:12}}>{fmt.kotTitleAr}</div>}
              <div style={{borderTop:fmt.kotSeparator==="solid"?"1px solid #000":"1px dashed #000",margin:"5px 0"}}/>
              {fmt.kotShowOrderType!==false&&<div>DINE-IN{fmt.kotShowTable!==false?" · Table 5":""}</div>}
              {fmt.kotShowTime!==false&&<div style={{fontSize:10}}>12:35 PM</div>}
              <div style={{borderTop:fmt.kotSeparator==="solid"?"1px solid #000":"1px dashed #000",margin:"5px 0"}}/>
              {[["2× Grilled Chicken","دجاج مشوي"],["1× Beef Burger","برجر لحم"]].map(([en,ar])=>(
                <div key={en} style={{fontWeight:fmt.kotLargeText?900:700,fontSize:fmt.kotLargeText?14:12}}>
                  {en}{fmt.kotShowArabic!==false&&<div style={{direction:"rtl",fontFamily:`'${fmt.arabicFont||"Tajawal"}'`,fontSize:11,color:"#555"}}>{ar}</div>}
                </div>
              ))}
              <div style={{borderTop:fmt.kotSeparator==="solid"?"1px solid #000":"1px dashed #000",margin:"5px 0"}}/>
              {fmt.kotFooter&&<div style={{textAlign:"center",fontSize:10}}>{fmt.kotFooter}</div>}
            </div>
          </Card>
        </div>
      )}
...
function AdvancedFeatures({sales,items,setItems}){
  ...
  const tabs=[["print","🖨️ Print & Receipt"],["kds","🍳 KDS"],["stocktakes","📦 Stock Takes"],["recipes","📋 Recipes"],["giftcards","🎁 Gift Cards"],["delivery","🛵 Delivery"],["locations","🏢 Locations"],["accounting","📤 Accounting"],["reports","📅 Reports"],["errorlog","⚠️ Error Log"]];
  return(
    <div>
      <div style={{display:"flex",gap:6,marginBottom:20,flexWrap:"wrap"}}>
        {tabs.map(([id,lbl])=>(
      ...
      {tab==="print"&&<UnifiedPrintSettings/>}
