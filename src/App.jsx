    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg, #0a1628 0%, #1A3A5C 50%, #0a2818 100%)",display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'Plus Jakarta Sans', sans-serif"}}>
      ...
      <div style={{width:"100%",maxWidth:520}}>
        ...
        <div style={{background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.15)",borderRadius:20,padding:32,backdropFilter:"blur(12px)"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            {[["ownerName","Your Full Name (Owner / Contact)","Mohammed Al-Rashid"],["businessName","Business Name — الاسم بالعربية أو الإنجليزية (Arabic / English)","Al Baik Restaurant — مطعم البيك"],["businessNameAr","اسم المنشأة بالعربية (Arabic Name — اختياري)","مطعم البيك"],["email","Email Address (for password recovery)","your@email.com"],["vatNumber","VAT / TRN (15 digits, starts with 3)","300000000000003"],["address","Business Address","King Fahd Road, Riyadh"],["city","City","Riyadh"],["phone","Phone Number","+966 50 000 0000"]].map(([k,label,ph])=>(
              <div key={k}>
                <label style={{fontSize:12,fontWeight:700,color:"rgba(255,255,255,0.6)",display:"block",marginBottom:5}}>{label}</label>
                <input value={form[k]} onChange={e=>{let v=e.target.value;if(k==="crNumber"){v=v.replace(/\D/g,"").slice(0,12);}set(k,v);}} placeholder={ph} maxLength={k==="crNumber"?12:undefined} inputMode={k==="crNumber"?"numeric":undefined} style={{width:"100%",padding:"11px 14px",background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:10,fontSize:13,color:"#fff",fontFamily:"inherit",direction:k==="businessNameAr"?"rtl":"ltr"}}/>
              </div>
            ))}
          </div>
  ...
  async function handleVerify(){
    ...
    try{
      ...
      if(existingSnap.empty){
        await addDoc(collection(db,"pending_activations"),{...businessData,licenseKey:cleanKey,submittedAt:new Date().toISOString(),status:"pending",isActive:true,credentialsApproved:false,forceLogout:false,subscriptionPlan:"basic",deviceId:navigator.userAgent.slice(0,100),deviceInfo:devInfo});
      }
...
// PAYMENT MODAL
// ═══════════════════════════════════════════════════════════════════
function PaymentModal({total,subtotal,vat,promos,onConfirm,onClose,license,vno=1,kotNo=1,customers=[],customerName:initCustName="",customerPhone:initCustPhone=""}){
  // ── State ────────────────────────────────────────────────────────
  ...
  function handleConfirm(){
    ...
    localStorage.setItem("restopos_print_save_pref",String(printAndSave));
    const payInfo={
      method,
    ...
    const actualPrint=saveWithoutPrint?false:printAndSave;
    onConfirm(
      payInfo.method,payInfo.given,payInfo.change,
      appliedPromo,totalDiscountAmt,finalTotal,finalVat,
      actualPrint,payInfo,manualDiscountAmt,promoDiscountAmt,
      isDraft,
      {customerName:custName,customerPhone:custPhone,invoiceNote}
    );
  }
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:3000,
      ...
      <div style={{background:"#fff",borderRadius:20,width:"100%",maxWidth:680,
        maxHeight:"96vh",overflow:"auto",boxShadow:"0 24px 80px rgba(0,0,0,0.35)",
        ...
        <div style={{background:"linear-gradient(135deg,#1A3A5C,#0F2340)",padding:"16px 20px",
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            <div>
              <div style={{color:"#fff",fontSize:16,fontWeight:800}}>💳 Charge & Payment</div>
              <div style={{color:"rgba(255,255,255,0.5)",fontSize:11,marginTop:1}}>Apply discounts and complete transaction</div>
            </div>
            <button onClick={onClose} style={{background:"rgba(255,255,255,0.12)",border:"none",
              color:"#fff",width:30,height:30,borderRadius:"50%",cursor:"pointer",fontSize:17,
              display:"flex",alignItems:"center",justifyContent:"center"}}>×</button>
          </div>
          {/* Invoice info bar */}
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            <div style={{padding:"4px 12px",background:"rgba(255,255,255,0.12)",borderRadius:20,fontSize:11,color:"rgba(255,255,255,0.8)",fontWeight:700}}>
              🧾 INV-{vno}
            </div>
            <div style={{padding:"4px 12px",background:"rgba(255,255,255,0.12)",borderRadius:20,fontSize:11,color:"rgba(255,255,255,0.8)",fontWeight:700}}>
              🍽️ KOT #{kotNo}
            </div>
            <div style={{padding:"4px 12px",background:"rgba(255,255,255,0.08)",borderRadius:20,fontSize:11,color:"rgba(255,255,255,0.5)"}}>
              {new Date().toLocaleTimeString("en-SA",{hour:"2-digit",minute:"2-digit"})}
            </div>
          </div>
        </div>

        <div style={{padding:"18px 22px",display:"flex",flexDirection:"column",gap:16,flex:1,overflowY:"auto"}}>

          {/* ── Amount summary ── */}
          <div style={{background:"#F0F7FF",border:"1.5px solid #C5DCF5",borderRadius:14,
            padding:"14px 18px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:8}}>
              <div>
                <div style={{fontSize:11,color:"#5A7A9A",fontWeight:700,marginBottom:2}}>AMOUNT DUE</div>
                <div style={{fontSize:34,fontWeight:900,color:"#1A3A5C",lineHeight:1}}>SAR {finalTotal.toFixed(2)}</div>
              </div>
              <div style={{fontSize:11,color:"#7A9AB5",textAlign:"right",lineHeight:1.8}}>
                <div>Subtotal: SAR {subtotal.toFixed(2)}</div>
                {totalDiscountAmt>0&&<div style={{color:"#D94040",fontWeight:700}}>Discount: −SAR {totalDiscountAmt.toFixed(2)}</div>}
                <div>VAT 15%: SAR {finalVat.toFixed(2)}</div>
                {(totalDiscountAmt>0)&&<div style={{fontSize:10,color:"#aaa"}}>(VAT on discounted amount)</div>}
              </div>
            </div>
          </div>

          {/* ── Customer Details ── */}
          <div style={{background:"#F8FAFF",border:"1.5px solid #E0E8F0",borderRadius:14,padding:"12px 16px"}}>
            <div style={{fontSize:11,fontWeight:800,color:"#3A5A7A",marginBottom:10,letterSpacing:"0.05em"}}>👤 CUSTOMER (OPTIONAL)</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,position:"relative"}}>
              <div>
                <div style={{fontSize:10,color:"#8A9AB0",fontWeight:600,marginBottom:4}}>Phone / Mobile</div>
                <input value={custPhone} onChange={e=>handlePhoneChange(e.target.value)}
                  placeholder="05xxxxxxxx" type="tel"
                  style={{width:"100%",padding:"9px 12px",border:"1.5px solid #E0E8F0",borderRadius:9,
                    fontSize:14,fontFamily:"inherit",color:"#1A3A5C"}}/>
                {custSuggestions.length>0&&(
                  <div style={{position:"absolute",top:"100%",left:0,right:0,background:"#fff",border:"1px solid #E0E8F0",borderRadius:10,boxShadow:"0 8px 24px rgba(0,0,0,0.12)",zIndex:50,marginTop:2}}>
                    {custSuggestions.map(c=>(
                      <div key={c.id} onClick={()=>selectCustomer(c)}
                        style={{padding:"10px 14px",cursor:"pointer",borderBottom:"1px solid #F0F4F8",display:"flex",justifyContent:"space-between"}}
                        onMouseEnter={e=>e.currentTarget.style.background="#F0F7FF"}
                        onMouseLeave={e=>e.currentTarget.style.background="#fff"}>
                        <div>
                          <div style={{fontSize:13,fontWeight:700,color:"#1A3A5C"}}>{c.name}</div>
                          <div style={{fontSize:11,color:"#8A9AB0"}}>{c.phone}</div>
                        </div>
                        {c.totalSpent>0&&<div style={{fontSize:11,color:"#1A6B4A",fontWeight:700,alignSelf:"center"}}>SAR {c.totalSpent?.toFixed(0)}</div>}
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div>
                <div style={{fontSize:10,color:"#8A9AB0",fontWeight:600,marginBottom:4}}>Name</div>
            ...
            {custName&&<div style={{marginTop:8,fontSize:11,color:"#1A6B4A",fontWeight:600}}>✓ Customer: {custName}{custPhone?" · "+custPhone:""}</div>}
          </div>

          {/* ── Invoice Note ── */}
          <div style={{background:"#FFFDF0",border:"1.5px solid #F0E8C0",borderRadius:14,padding:"12px 16px"}}>
            <div style={{fontSize:11,fontWeight:800,color:"#8A7000",marginBottom:8,letterSpacing:"0.05em"}}>📝 INVOICE NOTE (OPTIONAL)</div>
            <input value={invoiceNote} onChange={e=>setInvoiceNote(e.target.value)}
              placeholder="e.g. No onions, Birthday, VIP, Delivery note..."
              style={{width:"100%",padding:"9px 12px",border:"1.5px solid #F0E8C0",borderRadius:9,
                fontSize:13,fontFamily:"inherit",color:"#5A4A00"}}/>
          </div>

          {/* ── Discount section ── */}
          <div style={{background:"#FFFBF0",border:"1.5px solid #F0E0A0",borderRadius:14,padding:"14px 18px"}}>
            <div style={{fontSize:12,fontWeight:800,color:"#8A6000",marginBottom:10}}>🏷️ DISCOUNT</div>

            {/* Manual discount */}
            <div style={{marginBottom:10}}>
              <div style={{fontSize:11,color:"#8A7A5A",fontWeight:600,marginBottom:6}}>Manual Discount</div>
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                <input value={manualDiscount} onChange={e=>setManualDiscount(e.target.value)}
                  type="number" min="0" placeholder="0"
                  style={{flex:1,padding:"9px 12px",border:"1.5px solid #E8D090",borderRadius:10,
                    fontSize:16,fontWeight:700,fontFamily:"inherit",color:"#1A3A5C",textAlign:"center"}}/>
                <div style={{display:"flex",borderRadius:10,overflow:"hidden",border:"1.5px solid #E8D090"}}>
                  {["%","SAR"].map(t=>(
                    <button key={t} onClick={()=>setDiscountType(t)}
                      style={{padding:"9px 14px",background:discountType===t?"#F0A500":"#fff",
                        color:discountType===t?"#fff":"#8A7A5A",border:"none",cursor:"pointer",
                ...
                {manualDiscountAmt>0&&(
                  <div style={{fontSize:12,color:"#D94040",fontWeight:700,whiteSpace:"nowrap"}}>
                    −SAR {manualDiscountAmt.toFixed(2)}
                  </div>
                )}
              </div>
            </div>
            ...
            {/* Coupon code */}
            <div>
              <div style={{fontSize:11,color:"#8A7A5A",fontWeight:600,marginBottom:6}}>Coupon Code</div>
              <div style={{display:"flex",gap:8}}>
                <input value={promoCode} onChange={e=>{setPromoCode(e.target.value.toUpperCase());setPromoError("");}}
                  onKeyDown={e=>e.key==="Enter"&&applyPromo()}
                  placeholder="e.g. SAVE20"
                  style={{flex:1,padding:"9px 12px",border:`1.5px solid ${promoError?"#D94040":appliedPromo?"#1A8A4A":"#E8D090"}`,
                    borderRadius:10,fontSize:14,fontFamily:"inherit",fontWeight:600,
                    textTransform:"uppercase",letterSpacing:"0.05em"}}/>
                <button onClick={applyPromo}
                  style={{padding:"9px 18px",background:"#1A3A5C",color:"#fff",border:"none",
                    borderRadius:10,cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:"inherit",
                    whiteSpace:"nowrap"}}>
                  Apply
                </button>
              </div>
              {promoError&&<div style={{fontSize:11,color:"#D94040",marginTop:4}}>⚠️ {promoError}</div>}
              {appliedPromo&&<div style={{fontSize:12,color:"#1A8A4A",marginTop:4,fontWeight:700}}>
                ✓ {appliedPromo.code} — {appliedPromo.type==="%"?appliedPromo.value+"% off":"SAR "+appliedPromo.value+" off"}
                {" "}(−SAR {promoDiscountAmt.toFixed(2)})
                <button onClick={()=>{setAppliedPromo(null);setPromoCode("");}}
                  style={{marginLeft:8,background:"none",border:"none",color:"#D94040",cursor:"pointer",fontSize:11}}>
                  ✕ Remove
                </button>
              </div>}
            </div>
          ...
          {/* ── Payment method ── */}
          <div>
            <div style={{fontSize:12,fontWeight:800,color:"#3A5A7A",marginBottom:10}}>💳 PAYMENT METHOD</div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:7}}>
              {METHODS.map(m=>(
                <button key={m.id} onClick={()=>{setMethod(m.id);setSplitError("");setCashGiven("");setCardAmount("");setCashAmount("");}}
                  style={{padding:"10px 6px",border:`2px solid ${method===m.id?"#1A3A5C":"#E0E8F0"}`,
                    background:method===m.id?"#1A3A5C":"#fff",
                    color:method===m.id?"#fff":"#5A7A9A",borderRadius:10,cursor:"pointer",
            ...
            {method==="Cash"&&(
              <div style={{marginTop:12}}>
                <div style={{fontSize:11,fontWeight:700,color:"#5A7A9A",marginBottom:6}}>AMOUNT GIVEN</div>
                <input value={cashGiven} onChange={e=>setCashGiven(e.target.value)} type="number"
                  placeholder="0.00" autoFocus
                  style={{width:"100%",padding:"12px 16px",border:`2px solid ${shortfall?"#D94040":"#E0E8F0"}`,
                    borderRadius:10,fontSize:24,fontWeight:800,fontFamily:"inherit",
                    color:"#1A3A5C",textAlign:"center"}}/>
                {/* Denomination pad */}
                <div style={{marginTop:8}}>
                  <div style={{fontSize:10,color:"#8A9AB0",fontWeight:700,marginBottom:5}}>CASH DENOMINATIONS</div>
                  <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                    {DENOMS.map(d=>(
                      <button key={d} onClick={()=>addDenom(d)}
                        style={{padding:"6px 10px",
                          background:(denominations[d]||0)>0?"#1A3A5C":"#F0F7FF",
                          color:(denominations[d]||0)>0?"#fff":"#1A3A5C",
                          border:"1.5px solid #C5DCF5",borderRadius:8,cursor:"pointer",
                          fontFamily:"inherit",fontSize:11,fontWeight:700,minWidth:38,
                          position:"relative",textAlign:"center"}}>
                        {d}
                        {(denominations[d]||0)>0&&(
                          <span style={{position:"absolute",top:-6,right:-6,background:"#F0A500",
                    ...
                    <button onClick={()=>setCashGiven(finalTotal.toFixed(2))}
                      style={{padding:"6px 10px",background:"#E8F5EE",color:"#1A6B4A",
                        border:"1.5px solid #A8D5B8",borderRadius:8,cursor:"pointer",
                        fontFamily:"inherit",fontSize:11,fontWeight:700}}>
                      Exact
                    </button>
                    {Object.values(denominations).some(v=>v>0)&&(
                      <button onClick={clearDenoms}
                        style={{padding:"6px 10px",background:"rgba(239,68,68,0.1)",color:"#D94040",
                          border:"1.5px solid rgba(239,68,68,0.3)",borderRadius:8,cursor:"pointer",
                          fontFamily:"inherit",fontSize:11,fontWeight:700}}>
                        ✕ Clear
                      </button>
                    )}
                  ...
                  {Object.values(denominations).some(v=>v>0)&&(
                    <div style={{marginTop:6,fontSize:11,color:"#5A7A9A"}}>
                      {DENOMS.filter(d=>(denominations[d]||0)>0).map(d=>`${denominations[d]}×${d}`).join(" + ")} = SAR {denomTotal.toFixed(2)}
                    </div>
                  )}
                </div>
                {cashFloat>0&&(
                  <div style={{marginTop:10,background:shortfall?"#FDE8E8":"#E8F5EE",
                    border:`1.5px solid ${shortfall?"#D94040":"#1A8A4A"}`,borderRadius:12,
                    padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <span style={{fontSize:13,fontWeight:700,color:shortfall?"#D94040":"#1A6B4A"}}>
                      {shortfall?"⚠️ Shortfall":"✓ Change"}
                    </span>
                    <span style={{fontSize:24,fontWeight:900,color:shortfall?"#D94040":"#1A6B4A"}}>
                      SAR {shortfall?(finalTotal-cashFloat).toFixed(2):change.toFixed(2)}
                    </span>
                  </div>
                )}
              </div>
            ...
            {/* Both (Cash + Card) */}
            {method==="Both"&&(
              <div style={{marginTop:12,background:"#F8FAFF",border:"1.5px solid #C5DCF5",
                borderRadius:12,padding:"14px 16px"}}>
                <div style={{fontSize:11,fontWeight:700,color:"#5A7A9A",marginBottom:10}}>
                  SPLIT PAYMENT — Total: SAR {finalTotal.toFixed(2)}
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                  <div>
                    <div style={{fontSize:11,color:"#5A7A9A",fontWeight:600,marginBottom:4}}>💵 Cash Amount</div>
                    <input value={cashAmount} onChange={e=>{setCashAmount(e.target.value);setSplitError("");}}
                      type="number" placeholder="0.00"
                      style={{width:"100%",padding:"10px 12px",border:"1.5px solid #C5DCF5",borderRadius:10,
                        fontSize:18,fontWeight:800,fontFamily:"inherit",color:"#1A3A5C",textAlign:"center"}}/>
                  </div>
                  <div>
                    <div style={{fontSize:11,color:"#5A7A9A",fontWeight:600,marginBottom:4}}>💳 Card Amount</div>
                ...
                {bothCash>0||bothCard>0?(
                  <div style={{marginTop:10,padding:"10px 14px",borderRadius:10,
                    background:bothValid?"#E8F5EE":"#FDE8E8",
                    border:`1.5px solid ${bothValid?"#1A8A4A":"#D94040"}`}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:13,fontWeight:700}}>
                      <span style={{color:bothValid?"#1A6B4A":"#D94040"}}>
                        {bothValid?"✓ Amounts match":"⚠️ Mismatch"}
                      </span>
                      <span style={{color:bothValid?"#1A6B4A":"#D94040"}}>
                        {bothValid?"":"Difference: SAR "+Math.abs(bothDiff).toFixed(2)}
                      </span>
                    </div>
                    {!bothValid&&<div style={{fontSize:11,color:"#D94040",marginTop:4}}>
                      Cash + Card = SAR {bothTotal.toFixed(2)} · Need SAR {finalTotal.toFixed(2)}
                    </div>}
                  </div>
                ):null}
              </div>
          ...
          {/* ── Bottom options ── */}
          <div style={{background:"#F8FAFF",border:"1.5px solid #E0E8F0",borderRadius:12,padding:"14px 16px",display:"flex",flexDirection:"column",gap:10}}>
            <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center",justifyContent:"space-between"}}>
              {/* Print & Save toggle */}
              <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",flex:1,minWidth:160}}>
                <div onClick={()=>{setPrintAndSave(x=>!x);setSaveWithoutPrint(false);}}
                  style={{width:44,height:24,borderRadius:12,
                    background:(!saveWithoutPrint&&printAndSave)?"#1A6B4A":"#CBD5E0",
                    position:"relative",transition:"background 0.2s",cursor:"pointer",flexShrink:0}}>
                  <div style={{position:"absolute",top:2,left:(!saveWithoutPrint&&printAndSave)?22:2,width:20,height:20,
                    borderRadius:"50%",background:"#fff",transition:"left 0.2s",
                    boxShadow:"0 1px 4px rgba(0,0,0,0.2)"}}/>
                </div>
                <div>
                  <div style={{fontSize:12,fontWeight:700,color:"#1A3A5C"}}>
                    {saveWithoutPrint?"💾 Save Without Print":printAndSave?"🖨️ Print & Save":"💾 Save Only"}
                  </div>
                  <div style={{fontSize:10,color:"#8A9AB0"}}>
                    {saveWithoutPrint?"Saved to daily sales, no receipt":"Receipt prints after payment"}
                  </div>
                </div>
              </label>
              {/* Test printer */}
              <button onClick={testPrinter}
                style={{padding:"7px 12px",background:"#fff",border:"1.5px solid #C5DCF5",
                  borderRadius:9,color:"#1A3A5C",fontSize:11,fontWeight:700,
                  cursor:"pointer",fontFamily:"inherit"}}>
                🧪 Test
              </button>
            </div>

            {/* Save without print checkbox */}
            <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",
              padding:"8px 12px",background:saveWithoutPrint?"#EEF2FF":"#fff",
              border:`1.5px solid ${saveWithoutPrint?"#6366f1":"#E0E8F0"}`,borderRadius:9,
              transition:"all 0.15s"}}>
              <div onClick={()=>{setSaveWithoutPrint(x=>!x);}} style={{width:20,height:20,borderRadius:5,
                background:saveWithoutPrint?"#6366f1":"#fff",border:`2px solid ${saveWithoutPrint?"#6366f1":"#CBD5E0"}`,
                display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,cursor:"pointer",transition:"all 0.15s"}}>
                {saveWithoutPrint&&<span style={{color:"#fff",fontSize:13,fontWeight:900}}>✓</span>}
              </div>
              <div>
                <div style={{fontSize:12,fontWeight:700,color:saveWithoutPrint?"#6366f1":"#1A3A5C"}}>💾 Save Without Print</div>
                <div style={{fontSize:10,color:"#8A9AB0"}}>Invoice saved to daily sales — no receipt printed</div>
              </div>
            </label>

            {/* Draft Bill checkbox */}
            <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",
              padding:"8px 12px",background:isDraft?"#FFF8E8":"#fff",
              border:`1.5px solid ${isDraft?"#F0A500":"#E0E8F0"}`,borderRadius:9,
              transition:"all 0.15s"}}>
              <div onClick={()=>setIsDraft(x=>!x)} style={{width:20,height:20,borderRadius:5,
                background:isDraft?"#F0A500":"#fff",border:`2px solid ${isDraft?"#F0A500":"#CBD5E0"}`,
                display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,cursor:"pointer",transition:"all 0.15s"}}>
                {isDraft&&<span style={{color:"#fff",fontSize:13,fontWeight:900}}>✓</span>}
              </div>
              <div style={{flex:1}}>
                <div style={{fontSize:12,fontWeight:700,color:isDraft?"#A07000":"#1A3A5C"}}>📋 Draft Bill (D-Invoice)</div>
                <div style={{fontSize:10,color:"#8A9AB0"}}>Prints without ZATCA QR — numbered as {isDraft?"D-Invoice (e.g. 11A, 11B...)":"regular invoice"}</div>
              </div>
              {isDraft&&<span style={{fontSize:10,fontWeight:700,color:"#F0A500",background:"rgba(240,165,0,0.1)",padding:"2px 8px",borderRadius:20,border:"1px solid rgba(240,165,0,0.3)",whiteSpace:"nowrap"}}>No QR</span>}
            </label>
          </div>
          ...
          <button onClick={handleConfirm} disabled={!canConfirm}
            style={{width:"100%",padding:"15px",
              background:canConfirm
                ?"linear-gradient(135deg,#1A6B4A,#0F4A30)"
                :"#ccc",
              color:"#fff",border:"none",borderRadius:12,fontSize:15,fontWeight:800,
              cursor:canConfirm?"pointer":"not-allowed",fontFamily:"inherit",
              boxShadow:canConfirm?"0 4px 20px rgba(26,107,74,0.35)":"none",
              transition:"all 0.2s"}}>
            {!canConfirm&&method==="Cash"&&cashFloat>0?"⚠️ Insufficient Amount":
             !canConfirm&&method==="Both"?"⚠️ Fix Split Amounts":
             isDraft?`📋 Save Draft Bill (${method})`:
             saveWithoutPrint?`💾 Save Without Print (${method})`:
             `✓ Confirm ${method==="Both"?"Split":method} Payment${(!saveWithoutPrint&&printAndSave)?" & Print":""}`}
          </button>
        </div>
      </div>
      <iframe ref={printFrameRef} style={{display:"none",width:0,height:0,border:"none"}} title="pay-print-frame"/>
    </div>
  );
}
// ═══════════════════════════════════════════════════════════════════
...
function POS({items,sales,setSales,tables,setTables,promos,license,lang="en"}){
  ...
  async function confirmPayment(method,given,change,promo,totalDiscountAmt,finalTotal,finalVat,printAndSave,payInfo,manualDiscountAmt,promoDiscountAmt,isDraft=false,extraData={}){
    ...
    const inv={
      ...
      type:orderType==="dine-in"?"Dine-in":orderType==="takeaway"?"Takeaway":"Delivery",
    ...
    // Save to sales (drafts included) AND to separate draft store
    setSales(prev=>[...prev,inv]);setLastOrder(inv);
    if(isDraft){
      // Save to draft invoices list
      const drafts=LS.get("restopos_draft_invoices")||[];
      LS.set("restopos_draft_invoices",[...drafts,inv]);
      setLastZatcaInvoice(null); // No ZATCA for drafts
    }else{
      try{
        const zatcaInv=await generateZATCAInvoice({
          seller_name:license.businessName,seller_vat:license.vatNumber,
    ...
    if(isDraft&&printAndSave){
      // Draft: print without ZATCA QR — no dialog
      printDraftReceipt(inv,license);
    }else if(printAndSave){
      // Normal: try ESC/POS first (no dialog)
      let escPrinted=false;
      try{
        if("serial" in navigator&&isPortOpen(_billPort)){
          await printReceiptEscPos(inv,license);
          escPrinted=true;
        }
      }catch(e){console.warn("Auto ESC/POS print failed:",e);}
      if(!escPrinted){
        // Fallback: silent iframe print (no dialog)
        try{
          const html=buildReceiptHTML(inv,license,null,LS.get("restopos_invoice_format")||{});
          ...
          // Last fallback: show receipt modal
          setShowReceipt(true);
        }
      }
    }
    // Save without print — just close
  ...
  async function printKOT(){
    ...
    // Try ESC/POS kitchen printer first
    try{
      if("serial" in navigator&&(isPortOpen(_kitchenPort)||(await getAvailablePorts()).length>0)){
  ...
  return(
    <div style={{display:"flex",height:"calc(100vh - 52px)",overflow:"hidden"}}>
      {showPayment&&<PaymentModal total={total} subtotal={subtotal} vat={vat} promos={promos} license={license} vno={vno} kotNo={kotNo} customers={LS.get("restopos_customers")||[]} customerName={customerName} customerPhone={customerPhone} onConfirm={confirmPayment} onClose={()=>setShowPayment(false)}/>}
...
function Reports({sales,allSales,items,setSales}){
  ...
  function buildSummaryData(salesArr){
    const expenses=LS.get("restopos_expenses")||[];
    const dayExpenses=expenses.filter(e=>salesArr.length>0&&e.date===salesArr[0].date);
    const revenue=salesArr.reduce((s,o)=>s+o.total,0);
    const vat=salesArr.reduce((s,o)=>s+(o.vat||0),0);
    const discount=salesArr.reduce((s,o)=>s+(o.discount||0),0);
    const payBreakdown={};
    salesArr.forEach(o=>{payBreakdown[o.payMethod]=(payBreakdown[o.payMethod]||0)+o.total;});
    const itemMap={};
    salesArr.forEach(o=>(o.items||[]).forEach(it=>{
      if(!itemMap[it.id])itemMap[it.id]={name:it.name,nameAr:it.nameAr||"",category:it.category||"Items",qty:0,revenue:0,price:it.price};
    ...
    const catMap={};
    itemList.forEach(it=>{
      if(!catMap[it.category])catMap[it.category]={cat:it.category,qty:0,revenue:0,items:[]};
      catMap[it.category].qty+=it.qty;
      catMap[it.category].revenue+=it.revenue;
      catMap[it.category].items.push(it);
    });
    const catList=Object.values(catMap).sort((a,b)=>b.revenue-a.revenue);
    const expenseTotal=dayExpenses.reduce((s,e)=>s+e.amount,0);
    return{date:salesArr[0]?.date||TODAY,orderCount:salesArr.length,revenue,vat,discount,payBreakdown,itemList,catList,expenses:expenseTotal,netRevenue:revenue-expenseTotal};
  }
...
<div class="c big">DAILY SUMMARY</div>
<div class="c">${data.date} · ${LS.get("restopos_license_v2")?.businessName||"Restaurant"}</div>
<div class="c" style="font-size:10px">Printed: ${new Date().toLocaleString("en-SA")}</div>
<div class="hr"/>
${thermal?`<div class="row b"><span>Total Orders</span><span>${data.orderCount}</span></div>
<div class="row b"><span>Total Revenue</span><span>SAR ${data.revenue.toFixed(2)}</span></div>
<div class="row"><span>VAT Collected</span><span>SAR ${data.vat.toFixed(2)}</span></div>
${data.discount>0?`<div class="row"><span>Total Discount</span><span>-SAR ${data.discount.toFixed(2)}</span></div>`:""}
<div class="hr"/>
...
${data.catList.map(c=>`<div class="b">[${c.cat}]</div>${c.items.map(it=>`<div class="row"><span>  ${it.qty}x ${it.name.slice(0,22)}</span><span>SAR ${it.revenue.toFixed(2)}</span></div>`).join("")}<div class="row b"><span>  Total</span><span>SAR ${c.revenue.toFixed(2)}</span></div>`).join("")}
<div class="hr"/>
...
<table><thead><tr><th>Category</th><th>Qty</th><th>Revenue</th></tr></thead><tbody>
${data.catList.map(c=>`<tr><td>${c.cat}</td><td>${c.qty}</td><td><b>SAR ${c.revenue.toFixed(2)}</b></td></tr>`).join("")}
</tbody></table></div></div>
<div class="hr"/>
<div style="font-size:13px;font-weight:700;margin-bottom:8px">Item Sales Detail</div>
<table><thead><tr><th>Item</th><th>Category</th><th>Price</th><th>Qty</th><th>Revenue</th></tr></thead><tbody>
${data.itemList.map(it=>`<tr><td>${it.name}${it.nameAr?`<br><span style="direction:rtl;font-family:Tajawal,sans-serif;font-size:10px;color:#888">${it.nameAr}</span>`:""}</td><td>${it.category}</td><td>SAR ${it.price.toFixed(2)}</td><td><b>${it.qty}</b></td><td><b>SAR ${it.revenue.toFixed(2)}</b></td></tr>`).join("")}
<tr class="total-row"><td colspan="3">TOTAL</td><td>${data.itemList.reduce((s,i)=>s+i.qty,0)}</td><td>SAR ${data.revenue.toFixed(2)}</td></tr>
</tbody></table>
${data.expenses>0?`<div class="hr"/><div class="row b"><span>Expenses</span><span>-SAR ${data.expenses.toFixed(2)}</span></div><div class="row b" style="font-size:14px"><span>NET REVENUE</span><span>SAR ${data.netRevenue.toFixed(2)}</span></div>`:""}`}
<br/><br/></body></html>`;
    if(thermal&&"serial" in navigator&&isPortOpen(_billPort)){
      const parts=[escpos.init(),escpos.alignCenter(),escpos.bold(true),escpos.doubleSize(true),escpos.text("DAILY SUMMARY"),escpos.doubleSize(false),escpos.bold(false),escpos.text(data.date+" · "+data.orderCount+" orders"),escpos.text("------------------------------------------"),escpos.alignLeft(),escpos.bold(true)];
      const ln=(l,r="")=>{const g=W-l.length-r.length;return escpos.text(l+(g>0?" ".repeat(g):"")+r);};
      parts.push(ln("TOTAL REVENUE","SAR "+data.revenue.toFixed(2)));
      parts.push(escpos.bold(false));
      parts.push(ln("VAT","SAR "+data.vat.toFixed(2)));
      parts.push(escpos.text("-- PAYMENT --"));
      Object.entries(data.payBreakdown).filter(([,v])=>v>0).forEach(([k,v])=>parts.push(ln(k,"SAR "+v.toFixed(2))));
      parts.push(escpos.text("-- BY CATEGORY --"));
      data.catList.forEach(c=>{parts.push(escpos.bold(true),escpos.text(c.cat),escpos.bold(false));c.items.forEach(it=>parts.push(ln("  "+it.qty+"x "+it.name.slice(0,22),"SAR "+it.revenue.toFixed(2))));parts.push(ln("  Total","SAR "+c.revenue.toFixed(2)));});
      parts.push(escpos.text("------------------------------------------"),escpos.bold(true),ln("NET REVENUE","SAR "+data.netRevenue.toFixed(2)),escpos.bold(false),escpos.text(""),escpos.feed(4),escpos.cutPaper());
      const d2=escpos.merge(...parts);
      printEscPos(d2,"bill").catch(()=>{const w=window.open("","_blank","width=380,height=600");if(w){w.document.write(html);w.document.close();setTimeout(()=>{w.print();w.close();},500);}});
      return;
    }
  ...
  function handleCloseDay(){
    ...
    const payBreakdown={Cash:todaySales.filter(s=>s.payMethod==="Cash").reduce((s,o)=>s+o.total,0),Mada:todaySales.filter(s=>s.payMethod==="Mada").reduce((s,o)=>s+o.total,0),"Apple Pay":todaySales.filter(s=>s.payMethod==="Apple Pay").reduce((s,o)=>s+o.total,0),"Card":todaySales.filter(s=>s.payMethod==="Card").reduce((s,o)=>s+o.total,0),"Both":todaySales.filter(s=>s.payMethod==="Both").reduce((s,o)=>s+o.total,0)};
    ...
    const sd=buildSummaryData(todaySales);
    setDaySummaryData(sd);
  ...
  function setPreset(preset){
    ...
    else if(preset==="yesterday"){const y=new Date(today);y.setDate(y.getDate()-1);const ys=fmt(y);setDateFrom(ys);setDateTo(ys);}
    else if(preset==="week"){const w=new Date(today);w.setDate(w.getDate()-6);setDateFrom(fmt(w));setDateTo(TODAY);}
    else if(preset==="month"){const m=new Date(today);m.setDate(1);setDateFrom(fmt(m));setDateTo(TODAY);}
    else if(preset==="last_month"){const lm=new Date(today);lm.setMonth(lm.getMonth()-1);const start=new Date(lm.getFullYear(),lm.getMonth(),1);const end=new Date(lm.getFullYear(),lm.getMonth()+1,0);setDateFrom(fmt(start));setDateTo(fmt(end));}
    else if(preset==="3months"){const m3=new Date(today);m3.setMonth(m3.getMonth()-3);setDateFrom(fmt(m3));setDateTo(TODAY);}
    else if(preset==="year"){setDateFrom(today.getFullYear()+"-01-01");setDateTo(TODAY);}
  }
  ...
  function exportToExcel(type){
    ...
    if(type==="sales"){
      csvRows.push(["Invoice No","Date","Time","Type","Table","Customer","Items","Subtotal","Discount","VAT","Total","Payment Method","Cash Given","Change","Cashier"].map(esc).join(","));
      filtered.forEach(s=>{
        csvRows.push([
          s.id,s.date,s.time,s.type,s.table||"",s.customer||"",
          (s.items||[]).map(i=>i.qty+"x "+i.name).join("; "),
          (s.subtotal||0).toFixed(2),(s.discount||0).toFixed(2),
          (s.vat||0).toFixed(2),(s.total||0).toFixed(2),
          s.payMethod,s.given||"",s.change||"",s.cashier||""
        ].map(esc).join(","));
      });
    }else if(type==="items"){
      const itemMap={};
      filtered.forEach(o=>(o.items||[]).forEach(it=>{
        if(!itemMap[it.id])itemMap[it.id]={name:it.name,nameAr:it.nameAr||"",category:it.category||"",qty:0,revenue:0,price:it.price};
        itemMap[it.id].qty+=it.qty;itemMap[it.id].revenue+=it.qty*it.price;
      }));
      const list=Object.values(itemMap).sort((a,b)=>b.revenue-a.revenue);
      csvRows.push(["Item Name","Arabic Name","Category","Unit Price","Qty Sold","Revenue"].map(esc).join(","));
      list.forEach(it=>csvRows.push([it.name,it.nameAr,it.category,it.price.toFixed(2),it.qty,it.revenue.toFixed(2)].map(esc).join(",")));
    }else if(type==="category"){
      const catMap={};
      filtered.forEach(o=>(o.items||[]).forEach(it=>{
        const cat=it.category||"Items";
        if(!catMap[cat])catMap[cat]={category:cat,qty:0,revenue:0};
        catMap[cat].qty+=it.qty;catMap[cat].revenue+=it.qty*it.price;
      }));
      csvRows.push(["Category","Items Sold","Revenue"].map(esc).join(","));
      Object.values(catMap).sort((a,b)=>b.revenue-a.revenue).forEach(c=>csvRows.push([c.category,c.qty,c.revenue.toFixed(2)].map(esc).join(",")));
    }else if(type==="summary"){
      const totalRev=filtered.reduce((s,o)=>s+o.total,0);
      const totalVat=filtered.reduce((s,o)=>s+(o.vat||0),0);
      ...
      csvRows.push(["Metric","Value"].map(esc).join(","));
      csvRows.push([esc("Business"),esc(license.businessName)].join(","));
      csvRows.push([esc("Period"),esc(dateFrom+" to "+dateTo)].join(","));
      csvRows.push([esc("Total Orders"),esc(filtered.length)].join(","));
      csvRows.push([esc("Total Revenue"),esc("SAR "+totalRev.toFixed(2))].join(","));
      csvRows.push([esc("Total VAT"),esc("SAR "+totalVat.toFixed(2))].join(","));
      csvRows.push([esc("Total Discount"),esc("SAR "+totalDiscount.toFixed(2))].join(","));
      csvRows.push([esc("Net Revenue"),esc("SAR "+(totalRev-totalDiscount).toFixed(2))].join(","));
      csvRows.push(["",""].join(","));
      csvRows.push([esc("Payment Method"),esc("Amount")].join(","));
      Object.entries(payBr).filter(([,v])=>v>0).forEach(([k,v])=>csvRows.push([esc(k),esc("SAR "+v.toFixed(2))].join(",")));
    }
    ...
    a.href=url;
    a.download=`RestoPOS_${type}_${dateFrom}_${dateTo}.csv`;
  ...
  const DateFilter=()=>(
    <Card style={{marginBottom:16}}>
      ...
      <div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap"}}>
        {[["today","Today"],["yesterday","Yesterday"],["week","Last 7 Days"],["month","This Month"],["last_month","Last Month"],["3months","3 Months"],["year","This Year"]].map(([id,label])=>(
          <button key={id} onClick={()=>setPreset(id)}
            style={{padding:"5px 12px",borderRadius:20,border:`1.5px solid ${C.border}`,
              background:C.bg,color:C.textMid,fontFamily:"inherit",fontSize:11,
      ...
      <div style={{display:"flex",gap:12,alignItems:"flex-end",flexWrap:"wrap"}}>
        <div style={{display:"flex",gap:8,alignItems:"center",flex:1,minWidth:260}}>
          ...
          <span style={{color:C.textLight,marginBottom:2}}>→</span>
          <Inp label="To" value={dateTo} onChange={setDateTo} type="date"/>
        </div>
        <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:11,color:C.textMid}}>{filtered.length} orders · {fmtSAR(filtered.reduce((s,o)=>s+o.total,0))}</div>
            <div style={{fontSize:10,color:C.textLight}}>{dateFrom===dateTo?"Single day":dateFrom+" → "+dateTo}</div>
          </div>
          ...
          <div style={{position:"relative",display:"inline-block"}}>
            ...
            }} style={{padding:"8px 14px",background:"#1A6B4A",color:"#fff",border:"none",borderRadius:8,fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",gap:5}}>
              📊 Export Excel ▾
            </button>
            <div id="excel-menu" style={{display:"none",position:"absolute",right:0,top:"100%",marginTop:4,background:"#fff",border:`1px solid ${C.border}`,borderRadius:10,boxShadow:"0 4px 20px rgba(0,0,0,0.15)",zIndex:100,minWidth:180}}>
              {[["summary","📋 Summary Report"],["sales","🧾 All Invoices"],["items","🍔 Item Sales"],["category","📂 By Category"]].map(([type,label])=>(
                <button key={type} onClick={()=>{exportToExcel(type);document.getElementById("excel-menu").style.display="none";}}
                  style={{display:"block",width:"100%",padding:"10px 16px",background:"none",border:"none",textAlign:"left",fontSize:13,cursor:"pointer",fontFamily:"inherit",color:C.text,borderBottom:`1px solid ${C.border}`}}>
                  {label}
                </button>
              ))}
            </div>
  ...
  return(<div>
    ...
    {showDaySummary&&daySummaryData&&(
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:3000,display:"flex",alignItems:"center",justifyContent:"center",padding:16,overflowY:"auto"}}>
        <div style={{background:"#fff",borderRadius:20,width:"100%",maxWidth:600,maxHeight:"92vh",overflow:"auto",boxShadow:"0 24px 80px rgba(0,0,0,0.35)"}}>
          {/* Header */}
          <div style={{background:"linear-gradient(135deg,#1A3A5C,#0F2340)",padding:"18px 22px",borderRadius:"20px 20px 0 0",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,zIndex:10}}>
            <div>
              <div style={{color:"#fff",fontSize:17,fontWeight:800}}>📊 Day Closed — Summary Report</div>
              <div style={{color:"rgba(255,255,255,0.55)",fontSize:11,marginTop:2}}>{daySummaryData.date} · {daySummaryData.orderCount} orders</div>
            </div>
            <button onClick={()=>setShowDaySummary(false)} style={{background:"rgba(255,255,255,0.12)",border:"none",color:"#fff",width:32,height:32,borderRadius:"50%",cursor:"pointer",fontSize:18}}>×</button>
          </div>
          ...
          <div style={{padding:"18px 22px",display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
              {[["Orders",daySummaryData.orderCount,C.info],["Revenue","SAR "+daySummaryData.revenue.toFixed(2),C.primary],["VAT","SAR "+daySummaryData.vat.toFixed(2),C.zatca],["Net","SAR "+daySummaryData.netRevenue.toFixed(2),C.success]].map(([l,v,c])=>(
                <div key={l} style={{background:c+"11",border:`1.5px solid ${c}33`,borderRadius:10,padding:"10px 12px",textAlign:"center"}}>
            ...
            <div style={{background:C.bg,borderRadius:10,padding:"12px 14px"}}>
              <div style={{fontSize:12,fontWeight:700,marginBottom:8}}>💳 Payment Breakdown</div>
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                {Object.entries(daySummaryData.payBreakdown).filter(([,v])=>v>0).map(([k,v])=>(
                  <div key={k} style={{background:C.infoLight,border:`1px solid ${C.info}33`,borderRadius:8,padding:"6px 12px"}}>
                    <div style={{fontSize:10,color:C.info,fontWeight:700}}>{k}</div>
                    <div style={{fontSize:13,fontWeight:800,color:C.info}}>SAR {v.toFixed(2)}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Category summary */}
            <div>
              <div style={{fontSize:12,fontWeight:700,marginBottom:8}}>📂 Category Summary</div>
              {daySummaryData.catList.map(c=>(
                <div key={c.cat} style={{marginBottom:10,border:`1px solid ${C.border}`,borderRadius:10,overflow:"hidden"}}>
                  <div style={{background:C.primaryLight,padding:"8px 12px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <span style={{fontSize:12,fontWeight:700,color:C.primary}}>{c.cat}</span>
                    <span style={{fontSize:12,fontWeight:800,color:C.primary}}>SAR {c.revenue.toFixed(2)}</span>
                  </div>
                  <div style={{padding:"8px 12px"}}>
                    {c.items.map(it=>(
                      <div key={it.name} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"4px 0",borderBottom:`1px solid ${C.border}`,fontSize:12}}>
                        <div>
                          <span style={{fontWeight:600}}>{it.name}</span>
                          {it.nameAr&&<span style={{display:"block",direction:"rtl",fontFamily:"'Tajawal',sans-serif",fontSize:11,color:C.textMid}}>{it.nameAr}</span>}
                        </div>
                        <div style={{textAlign:"right",flexShrink:0,marginLeft:8}}>
                          <div style={{fontWeight:700,color:C.primary}}>×{it.qty}</div>
                          <div style={{fontSize:11,color:C.textMid}}>SAR {it.revenue.toFixed(2)}</div>
                        </div>
                      </div>
                    ))}
                    <div style={{display:"flex",justifyContent:"space-between",padding:"6px 0",fontSize:12,fontWeight:700,color:C.primary}}>
                      <span>Total {c.qty} items</span><span>SAR {c.revenue.toFixed(2)}</span>
                    </div>
                  </div>
                </div>
              ))}
            ...
            {/* Expenses */}
            {daySummaryData.expenses>0&&(
              <div style={{display:"flex",justifyContent:"space-between",padding:"10px 14px",background:C.dangerLight,border:`1px solid ${C.danger}33`,borderRadius:10,fontSize:13,fontWeight:700}}>
                <span style={{color:C.danger}}>Total Expenses</span>
                <span style={{color:C.danger}}>-SAR {daySummaryData.expenses.toFixed(2)}</span>
              </div>
            )}

            {/* Action buttons */}
            <div style={{display:"flex",gap:8,flexWrap:"wrap",paddingTop:4}}>
              <button onClick={()=>printDaySummary(daySummaryData,false)}
                style={{flex:1,padding:"12px",background:"linear-gradient(135deg,#1A3A5C,#0F2340)",color:"#fff",border:"none",borderRadius:10,fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
                📄 View / Print A4 PDF
              </button>
              <button onClick={()=>printDaySummary(daySummaryData,true)}
                style={{flex:1,padding:"12px",background:"linear-gradient(135deg,#1A6B4A,#134D36)",color:"#fff",border:"none",borderRadius:10,fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
                🖨️ Print Thermal Receipt
              </button>
            </div>
            <button onClick={()=>setShowDaySummary(false)}
              style={{width:"100%",padding:"10px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:10,fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",color:C.textMid}}>
              Close
            </button>
          </div>
        </div>
    ...
    {tab==="summary"&&<><DateFilter/>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:16}}>
        <StatCard icon="💰" label="Revenue" value={fmtSAR(filtered.filter(o=>!o.isDraft).reduce((s,o)=>s+o.total,0))} color={C.primary} bg={C.primaryLight}/>
        <StatCard icon="🧾" label="Orders" value={filtered.filter(o=>!o.isDraft).length} color={C.info} bg={C.infoLight}/>
        <StatCard icon="📊" label="VAT" value={fmtSAR(filtered.filter(o=>!o.isDraft).reduce((s,o)=>s+o.vat,0))} color={C.accent} bg={C.accentLight}/>
        <StatCard icon="💵" label="Avg Order" value={fmtSAR(filtered.filter(o=>!o.isDraft).length?filtered.filter(o=>!o.isDraft).reduce((s,o)=>s+o.total,0)/filtered.filter(o=>!o.isDraft).length:0)} color={C.success} bg={C.successLight}/>
      </div>
      {/* D-Invoices Section */}
      {(()=>{
        const draftSales=filtered.filter(o=>o.isDraft);
        if(draftSales.length===0)return null;
        return(
          <Card style={{border:`2px solid #F0A500`,background:"#FFFBF0",marginTop:8}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:10}}>
              <div>
                <div style={{fontSize:14,fontWeight:800,color:"#A07000"}}>📋 D-Invoices (Draft Bills) — {draftSales.length}</div>
                <div style={{fontSize:11,color:"#B08030",marginTop:2}}>Printed without ZATCA QR — not included in official totals above</div>
              </div>
              <button onClick={()=>{
                if(!confirm("Clear all D-Invoices in this period? This cannot be undone."))return;
                const draftIds=new Set(draftSales.map(d=>d.id));
                const remaining=(LS.get("restopos_draft_invoices")||[]).filter(d=>!draftIds.has(d.id));
                LS.set("restopos_draft_invoices",remaining);
                setSales(prev=>prev.filter(s=>!draftIds.has(s.id)));
                alert("✅ D-Invoices cleared from this period.");
              }} style={{padding:"6px 14px",background:"rgba(239,68,68,0.1)",border:"1.5px solid rgba(239,68,68,0.3)",borderRadius:8,color:"#D94040",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
                🗑️ Clear All D-Invoices
              </button>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:5}}>
              {draftSales.map(d=>(
                <div key={d.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",background:"#fff",borderRadius:8,border:"1px solid #F0E0A0",flexWrap:"wrap",gap:8}}>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    <span style={{fontFamily:"monospace",fontWeight:800,color:"#F0A500",fontSize:13}}>{d.id}</span>
                    <span style={{fontSize:11,color:"#888"}}>{d.date} {d.time}</span>
                    {d.customer&&<span style={{fontSize:11,color:"#666"}}>· {d.customer}</span>}
                  </div>
                  <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
                    <span style={{fontSize:11,color:"#666",maxWidth:200,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{(d.items||[]).map(i=>i.qty+"x "+i.name).join(", ")}</span>
                    <strong style={{color:"#A07000",fontSize:13}}>SAR {(d.total||0).toFixed(2)}</strong>
                    <span style={{fontSize:10,background:"rgba(240,165,0,0.12)",color:"#A07000",padding:"2px 8px",borderRadius:12,border:"1px solid rgba(240,165,0,0.3)",fontWeight:700}}>{d.payMethod||"Cash"}</span>
                  </div>
                </div>
              ))}
            </div>
            <div style={{marginTop:10,paddingTop:8,borderTop:"1px solid #F0E0A0",display:"flex",justifyContent:"space-between",fontSize:13,fontWeight:800}}>
              <span style={{color:"#A07000"}}>Total D-Invoices ({draftSales.length})</span>
              <span style={{color:"#A07000"}}>SAR {draftSales.reduce((s,d)=>s+(d.total||0),0).toFixed(2)}</span>
            </div>
          </Card>
        );
      })()}
          ...
          <Card key={d.date} style={{marginBottom:12}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
              ...
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                ...
                <button onClick={()=>{
                  // Rebuild summary from archived sales for that date
                  const arch=LS.get("restopos_archived_sales")||[];
                  const daySales=arch.filter(s=>s.date===d.date);
                  if(daySales.length===0){alert("Detailed item data not available for this day.");return;}
                  const sd=buildSummaryData(daySales);
                  setDaySummaryData({...sd,date:d.date});
                  setShowDaySummary(true);
                }} style={{padding:"5px 10px",background:C.infoLight,border:`1px solid ${C.info}33`,borderRadius:7,color:C.info,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap"}}>
                  📊 View
                </button>
              </div>
            </div>
...
function CloudSyncStatus(){
  ...
  async function restoreNow(){
    ...
    if(restored)alert("✅ Data restored! Please refresh the page.");
    else alert("No cloud backup found for this license key.");
  }
  return(
    <Card style={{marginBottom:16}}>
      ...
      {activeShift?(
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginBottom:24}}>
          ...
          <Card>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              <StatCard icon="🧾" label={t("Orders",lang)} value={shiftSales.length} color={C.primary} bg={C.primaryLight}/>
              <StatCard icon="💰" label={t("Revenue",lang)} value={fmtSAR(shiftSales.reduce((s,o)=>s+o.total,0))} color={C.success} bg={C.successLight}/>
...
export default function App(){
  const [step,setStep]=useState("checking");const [businessData,setBusinessData]=useState(null);const [license,setLicense]=useState(null);const [currentUser,setCurrentUser]=useState(null);const [screen,setScreen]=useState("pos");
  ...
  return(
    <ErrorBoundary>
    <div dir={dir(lang)} style={{fontFamily:lang==="ar"?"'Tajawal','Plus Jakarta Sans',sans-serif":"'Plus Jakarta Sans','Tajawal',sans-serif",background:C.bg,height:"100vh",display:"flex",flexDirection:"column",overflow:"hidden",fontSize:`${uiScale}%`}}>
      ...
      <div style={{display:"flex",alignItems:"stretch",flexShrink:0,zIndex:100,boxShadow:"0 2px 12px rgba(0,0,0,0.18)",minHeight:50,width:"100%",flexWrap:"nowrap"}}>
        ...
        <div style={{background:"linear-gradient(135deg,#1A3D2B 0%,#1F4D36 100%)",display:"flex",alignItems:"center",gap:4,padding:"0 8px",flexShrink:0,borderLeft:"1px solid rgba(255,255,255,0.1)"}}>
          ...
          {pwaInstall.prompt&&!pwaInstall.installed&&(
            <button onClick={pwaInstall.install} style={{fontSize:9,background:"rgba(26,107,74,0.3)",color:"#7FFAB5",padding:"3px 8px",borderRadius:4,fontWeight:800,border:"1px solid rgba(26,107,74,0.5)",cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap"}}>⬇️ Install App</button>
          )}
