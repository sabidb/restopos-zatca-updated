...
function buildPresetHTML(order,license,zatcaInvoice,fmt,qrImgSrc,opts){
  ...
  const sepLine=style==="s4"?"2px solid #000":style==="s3"?"1px solid #ccc":"1px dashed #000";
  ...
  if(style==="s2"){
    header=`<div style="background:${headerColor};color:#fff;margin:-4mm -4mm 8px;padding:10px 8px;text-align:center;border-radius:0 0 8px 8px">${logoHTML}${headBlock}</div>`;
  }else{
    header=`<div style="text-align:center">${logoHTML}${headBlock}</div>`;
  }
  // ── INVOICE TYPE BOX ──
  const typeBox=isDraft
    ? boxed(`<div style="font-weight:900;font-size:${headFont}px">DRAFT — مسودة</div>`,true)
    : boxed(`<div style="direction:rtl;font-size:${bodyFont}px">فاتورة ضريبية مبسطة</div><div style="font-weight:900;font-size:${headFont}px">Simplified Tax Invoice</div>`,true);
  ...
  const tokenBox=boxed(`<div style="display:flex;justify-content:space-between;align-items:center;gap:8px"><span style="font-weight:900;font-size:${headFont}px">Token No: ${_escHTML(order.token||order.kot||"—")}</span><span style="direction:rtl;font-weight:700;font-size:${headFont}px">${_escHTML(orderTypeAr||order.type||"")}</span></div>`);
  // ── VOUCHER ──
  ...
  // ── AMOUNT IN WORDS + RECEIVED/BALANCE (skip for draft) ──
  const wordsHTML=isDraft?"":`<div style="font-size:${bodyFont-1}px;font-style:italic;margin:5px 0;word-break:break-word">Amount in Words: ${_amountWords(order.total||0)}</div>`;
  const payHTML=isDraft?"":`
  ...
  const qrBlock=(!isDraft&&qrImgSrc)
    ? `<div style="text-align:center;margin:8px 0"><img src="${qrImgSrc}" style="width:${qrSize}px;height:${qrSize}px;max-width:100%;display:block;margin:4px auto"/>${zatcaInvoice?.invoice_number?`<div style="font-size:${bodyFont-3}px;color:#444">Invoice: ${_escHTML(zatcaInvoice.invoice_number)}${zatcaInvoice.icv?" · ICV: "+_escHTML(zatcaInvoice.icv):""}</div>`:""}<div style="font-size:${bodyFont-4}px;color:#777">ZATCA Phase 2 · Scan to verify</div></div>`
    : "";
  const footerHTML=`<div style="text-align:center;margin-top:6px">
    ...
    ${isDraft?`<div style="font-size:9px;color:#aaa;margin-top:4px">DRAFT — Not a tax invoice</div>`:""}
  </div>`;
  return `<!DOCTYPE html><html><head><meta charset="UTF-8">
...
*{box-sizing:border-box;margin:0;padding:0}
html,body{max-width:${paperWidth};overflow-x:hidden}
body{font-family:${bodyFamily};font-size:${bodyFont}px;width:${paperWidth};padding:4mm;color:#000;background:#fff;line-height:1.4;word-break:break-word;overflow-wrap:anywhere}
img{max-width:100%}
</style></head><body>
${header}
${typeBox}
...
// KOT preset builder — 4 styles, kitchen ticket (no totals/QR)
function buildPresetKOT(order,fmt){
  ...
  const logoHTML=fmt.logoUrl?`<img src="${_escHTML(fmt.logoUrl)}" style="max-width:100%;max-height:${logoSize}px;display:block;margin:0 auto 5px"/>`:"";
  const title=`<div style="text-align:center;font-weight:900;font-size:${headFont}px">${style==="s2"?`<span style="background:#000;color:#fff;padding:3px 12px;border-radius:4px">KOT #${_escHTML(order.kot||"")}</span>`:`KOT #${_escHTML(order.kot||"")}`}</div>`;
  const orderTypeAr=order.typeAr||(order.type==="Takeaway"?"سفري":order.type==="Dine-in"?"محلي":"");
  const rows=items.map(it=>{
    ...
    return `<div style="margin:${lineGap}px 0;${style==="s4"?"font-weight:900;":"font-weight:800;"}">
      <div style="font-size:${bodyFont+1}px">${(it.qty||0)} x ${_escHTML(it.name)}</div>${ar}</div>`;
  }).join("");
  return `<!DOCTYPE html><html><head><meta charset="UTF-8">
<link href="https://fonts.googleapis.com/css2?family=Noto+Naskh+Arabic:wght@400;700&family=Amiri:wght@400;700&family=Cairo:wght@400;700&display=swap" rel="stylesheet">
<style>@page{size:${paperWidth} auto;margin:0}*{box-sizing:border-box;margin:0;padding:0}
html,body{max-width:${paperWidth};overflow-x:hidden}
body{font-family:${bodyFamily};font-size:${bodyFont}px;width:${paperWidth};padding:4mm;color:#000;background:#fff;line-height:1.35}
img{max-width:100%}</style></head><body>
${logoHTML}
${title}
${SEP}
<div style="text-align:center;font-size:${bodyFont}px;font-weight:700">${_escHTML((order.type||"").toUpperCase())}${order.table?" · Table "+_escHTML(order.table):""}${orderTypeAr?` <span style="direction:rtl">${_escHTML(orderTypeAr)}</span>`:""}</div>
<div style="text-align:center;font-size:${bodyFont-4}px">${_escHTML(order.time||new Date().toLocaleTimeString("en-SA"))}</div>
${SEP}
${rows}
${SEP}
<div style="text-align:center;font-size:${bodyFont-4}px">Kitchen Copy</div>
<br/><br/></body></html>`;
}
...
function POS({items,sales,setSales,tables,setTables,promos,license,lang="en",currentUser=null}){
      ...
      try{
        ...
        try{
          if(isQZConnected()&&_qzKitchenPrinter){
            const kotHTML=buildKOTHtml({kot:newKot2,type:kType,table:kTable,time:new Date().toLocaleTimeString("en-SA"),items:kotCart});
  ...
  async function printKOT(){
    ...
    if(isQZConnected()&&_qzKitchenPrinter){
      try{
        const kotHTML=buildKOTHtml({kot:newKot,type:orderType,table:selectedTable,time:new Date().toLocaleTimeString("en-SA"),items:cart});
...
function PresetFormatTab({license,company}){
  ...
  // defaults per mode
  const DEFAULTS={
    invoice:{usePreset:true,presetStyle:"s1",paperWidth:"80mm",headFont:14,bodyFont:12,totalFont:16,lineGap:3,logoSize:60,qrSize:120,headerColor:"#1A6B4A",logoUrl:"",footer:"Thank you — visit again",footerAr:"شكراً لك زيارة مرة أخرى"},
    draft:{usePreset:true,presetStyle:"s1",paperWidth:"80mm",headFont:14,bodyFont:12,totalFont:16,lineGap:3,logoSize:60,headerColor:"#1A6B4A",logoUrl:"",footer:"Thank you — visit again",footerAr:"شكراً لك زيارة مرة أخرى"},
    kot:{usePreset:true,presetStyle:"s1",paperWidth:"80mm",headFont:20,bodyFont:16,lineGap:4,logoSize:50,logoUrl:""},
  };
