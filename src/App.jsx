...
const APP_VERSION="v28.7.5";
const APP_VERSION_FULL="RestoPOS v28.7.5 · ZATCA Phase 2";
...
function _stack(ar,en,fs,gap){
  const a=ar?`<div style="direction:rtl;unicode-bidi:isolate;font-family:'Noto Naskh Arabic','Amiri','Tajawal',sans-serif !important;font-size:${fs}px;font-weight:700;word-break:break-word;line-height:1.6">${_escHTML(ar)}</div>`:"";
...
function _arSpan(text,extra){return `<span style="direction:rtl;unicode-bidi:isolate;font-family:'Noto Naskh Arabic','Amiri','Tajawal',sans-serif !important;font-weight:700${extra?";"+extra:""}">${_escHTML(text)}</span>`;}
function _arBlock(text,fs,extra){return `<div style="direction:rtl;unicode-bidi:isolate;font-family:'Noto Naskh Arabic','Amiri','Tajawal',sans-serif !important;font-weight:700;font-size:${fs}px;line-height:1.6${extra?";"+extra:""}">${_escHTML(text)}</div>`;}
...
function buildPresetHTML(order,license,zatcaInvoice,fmt,qrImgSrc,opts){
  ...
  const bodyFamily=style==="s1"?"'Courier New','Noto Naskh Arabic','Amiri',monospace":"'Amiri','Noto Naskh Arabic',serif";
...
function buildPresetKOT(order,fmt){
  ...
  const bodyFamily=style==="s1"?"'Courier New','Noto Naskh Arabic','Amiri',monospace":"'Amiri','Noto Naskh Arabic',sans-serif";
  const sepLine=style==="s4"?"2px solid #000":style==="s3"?"1px solid #000":"1px dashed #000";
...
function buildKOTHtml(order){
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><style>@page{size:80mm auto;margin:0}body{font-family:'Courier New',monospace;font-size:15px;width:80mm;padding:4mm}.big{font-size:22px;font-weight:900;text-align:center}.hr{border:none;border-top:1px dashed #000;margin:6px 0}.ar{direction:rtl;unicode-bidi:isolate;font-family:'Noto Naskh Arabic','Amiri','Tajawal',sans-serif;font-weight:700;line-height:1.6}</style></head><body><div class="big">KOT #${_escHTML(order.kot||"")}</div><div class="hr"></div><div>${_escHTML(String(order.type||"").toUpperCase())}${order.table?" - Table "+_escHTML(order.table):""}</div><div style="font-size:11px">${_escHTML(order.time||new Date().toLocaleTimeString("en-SA"))}</div><div class="hr"></div>${it2.map(it=>`<div style="font-weight:800;font-size:17px">${it.qty}x ${_escHTML(it.name)}</div>${it.nameAr?`<div class="ar">${_escHTML(it.nameAr)}</div>`:""}`).join("")}<div class="hr"></div><div style="text-align:center;font-size:11px">Kitchen Copy</div></body></html>`;
}
