...
function POS({items,sales,setSales,tables,setTables,promos,license,lang="en"}){
  ...
  async function confirmPayment(method,given,change,promo,totalDiscountAmt,finalTotal,finalVat,printAndSave,payInfo,manualDiscountAmt,promoDiscountAmt,isDraft=false,extraData={}){
    ...
    // Save to sales — direct LS save FIRST (synchronous), then React state
    try{
      // 1. Save directly to localStorage immediately (synchronous, reliable)
      const currentSales=LS.get("restopos_sales")||[];
      const updatedSales=[...currentSales,inv];
      LS.set("restopos_sales",updatedSales);
      // 2. Update React state (will trigger re-render)
      setSales(updatedSales);
      console.log("[Sale saved]",inv.id,"Total:",inv.total,"Method:",inv.payMethod);
    }catch(e){
      console.error("[Sales] Save failed:",e);
      alert("⚠️ Invoice "+inv.id+" may not have saved. Please check reports.");
    }
    setLastOrder(inv);
