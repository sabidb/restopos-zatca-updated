...
function PaymentModal({total,subtotal,vat,promos,onConfirm,onClose,license,vno=1,kotNo=1,customers=[],customerName:initCustName="",customerPhone:initCustPhone=""}){
  ...
  function handleConfirm(){
    ...
    // saveWithoutPrint is passed via state, printAndSave=false means save only
    const actualPrint=saveWithoutPrint?false:true;
    onConfirm(
      ...
      <div style={{background:"#fff",borderRadius:20,width:"100%",maxWidth:680,
        ...
        <div style={{padding:"18px 22px",display:"flex",flexDirection:"column",gap:16,flex:1,overflowY:"auto"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,position:"relative"}}>
              <div>
                ...
                <input value={custPhone} onChange={e=>handlePhoneChange(e.target.value)}
                  ...
                  style={{width:"100%",padding:"9px 12px",border:"1.5px solid #B0C8E8",borderRadius:9,
                    fontSize:14,fontFamily:"inherit",color:"#000",background:"#fff"}}/>
              ...
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                <input value={manualDiscount} onChange={e=>setManualDiscount(e.target.value)}
                  ...
                  style={{flex:1,padding:"9px 12px",border:"1.5px solid #E8D090",borderRadius:10,
                    fontSize:16,fontWeight:700,fontFamily:"inherit",color:"#000",background:"#fff",textAlign:"center"}}/>
            ...
            {method==="Cash"&&(
              <div style={{marginTop:12}}>
                ...
                <input value={cashGiven} onChange={e=>setCashGiven(e.target.value)} type="number"
                  ...
                  style={{width:"100%",padding:"12px 16px",border:`2px solid ${shortfall?"#D94040":"#B0C8E8"}`,
                    borderRadius:10,fontSize:24,fontWeight:800,fontFamily:"inherit",
                    color:"#000",background:"#fff",textAlign:"center"}}/>
                {/* Denomination pad */}
                <div style={{marginTop:8}}>
                  ...
                  <div>
                    ...
                    <input value={cashAmount} onChange={e=>{setCashAmount(e.target.value);setSplitError("");}}
                      ...
                      style={{width:"100%",padding:"10px 12px",border:"1.5px solid #C5DCF5",borderRadius:10,
                        fontSize:18,fontWeight:800,fontFamily:"inherit",color:"#000",background:"#fff",textAlign:"center"}}/>
                  </div>
                  <div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",
            ...
            border:`1.5px solid ${isDraft?"#F0A500":"#E0E8F0"}`,borderRadius:10,cursor:"pointer"}}
            onClick={()=>setIsDraft(x=>!x)}>
            <div style={{width:22,height:22,borderRadius:6,flexShrink:0,
              background:isDraft?"#F0A500":"#fff",border:`2px solid ${isDraft?"#F0A500":"#CBD5E0"}`,
              display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.15s"}}>
              {isDraft&&<span style={{color:"#fff",fontSize:14,fontWeight:900,lineHeight:1}}>✓</span>}
            </div>
            <div style={{flex:1}}>
              <div style={{fontSize:13,fontWeight:700,color:isDraft?"#A07000":"#1A3A5C"}}>
                📋 Draft Bill {isDraft?"(D-Invoice — no ZATCA QR)":""}
              </div>
              <div style={{fontSize:10,color:"#8A9AB0"}}>
                {isDraft?"Will print as D-Invoice (e.g. 11A, 11B...) without ZATCA QR code":"Tap to mark as draft bill"}
              </div>
            </div>
            <button onClick={e=>{e.stopPropagation();testPrinter();}}
              style={{padding:"5px 10px",background:"#fff",border:"1.5px solid #C5DCF5",
                borderRadius:7,color:"#1A3A5C",fontSize:10,fontWeight:700,
          ...
          {/* ── Action buttons ── */}
          {!canConfirm&&(
            <div style={{padding:"10px 14px",background:"#FDE8E8",border:"1px solid #D94040",borderRadius:10,fontSize:12,fontWeight:700,color:"#D94040",textAlign:"center"}}>
              {method==="Cash"&&cashFloat>0?"⚠️ Cash amount is less than total":method==="Both"?"⚠️ Fix split amounts":"⚠️ Enter payment details"}
            </div>
          )}
          <div style={{display:"flex",gap:10}}>
            {/* Save Bill — smaller */}
            <button onClick={()=>{setSaveWithoutPrint(true);setTimeout(()=>handleConfirm(),0);}}
              disabled={!canConfirm}
              style={{flex:1,padding:"13px 10px",
                background:canConfirm?"#2176AE":"#ccc",
                color:"#fff",border:"none",borderRadius:12,fontSize:13,fontWeight:700,
                cursor:canConfirm?"pointer":"not-allowed",fontFamily:"inherit"}}>
              💾 Save Bill
            </button>
            {/* Print & Save — larger */}
            <button onClick={()=>{setSaveWithoutPrint(false);setTimeout(()=>handleConfirm(),0);}}
              disabled={!canConfirm}
              style={{flex:2,padding:"13px",
                background:canConfirm?"linear-gradient(135deg,#1A6B4A,#0F4A30)":"#ccc",
...
function POS({items,sales,setSales,tables,setTables,promos,license,lang="en"}){
  ...
  async function confirmPayment(method,given,change,promo,totalDiscountAmt,finalTotal,finalVat,printAndSave,payInfo,manualDiscountAmt,promoDiscountAmt,isDraft=false,extraData={}){
    ...
    // ALWAYS save to sales first — critical
    try{setSales(prev=>[...prev,inv]);}catch(e){console.error("[Sales] Save failed:",e);}
    setLastOrder(inv);
    if(isDraft){
    ...
    if(!printAndSave){
      // Save only — show banner and done
      setPrintBanner({msg:"💾 Invoice Saved Successfully",type:"save"});
      setTimeout(()=>setPrintBanner(null),2000);
    }else if(isDraft){
      // Draft: print without ZATCA QR
      try{printDraftReceipt(inv,license);}catch(e){console.warn("[Draft Print]",e);}
      setPrintBanner({msg:"📋 Draft Bill Saved & Printed",type:"success"});
      setTimeout(()=>setPrintBanner(null),2500);
    }else{
      // Normal: try ESC/POS first, then silent iframe
      let printed=false;
      try{
      ...
      }catch(e){console.warn("[ESC/POS]",e);}
      if(!printed){
        try{
