      ...
      if(!printed){
        try{
          ...
          await new Promise(res=>setTimeout(()=>{
            try{frame.contentWindow.focus();frame.contentWindow.print();printed=true;printMethod="Browser";}
            catch(e){console.warn("[iframe]",e);}
            setTimeout(()=>{try{document.body.removeChild(frame);}catch(e){}},4000);
            res();
          },400));
        }catch(e){console.warn("[iframe]",e.message);}
      }

      if(printed){
        setPrintBanner({msg:`✅ ${inv.id} — Saved & Printed (${printMethod})`,type:"success"});
      }else{
        setPrintBanner({msg:`⚠️ ${inv.id} saved — Print failed. Is QZ Tray running?`,type:"error"});
      }
      setTimeout(()=>setPrintBanner(null),4000);
    }
  }
...
function Create({items,setItems,promos,setPromos}){
  ...
  return(<div>
    ...
    {tab==="categories"&&(()=>{
      ...
      const [selectedCatView,setSelectedCatView]=React.useState(categories[0]||"");
      ...
      function deleteCategory(cat){
        ...
        if(catItems.length>0){
          ...
          if(!confirm(`"${cat}" has ${catItems.length} item(s). Move them to "${other||"Uncategorized"}" and delete?`))return;
          if(other)setItems(prev=>prev.map(i=>i.category===cat?{...i,category:other}:i));
        }else{
          if(!confirm(`Delete category "${cat}"?`))return;
        }
        saveCategories(categories.filter(c=>c!==cat));
        if(selectedCatView===cat)setSelectedCatView(categories.filter(c=>c!==cat)[0]||"");
      }
      function moveItemToCategory(item,newCat){
      ...
      return(
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"200px 1fr",gap:14}}>
            {/* Category list */}
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              <div style={{fontSize:12,fontWeight:700,color:C.textMid,marginBottom:4}}>CATEGORIES ({categories.length})</div>
              {categories.map(cat=>(
                <div key={cat}
                  ...
                  {editingCat===cat?(
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:6}} onClick={()=>setSelectedCatView(cat)}>
                      <span style={{flex:1,fontSize:13,fontWeight:selectedCatView===cat?700:500,color:selectedCatView===cat?C.primary:C.text}}>{cat}</span>
                      <span style={{fontSize:10,color:C.textLight}}>{items.filter(i=>i.category===cat).length}</span>
                      <button onClick={e=>{e.stopPropagation();startEdit(cat);}} style={{background:"none",border:"none",color:C.info,cursor:"pointer",fontSize:13,padding:"0 2px"}} title="Edit">✏️</button>
                      <button onClick={e=>{e.stopPropagation();deleteCategory(cat);}} style={{background:"none",border:"none",color:C.danger,cursor:"pointer",fontSize:13,padding:"0 2px"}} title="Delete">🗑️</button>
                    </div>
                  )}
                </div>
              ))}
            ...
            <Card>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                <div style={{fontSize:13,fontWeight:700}}>📋 Items in "{selectedCatView}" ({catItems.length})</div>
                <div style={{fontSize:11,color:C.textMid}}>Tap item to move to another category</div>
              </div>
              {catItems.length===0?(
                <div style={{textAlign:"center",padding:"30px 0",color:C.textLight}}>No items in this category</div>
              ):(
                <div style={{display:"flex",flexDirection:"column",gap:6}}>
                  {catItems.map(it=>(
                    <div key={it.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",
                      background:movingItem?.id===it.id?C.primaryLight:"#F8FAFF",
                      borderRadius:8,border:`1px solid ${movingItem?.id===it.id?C.primary:C.border}`}}>
                      <div style={{flex:1}}>
                      ...
                      <div style={{display:"flex",flexDirection:"column",gap:4,alignItems:"flex-end"}}>
                        <div style={{fontSize:9,color:C.textLight,fontWeight:700}}>MOVE TO</div>
                        <select onChange={e=>{if(e.target.value)moveItemToCategory(it,e.target.value);e.target.value="";}}
                          value=""
                          style={{padding:"4px 8px",border:`1px solid ${C.border}`,borderRadius:6,fontSize:11,fontFamily:"inherit",background:"#fff",cursor:"pointer"}}>
          ...
          <div style={{fontSize:12,color:C.success,fontWeight:600}}>✓ {categories.length} categories · {items.length} items total · Changes sync automatically</div>
        </div>
      );
    })()}
