...
function buildPresetHTML(order,license,zatcaInvoice,fmt,qrImgSrc,opts){
  ...
  const footerHTML=`<div style="text-align:center;margin-top:6px">
    ...
    ${isDraft?`<div style="font-size:9px;color:#000;margin-top:4px">DRAFT — Not a tax invoice</div>`:""}
  </div>`;
  return `<!DOCTYPE html><html><head><meta charset="UTF-8">
...
function buildPresetKOT(order,fmt){
  ...
  const title=`<div style="text-align:center;font-weight:900;font-size:${headFont}px">${style==="s2"?`<span class="kbadge" style="background:#000;color:#fff;padding:3px 12px;border-radius:4px">KOT #${_escHTML(order.kot||"")}</span>`:`KOT #${_escHTML(order.kot||"")}`}</div>`;
