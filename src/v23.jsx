...
// PAYMENT MODAL
// ═══════════════════════════════════════════════════════════════════
function PaymentModal({total,subtotal,vat,promos,onConfirm,onClose,license}){
  // ── State ────────────────────────────────────────────────────────
  ...
  const [discountType,setDiscountType]=useState("%"); // "%" or "SAR"
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:3000,
      ...
      <div style={{background:"#fff",borderRadius:20,width:"100%",maxWidth:540,
        ...
        {/* Header */}
        <div style={{background:"linear-gradient(135deg,#1A3A5C,#0F2340)",padding:"18px 22px",
          borderRadius:"20px 20px 0 0",display:"flex",justifyContent:"space-between",alignItems:"center",
          flexShrink:0}}>
          <div>
            <div style={{color:"#fff",fontSize:17,fontWeight:800}}>💳 Charge & Payment</div>
            <div style={{color:"rgba(255,255,255,0.55)",fontSize:11,marginTop:2}}>Apply discounts and complete transaction</div>
          </div>
        ...
        <div style={{padding:"18px 22px",display:"flex",flexDirection:"column",gap:16,flex:1,overflowY:"auto"}}>
                ...
                <div style={{display:"flex",gap:6,marginTop:8,flexWrap:"wrap"}}>
                  {QUICK.map(q=>(
                    <button key={q} onClick={()=>setCashGiven(String(q))}
                      style={{flex:1,minWidth:50,padding:"7px 4px",
                        background:parseFloat(cashGiven)===q?"#1A3A5C":"#F0F7FF",
                        color:parseFloat(cashGiven)===q?"#fff":"#1A3A5C",
                  ...
                  <button onClick={()=>setCashGiven(finalTotal.toFixed(2))}
                    style={{flex:1,minWidth:55,padding:"7px 4px",background:"#E8F5EE",
                      color:"#1A6B4A",border:"1.5px solid #A8D5B8",borderRadius:8,
          ...
          {/* ── Bottom options ── */}
          <div style={{background:"#F8FAFF",border:"1.5px solid #E0E8F0",borderRadius:12,
            padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",
            flexWrap:"wrap",gap:10}}>
            {/* Print & Save toggle */}
            <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer"}}>
              <div onClick={()=>setPrintAndSave(x=>!x)}
                style={{width:44,height:24,borderRadius:12,
                  background:printAndSave?"#1A6B4A":"#CBD5E0",
                  position:"relative",transition:"background 0.2s",cursor:"pointer",flexShrink:0}}>
                <div style={{position:"absolute",top:2,left:printAndSave?22:2,width:20,height:20,
                  borderRadius:"50%",background:"#fff",transition:"left 0.2s",
                  boxShadow:"0 1px 4px rgba(0,0,0,0.2)"}}/>
              </div>
              <div>
                <div style={{fontSize:13,fontWeight:700,color:"#1A3A5C"}}>
                  {printAndSave?"🖨️ Print & Save":"💾 Save Only"}
                </div>
                <div style={{fontSize:10,color:"#8A9AB0"}}>
                  {printAndSave?"Receipt will print after payment":"No receipt print"}
                </div>
              </div>
            </label>
            {/* Test printer */}
            <button onClick={testPrinter}
              style={{padding:"8px 14px",background:"#fff",border:"1.5px solid #C5DCF5",
                borderRadius:10,color:"#1A3A5C",fontSize:12,fontWeight:700,
          ...
          <button onClick={handleConfirm} disabled={!canConfirm}
            ...
            {!canConfirm&&method==="Cash"&&cashFloat>0?"⚠️ Insufficient Amount":
             ...
             `✓ Confirm ${method==="Both"?"Split":method} Payment${printAndSave?" & Print":""}`}
          </button>
        </div>
      </div>
...
function POS({items,sales,setSales,tables,setTables,promos,license,lang="en"}){
  ...
  async function confirmPayment(method,given,change,promo,totalDiscountAmt,finalTotal,finalVat,printAndSave,payInfo,manualDiscountAmt,promoDiscountAmt){
    ...
    const inv={
      ...
      vat:finalVat,
      total:finalTotal,
      status:"completed",cashier:"Admin",
      payMethod:method,
    ...
    if(printAndSave){
      // Try direct ESC/POS auto-print first
      try{
        if("serial" in navigator&&isPortOpen(_billPort)){
          await printReceiptEscPos(inv,license);
          // Still show receipt modal for ZATCA QR/download
        }
  ...
  return(
    <div style={{display:"flex",height:"calc(100vh - 52px)",overflow:"hidden"}}>
      {showPayment&&<PaymentModal total={total} subtotal={subtotal} vat={vat} promos={promos} license={license} onConfirm={confirmPayment} onClose={()=>setShowPayment(false)}/>}
