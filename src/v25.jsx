...
function buildReceiptHTML(order,license,zatcaInvoice,fmt,qrImgSrc){
  ...
  // Meta row
  let meta=`<div style="display:flex;justify-content:space-between;font-size:${fontSize-1}px;gap:6px"><span style="word-break:break-word">${_escHTML(order.id||"")}</span><span style="white-space:nowrap">${_escHTML(order.date||"")} ${_escHTML(order.time||"")}</span></div>`;
  ...
  // Items — grouped by category when enabled. Long names wrap to next line; price never overflows.
  function lineHTML(it){
    const ar=(showArabicName&&it.nameAr)?`<div style="direction:rtl;font-family:'Noto Naskh Arabic','Tajawal',sans-serif;font-size:${fontSize-2}px;color:#555;word-break:break-word">${_escHTML(it.nameAr)}</div>`:"";
    return `<div style="display:flex;justify-content:space-between;align-items:flex-start;margin:2px 0;gap:6px${boldItems?";font-weight:700":""}"><span style="flex:1 1 auto;min-width:0;padding-right:4px;word-break:break-word;overflow-wrap:anywhere">${_escHTML(it.name)}<span style="color:#777"> x${it.qty}</span>${ar}</span><span style="flex:0 0 auto;white-space:nowrap;text-align:right">SAR ${(it.qty*it.price).toFixed(2)}</span></div>`;
  }
  let itemsHTML="";
  if(showCategories){
...
${imgPos==="top"?imgHTML:""}
<div class="hr"/>
<div class="row"><span style="word-break:break-word">${_escHTML(order.id||"")}</span><span>${_escHTML(order.date||"")} ${_escHTML(order.time||"")}</span></div>
...
<div class="hr"/>
${items.map(it=>`<div class="item"><span class="nm">${it.qty}x ${_escHTML(it.name)}</span><span class="pr">SAR ${(it.qty*it.price).toFixed(2)}</span></div>${it.nameAr?`<div style="direction:rtl;font-family:'Noto Naskh Arabic','Tajawal',sans-serif;font-size:${fontSize-1}px;color:#555;word-break:break-word">${_escHTML(it.nameAr)}</div>`:""}`).join("")}
<div class="hr"/>
...
function printDraftReceipt(order,license){
  ...
  const html=buildDraftReceiptHTML(order,license,draftFmt).replace("</body></html>","<script>window.onload=function(){window.print();window.close();}<\/script></body></html>");
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <Card>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                ...
                <Inp label="Header Logo URL (optional)" value={draftFmt.logoUrl||""} onChange={v=>updateDraft("logoUrl",v)} placeholder="https://example.com/logo.png"/>
            ...
            <Card>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                <Inp label="Image URL" value={draftFmt.imageUrl||""} onChange={v=>updateDraft("imageUrl",v)} placeholder="https://example.com/qr-or-logo.png"/>
                {draftFmt.imageUrl&&<>
