...
const APP_VERSION_FULL="RestoPOS v27 · ZATCA Phase 2";
...
function ZATCASetup({license,sales=[]}){
  ...
  (sales||[]).forEach(s=>{
    ...
    revenueByYear[yr] = (revenueByYear[yr]||0) + taxable;
  });
  ...
  async function handleActivate(){
    ...
    try{
      ...
      LS.set(ZATCA_SETUP_STATUS_KEY,{activated:true,branchName,vatNumber,companyName,activatedAt:new Date().toISOString(),complianceRequestId:data.complianceRequestId});
      setStatus("activated");
      setMsg("✅ ZATCA Phase 2 activated. Invoices can now be signed and reported to FATOORA.");
      setOtp("");
    }catch(e){
    ...
    <div style={{maxWidth:640}}>
      ...
      <Card style={{marginBottom:16}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
          ...
          <div><div style={{fontSize:18,fontWeight:800}}>Phase 2 Eligibility Check</div><div style={{fontSize:12,color:C.textMid}}>Based on your RestoPOS taxable revenue</div></div>
        </div>
          ...
          <>
            <div style={{display:"flex",flexWrap:"wrap",gap:10,marginBottom:14}}>
              {qualifyingYears.map(y=>(
                <div key={y} style={{flex:"1 1 110px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:10,padding:"10px 12px"}}>
                  <div style={{fontSize:11,color:C.textLight,fontWeight:600}}>{y}</div>
        ...
        <div style={{fontSize:11,color:C.textLight,marginTop:12,lineHeight:1.5}}>
          ⚖️ This estimate counts only sales recorded in RestoPOS, for calendar years ZATCA uses (2022–2024). Your official obligation is confirmed by ZATCA's notification email to your registered contact. If you take payments outside RestoPOS, your true total may be higher.
        </div>
      </Card>
      ...
      <Card>
        ...
        {status==="activated"?(
          <div style={{background:C.successLight,border:`1px solid ${C.success}44`,borderRadius:10,padding:16,marginTop:14}}>
            <div style={{fontWeight:800,color:C.success,fontSize:15,marginBottom:4}}>✓ Phase 2 Active</div>
            <div style={{fontSize:13,color:C.textMid}}>This branch ({saved.branchName||branchName}) is onboarded with ZATCA. Invoices are signed automatically when reported from the Invoices screen.</div>
            ...
            <button onClick={()=>{setStatus("idle");setMsg("");}} style={{marginTop:12,background:"transparent",border:`1.5px solid ${C.border}`,borderRadius:8,padding:"8px 16px",color:C.textMid,fontFamily:"inherit",fontSize:12,fontWeight:600,cursor:"pointer"}}>Re-activate / Add another branch</button>
          </div>
        ):(
          <>
            ...
            <button onClick={handleActivate} disabled={busy} style={{marginTop:18,width:"100%",background:busy?C.textLight:C.zatca,color:"#fff",border:"none",borderRadius:10,padding:"13px",fontFamily:"inherit",fontSize:14,fontWeight:700,cursor:busy?"not-allowed":"pointer"}}>{busy?"Activating…":"🔐 Activate ZATCA Phase 2"}</button>
          </>
        )}
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:20,flexWrap:"wrap"}}>
        {[["balancesheet","📋 Balance Sheet"],["cashflow","💧 Cash Flow"],["trial","⚖️ Trial Balance"],["gl","📒 General Ledger"],["vat","🧾 VAT Liability"],["pnl","📈 P&L"],["expenses","💸 Expenses"],["zatcasetup","🔐 ZATCA Setup"]].map(([id,lbl])=>(
