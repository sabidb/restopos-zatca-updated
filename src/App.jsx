...
const APP_VERSION_FULL="RestoPOS v29.2.5 · ZATCA Phase 2";
...
function Reports({sales,allSales,items,setSales}){
  ...
  function buildSummaryData(salesArr,dateStr){
    ...
    const cardSale=salesArr.filter(o=>["Mada","Card","Apple Pay","Both"].includes(o.payMethod)).reduce((s,o)=>s+(o.total||0),0);
...
function AdvancedFeatures({sales,items,setItems,license,company,invoiceFormat,setInvoiceFormat,users,setUsers}){
  ...
  const tabs=[["qztray","🖨️ QZ Tray"],["silentprint","🔇 Silent Printing"],["kitchen","🍽️ Kitchen Printer"],["users","👤 Users"],["kds","🍳 KDS"],["stocktakes","📦 Stock Takes"],["recipes","📋 Recipes"],["giftcards","🎁 Gift Cards"],["delivery","🛵 Delivery"],["locations","🏢 Locations"],["accounting","📤 Accounting"],["reports","📅 Reports"],["printer","🖨️ ESC/POS"],["errorlog","⚠️ Error Log"],["analytics","📉 Analytics"],["audit","🔍 Audit Trail"],["tools","🔧 Tools"]];
  return(
    <div>
      <div style={{display:"flex",gap:6,marginBottom:20,flexWrap:"wrap"}}>
        {tabs.map(([id,lbl])=>(
