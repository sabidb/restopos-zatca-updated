...
const SYNC_KEYS=[
  ...
  "restopos_draft_format","restopos_dashboard_config","restopos_draft_invoices","restopos_saved_invoices",
];
      ...
      try{
        ...
        try{
          if(isQZConnected()&&_qzKitchenPrinter){
            const kotHTML=`<!DOCTYPE html><html><head><meta charset="UTF-8"><style>@page{size:80mm auto;margin:0}body{font-family:'Courier New',monospace;font-size:15px;width:80mm;padding:4mm}.big{font-size:22px;font-weight:900;text-align:center}.hr{border:none;border-top:1px dashed #000;margin:6px 0}.ar{direction:rtl;font-family:'Tajawal','Noto Naskh Arabic',sans-serif}</style></head><body><div class="big">KOT #${newKot2}</div><div class="hr"></div><div>${String(kType).toUpperCase()}${kTable?" - Table "+kTable:""}</div><div style="font-size:11px">${new Date().toLocaleTimeString("en-SA")}</div><div class="hr"></div>${kotCart.map(it=>`<div style="font-weight:800;font-size:17px">${it.qty}x ${it.name}</div>${it.nameAr?`<div class="ar">${it.nameAr}</div>`:""}`).join("")}<div class="hr"></div><div style="text-align:center;font-size:11px">Kitchen Copy</div></body></html>`;
    ...
    if(!printAndSave){
      ...
      try{
        ...
        if(isQZConnected()&&_qzBillPrinter){
          await printWithQZ(draftHTML,_qzBillPrinter,_draftFmt.paperWidth||"80mm");
      ...
      // ── Try 1: ESC/POS USB (raw text, printer Font A — NEVER corrupts) ──
...
function Settings({company,setCompany,tables,setTables,license,onClearLicense,onSwitchAccount,pins,setPins,invoiceFormat,setInvoiceFormat,lang="en",onLangChange,sales=[],items=[]}){
  ...
  const tabs=[["company","🏢 Company"],["dashboard","📊 Dashboard"],["tables","🪑 Tables"],["printers","🖨️ Bill Printer"],["invoices","📄 Invoices"],["backup","💾 Backup"],["security","🔐 Security"],["license","📋 License"],["language","🌐 Language"]];
...
function Help({license: helpLicense}){
  ...
  useEffect(()=>{
    ...
    // Load notifications from pending_activations doc + activity log
    ...
    getDocs(q).then(snap=>{
      ...
      if(!snap.empty){
        ...
        if(data.notification?.text){
          notifs.push({type:"message",icon:"📨",title:"Message from Admin",body:data.notification.text,time:data.notification.sentAt,read:data.notification.read});
        }
        // Plan changes
        if(data.planUpdatedAt){
          notifs.push({type:"plan",icon:"💳",title:"Plan Updated",body:`Your subscription plan was updated to ${SUBSCRIPTION_PLANS?.[data.subscriptionPlan]?.name||data.subscriptionPlan||"Basic"}.`,time:data.planUpdatedAt,read:true});
        }
        // Expiry set
        if(data.expiryUpdatedAt){
          notifs.push({type:"expiry",icon:"📅",title:"Subscription Expiry Set",body:`Your subscription expiry was set to ${data.customExpiryDate||"—"} by admin.`,time:data.expiryUpdatedAt,read:true});
        }
        // Password reset
        if(data.passwordResetAt&&data.passwordResetByAdmin){
          notifs.push({type:"security",icon:"🔑",title:"Password Reset by Admin",body:"Your password was reset by the admin. Please set a new password on your next login.",time:data.passwordResetAt,read:true});
        }
        // Force logout events
        if(data.forceLogoutAt){
          notifs.push({type:"logout",icon:"⚡",title:"Session Ended by Admin",body:"Your session was force-ended by the admin.",time:data.forceLogoutAt,read:true});
        }
        // Approved
        if(data.approvedAt){
          notifs.push({type:"approved",icon:"✅",title:"Account Approved",body:"Your RestoPOS account was approved and activated.",time:data.approvedAt,read:true});
        }
        // Renewed
        if(data.renewedAt){
          notifs.push({type:"renewal",icon:"🔄",title:"Subscription Renewed",body:"Your subscription was renewed by admin.",time:data.renewedAt,read:true});
        }
      }
      // Sort newest first
  ...
  // Subscribe to live chat in real-time
  useEffect(()=>{
    ...
    // Mark room as "has unread from client side" by ensuring room doc exists
    setDoc(doc(db,"live_chats",chatRoomId),{
      licenseKey:lic.licenseKey,businessName:lic.businessName||"",
      lastClientActivity:new Date().toISOString(),status:"active"
    },{merge:true}).catch(()=>{});
    return()=>unsub();
  },[tab,chatRoomId]);
  ...
  async function sendChatMessage(){
    ...
    try{
      let photoUrl=null;
      if(chatPhoto){
        const photoRef=ref(storage,`chat/${chatRoomId}/${Date.now()}_${chatPhoto.name}`);
        await uploadBytes(photoRef,chatPhoto);
        photoUrl=await getDownloadURL(photoRef);
      }
      await addDoc(collection(db,"live_chats",chatRoomId,"messages"),{
        text:chatInput.trim()||"",
        photoUrl,
        sender:"client",
        senderName:lic.businessName||"Client",
        sentAt:new Date().toISOString(),
        read:false,
      });
      await setDoc(doc(db,"live_chats",chatRoomId),{
        licenseKey:lic.licenseKey,businessName:lic.businessName||"",
        lastMessage:chatInput.trim()||"📷 Photo",
        lastActivity:new Date().toISOString(),
        hasUnread:true,status:"active",
      },{merge:true});
  ...
  const sections=[["guide","🚀","Guide"],["zatca","⬛","ZATCA"],["ai","🤖","AI Help"],["notifications","🔔",unreadCount>0?`Updates (${unreadCount})`:"Updates"],["livechat","💬","Live Chat"],["update","🔄","Update"],["upgrade","⬆️","Upgrade"],["live","🆘","Live Help"],["support","📞","Support"]];
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          <Card>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
              ...
              <div>
                <div style={{fontSize:17,fontWeight:800}}>Admin Updates</div>
                <div style={{fontSize:12,color:C.textMid}}>All changes made to your account by RestoPOS admin</div>
              </div>
            </div>
            {notifLoading?(
              <div style={{textAlign:"center",padding:"30px 0",color:C.textLight}}>⏳ Loading updates…</div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {notifications.map((n,i)=>(
                  <div key={i} style={{display:"flex",gap:12,padding:"12px 14px",borderRadius:10,
                    background:n.read?"rgba(0,0,0,0.02)":"linear-gradient(135deg,#EFF6FF,#F0FDF4)",
                    border:`1px solid ${n.read?C.border:"#93c5fd"}`,position:"relative"}}>
      ...
      {tab==="livechat"&&(
        <Card style={{padding:0,overflow:"hidden",display:"flex",flexDirection:"column",height:"70vh"}}>
          ...
          <div style={{padding:"14px 16px",background:"linear-gradient(135deg,#1A3D2B,#1F4D36)",display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
            ...
            <div style={{marginLeft:"auto",width:8,height:8,borderRadius:"50%",background:"#2ECC71",boxShadow:"0 0 6px #2ECC71"}}/>
          </div>
          ...
          <div ref={liveChatRef} style={{flex:1,overflowY:"auto",padding:14,display:"flex",flexDirection:"column",gap:10,background:"#F8F9FB"}}>
            {chatMessages.length===0&&(
              <div style={{textAlign:"center",padding:"40px 20px",color:C.textLight}}>
                ...
                <div style={{fontSize:12,marginTop:4}}>Ask anything — billing questions, ZATCA issues, errors, or feature requests. You can also send photos of error screens.</div>
              </div>
            )}
                  ...
                  <div style={{maxWidth:"78%",padding:"10px 13px",borderRadius:isClient?"16px 16px 4px 16px":"16px 16px 16px 4px",
                    ...
                    {msg.photoUrl&&(
                      <div style={{marginTop:msg.text?8:0}}>
                        <img src={msg.photoUrl} alt="attachment" style={{maxWidth:"100%",maxHeight:200,borderRadius:8,cursor:"pointer",border:`1px solid ${isClient?"rgba(255,255,255,0.2)":C.border}`}}
                          onClick={()=>window.open(msg.photoUrl,"_blank")}/>
                      </div>
                    )}
                  </div>
                  <div style={{fontSize:9,color:C.textLight,padding:"0 4px"}}>
                    {msg.sender==="admin"?"RestoPOS Support":lic.businessName||"You"} · {msg.sentAt?.slice(11,16)||""}
                  </div>
                </div>
              );
            })}
          ...
          {chatPhotoPreview&&(
            <div style={{padding:"8px 14px",background:"#F0F9F4",borderTop:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:8}}>
              <img src={chatPhotoPreview} alt="preview" style={{width:50,height:50,objectFit:"cover",borderRadius:8,border:`1px solid ${C.border}`}}/>
              <div style={{flex:1,fontSize:12,color:C.textMid}}>📎 {chatPhoto?.name}</div>
              <button onClick={()=>{setChatPhoto(null);setChatPhotoPreview(null);}}
          ...
          <div style={{padding:"10px 12px",borderTop:`1px solid ${C.border}`,background:"#fff",display:"flex",gap:8,alignItems:"flex-end",flexShrink:0}}>
            <input ref={photoInputRef} type="file" accept="image/*" style={{display:"none"}} onChange={e=>{
              ...
              setChatPhoto(file);
              const reader=new FileReader();
              reader.onload=ev=>setChatPhotoPreview(ev.target.result);
              reader.readAsDataURL(file);
              e.target.value="";
            }}/>
            <button onClick={()=>photoInputRef.current?.click()}
              style={{padding:"9px 10px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:8,cursor:"pointer",fontSize:16,flexShrink:0,color:C.textMid}}>
              📷
            </button>
...
function BackupManager({sales,items}){
  ...
  function downloadFullBackup(){
    const backup={
      ...
      sales,items,
      settings:{company:LS.get("restopos_company"),invoiceFormat:LS.get("restopos_invoice_format"),draftFormat:LS.get("restopos_draft_format"),dashboardConfig:LS.get("restopos_dashboard_config"),tables:LS.get("restopos_tables"),promos:LS.get("restopos_promos")},
...
function AdvancedFeatures({sales,items,setItems,license,company,invoiceFormat,setInvoiceFormat,users,setUsers}){
  ...
  const tabs=[["qztray","🖨️ QZ Tray"],["silentprint","🔇 Silent Printing"],["invoice","🧾 Invoice Format"],["kitchen","🍽️ Kitchen Printer"],["users","👤 Users"],["kds","🍳 KDS"],["stocktakes","📦 Stock Takes"],["recipes","📋 Recipes"],["giftcards","🎁 Gift Cards"],["delivery","🛵 Delivery"],["locations","🏢 Locations"],["accounting","📤 Accounting"],["reports","📅 Reports"],["printer","🖨️ ESC/POS"],["errorlog","⚠️ Error Log"]];
  return(
    <div>
      <div style={{display:"flex",gap:6,marginBottom:20,flexWrap:"wrap"}}>
        {tabs.map(([id,lbl])=>(
...
export default function App(){
  ...
  const [announcementBanner,setAnnouncementBanner]=useState(()=>LS.get("restopos_announcement")||"");
  ...
  // LIVE ANNOUNCEMENT LISTENER — picks up admin broadcasts in real-time
  useEffect(()=>{
    const unsub=onSnapshot(doc(db,"config","announcement"),snap=>{
      if(snap.exists()){
        const txt=snap.data().text||"";
        LS.set("restopos_announcement",txt);
  ...
  return(
    ...
    <div dir={dir(lang)} style={{fontFamily:lang==="ar"?"'Tajawal','Plus Jakarta Sans',sans-serif":"'Plus Jakarta Sans','Tajawal',sans-serif",background:C.bg,height:"100vh",display:"flex",flexDirection:"column",overflow:"hidden",zoom:`${uiScale}%`}}>
      ...
      <div style={{flex:1,padding:screen==="pos"?0:20,overflowY:screen==="pos"?"hidden":"auto",width:"100%",minHeight:0,height:"calc(100vh - 50px)"}}>
        {/* 📢 Live Announcement Banner — hidden on POS */}
        ...
        {/* 🔔 Admin Notification Popup */}
        {adminNotification&&(
          <div style={{position:"fixed",bottom:24,right:24,zIndex:9999,maxWidth:340,background:"#1A3D2B",border:"1px solid rgba(46,204,113,0.4)",borderRadius:14,padding:"14px 16px",boxShadow:"0 8px 32px rgba(0,0,0,0.35)",animation:"slideIn 0.3s ease"}}>
        ...
        {screen==="help"&&<Help license={license}/>}
        </TabBoundary>
      </div>
    </div>
    </ErrorBoundary>
...
function ZATCAVatEngine() {
  ...
  const days    = vatDaysLeft(period);
  const txToday = allTx.filter(t => t.date === vatTodayStr()).length;

  // ── Print / Save the official VAT return as a clean PDF ───
...
<title>VAT Return ${vatPeriodLabel(period)}</title>
<style>
...
<div class="hd">
  ...
  <div class="meta">
    <div><strong>Period:</strong> ${vatPeriodLabel(period)}</div>
    <div><strong>Filing deadline:</strong> ${vatFilingDeadline(period)}</div>
...
<div class="foot">RestoPOS · VAT Engine · restopos.store · Generated ${new Date().toLocaleString("en-SA")}</div>
    ...
    // Use a hidden iframe (no popup, works silently in browser/QZ environments)
    ...
    setTimeout(() => {
      try { frame.contentWindow.focus(); frame.contentWindow.print(); }
        ...
        else showToast("Allow pop-ups to print the VAT return.", "error");
      }
    }, 400);
  };
    ...
    <div style={S.root}>
      ...
      <div style={S.header}>
        ...
        <div style={S.headerRight}>
          ...
          <button style={{ ...S.addBtn, background:"#1A6B4A" }} onClick={printVATReturn}>🖨️ Print VAT Return</button>
