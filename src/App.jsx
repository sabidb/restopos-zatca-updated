...
function POS({items,sales,setSales,tables,setTables,promos,license,lang="en"}){
  ...
  async function confirmPayment(method,given,change,promo,totalDiscountAmt,finalTotal,finalVat,printAndSave,payInfo,manualDiscountAmt,promoDiscountAmt,isDraft=false,extraData={}){
    ...
    // ── Close payment window and clear cart IMMEDIATELY ──
    setCart([]);
    ...
    if(!printAndSave){
      ...
      try{
        ...
        setPrintBanner({msg:"⚠️ Draft saved but print failed: "+e.message,type:"error"});
      }
      setTimeout(()=>setPrintBanner(null),4000);
    }else{
      // Normal invoice: try ESC/POS → silent iframe → show error
      let printed=false;
      let printErr="";
      // 1. Try ESC/POS thermal printer
      try{
        if("serial" in navigator&&isPortOpen(_billPort)){
          await printReceiptEscPos(inv,license);
          printed=true;
        }
      }catch(e){printErr=e.message;console.warn("[ESC/POS]",e);}
      // 2. Try silent iframe browser print
      if(!printed){
        try{
          ...
          await new Promise(res=>{
            setTimeout(()=>{
              try{frame.contentWindow.focus();frame.contentWindow.print();printed=true;}
              catch(e2){printErr=e2.message;}
              setTimeout(()=>{try{document.body.removeChild(frame);}catch(e){}},3000);
              res();
            },350);
          });
        }catch(e){printErr=e.message;console.warn("[iframe print]",e);}
      }
      // 3. Show result banner
      if(printed){
        setPrintBanner({msg:"✅ "+inv.id+" — Saved & Printed",type:"success"});
      }else{
        setPrintBanner({msg:"⚠️ Invoice "+inv.id+" saved — Print failed. Check printer.",type:"error"});
      }
      setTimeout(()=>setPrintBanner(null),4000);
    }
  }
