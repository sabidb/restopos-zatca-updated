...
const APP_VERSION_FULL="RestoPOS v29.3.0 · ZATCA Phase 2";
...
function getItemOrder(){const o=LS.get("restopos_item_order");return o&&typeof o==="object"?o:{};}
function saveItemOrder(map){LS.set("restopos_item_order",map);const lic=LS.get("restopos_license_v2")?.licenseKey;if(lic)debouncedSync(lic,"restopos_item_order",map);}
...
function PaymentModal({total,subtotal,vat,promos,onConfirm,onClose,license,vno=1,kotNo=1,customers=[],customerName:initCustName="",customerPhone:initCustPhone="",orderType:initOrderType="takeaway"}){
  ...
  function testPrinter(){
    const html=`<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
      ...
      <div style="font-family:'Tahoma','Arial','Segoe UI',sans-serif;direction:rtl;margin-top:4px">الطابعة تعمل بشكل صحيح</div>
      <hr class="hr"/>
  ...
  function handleConfirm(shouldPrint=true){
    ...
    const payInfo={
      ...
      cardAmount:method==="Both"?bothCard:method==="Card"?finalTotal:0,
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",flex:1,overflow:"hidden",minHeight:0}}>
          ...
          <div style={{padding:"14px 16px",borderRight:"1px solid #E0E8F0",display:"flex",flexDirection:"column",gap:10,overflowY:"auto"}}>
            ...
            <div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:5,direction:"ltr"}}>
                {METHODS.map(m=>(
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,direction:"ltr"}}>
              ...
              <div style={{background:"#F8FAFF",border:"1px solid #E0E8F0",borderRadius:12,padding:"12px 12px"}}>
                ...
                <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                  {[["normal","📄","Normal"],["telephone","📞","Phone"],["kot-only","🍽️","KOT Only"]].map(([id,icon,label])=>(
                    <button key={id} onClick={()=>setLocalBillType(id)}
                      style={{flex:1,minWidth:60,padding:"10px 4px",border:`2px solid ${localBillType===id?(id==="kot-only"?"#7C3AED":"#1A6B4A"):"#E0E8F0"}`,
                        background:localBillType===id?(id==="kot-only"?"#EDE9FE":"#E8F5EE"):"#fff",
                        color:localBillType===id?(id==="kot-only"?"#7C3AED":"#1A6B4A"):"#8A9AB0",
                        borderRadius:9,fontFamily:"inherit",fontSize:11,fontWeight:700,cursor:"pointer"}}>
                      {icon} {label}
                    </button>
                  ))}
                </div>
                {localBillType==="kot-only"&&<div style={{marginTop:6,fontSize:9,color:"#7C3AED",fontWeight:600,lineHeight:1.4}}>🍽️ KOT printed only — no customer bill. Saved as D-Invoice, number not incremented.</div>}
              </div>
            </div>
          ...
          <div style={{padding:"14px 16px",display:"flex",flexDirection:"column",gap:10,overflowY:"auto"}}>
            ...
            {method==="Card"&&(
              <div style={{background:"#F0F7FF",borderRadius:10,padding:"12px 14px",textAlign:"center"}}>
                ...
                <div style={{fontSize:14,fontWeight:700,color:"#1A3A5C"}}>Card Payment</div>
...
function buildPresetHTML(order,license,zatcaInvoice,fmt,qrImgSrc,opts){
  ...
  const rows=items.map(it=>{
    const ar=it.nameAr?_arBlock(it.nameAr,bodyFont,"word-break:break-word"):"";
...
function buildPresetKOT(order,fmt){
  ...
  const rows=items.map(it=>{
    totalQty+=(it.qty||0);
    const ar=it.nameAr?_arBlock(it.nameAr,bodyFont):"";
    const descLine=(it.descriptions&&it.descriptions.length>0)?`<div style="font-size:${bodyFont-4}px;font-style:italic;color:#333;padding-left:6px;margin-top:1px">↳ ${it.descriptions.map(_escHTML).join(", ")}</div>`:"";
    return `<div style="margin:${lineGap}px 0;font-weight:${wItems}">
...
function POS({items,sales,setSales,tables,setTables,promos,license,lang="en",currentUser=null}){
  ...
  async function confirmPayment(method,given,change,promo,totalDiscountAmt,finalTotal,finalVat,printAndSave,payInfo,manualDiscountAmt,promoDiscountAmt,isDraft=false,extraData={}){
    ...
    const treatAsDraft=isDraft||isKotOnly;
    let invoiceId;
    if(treatAsDraft){
      // Draft / KOT Only: use current vno + letter suffix (don't increment vno)
    ...
    const dailyToken=treatAsDraft?getDailyToken():bumpDailyToken();
    const inv={
      id:invoiceId,
      token:dailyToken,
      voucher:invoiceId,
      ...
      vat:finalVat,
      total:finalTotal,
      status:treatAsDraft?"draft":"completed",
      ...
      printAndSave,
      isDraft:treatAsDraft||false,
      isKotOnly:isKotOnly||false,
      baseVno:treatAsDraft?vno:undefined,
    };
    // ── SAVE INVOICE ─────────────────────────────────────────────
    ...
    if(treatAsDraft){
      ...
      try{
        const drafts=LS.get("restopos_draft_invoices")||[];
        LS.set("restopos_draft_invoices",[...drafts,{...inv,isDraft:true,isKotOnly}]);
        ...
        if(!kotDone&&"serial" in navigator){
          try{
            if(isPortOpen(_kitchenPort)||(await getAvailablePorts()).length>0){
              await printKOTEscPos(kotCart,kType,kTable,newKot2,inv.billType);
              kotDone=true;
            }
    ...
    if(!printAndSave){
      ...
      try{
        ...
        const kotHTML=buildKOTHtml({kot:newKotK,token:inv.token,voucher:inv.voucher||inv.id,date:inv.date,type:inv.type==="Dine-in"?"Dine-in":inv.type==="Takeaway"?"Takeaway":"Delivery",billType:inv.billType,table:kTable,time:new Date().toLocaleTimeString("en-SA"),items:inv.items||[]});
        let kotDone=false;
        if(isQZConnected()&&_qzKitchenPrinter){
          try{await printWithQZ(kotHTML,_qzKitchenPrinter,"80mm");kotDone=true;}catch(e){console.warn("[KOT-Only QZ]",e);}
        }
        if(!kotDone){
          try{
            let frame=document.getElementById("restopos-print-frame");
  ...
  async function printKOT(){
    ...
    if(isQZConnected()&&_qzKitchenPrinter){
      try{
        ...
        const kotHTML=buildKOTHtml({kot:newKot,token:getDailyToken(),voucher:"KOT-"+newKot,date:TODAY,type:_kType,billType,table:selectedTable,time:new Date().toLocaleTimeString("en-SA"),items:cart});
    ...
    try{
      if("serial" in navigator&&(isPortOpen(_kitchenPort)||(await getAvailablePorts()).length>0)){
        await printKOTEscPos(cart,orderType,selectedTable,newKot,billType);
  ...
  // ── POS edit mode: drag-to-reorder + box-size ──
  const [editMode,setEditMode]=useState(false);
  const [itemOrder,setItemOrder]=useState(()=>getItemOrder());       // committed order
  const [draftOrder,setDraftOrder]=useState({});                      // unsaved order while editing
  const [boxHeight,setBoxHeight]=useState(()=>getPosBoxHeight());     // committed height
  const [draftHeight,setDraftHeight]=useState(()=>getPosBoxHeight()); // unsaved height while editing
  const [dragId,setDragId]=useState(null);
  const effOrder=editMode?draftOrder:itemOrder;
  const effHeight=editMode?draftHeight:boxHeight;
  function enterEdit(){setDraftOrder({...itemOrder});setDraftHeight(boxHeight);setEditMode(true);}
  function cancelEdit(){setEditMode(false);setDragId(null);}
  function saveEdit(){
    setItemOrder(draftOrder);saveItemOrder(draftOrder);
    setBoxHeight(draftHeight);savePosBoxHeight(draftHeight);
    setEditMode(false);setDragId(null);
    showN("✅ Layout saved");
  }
  // Reorder within the current visible list: move dragId before targetId
  function handleReorder(targetId){
    ...
    arr.forEach((id,idx)=>{next[id]=idx;});
    setDraftOrder(next);
  }
  ...
  const filteredItems=useMemo(()=>{
    ...
    // Apply custom drag order: items with a saved index sort first by it, the rest keep natural order
    const ord=effOrder||{};
    return list.slice().sort((a,b)=>{
      const ai=ord[a.id], bi=ord[b.id];
  ...
  return(
    <div style={{display:"flex",height:"calc(100vh - 52px)",overflow:"hidden"}}>
              ...
              <div style={{background:"#F8FAFF",borderRadius:10,overflow:"hidden",border:"1px solid #E0E8F0"}}>
                ...
                {(bill.items||[]).map((it,i)=>(
                  <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",
                    ...
                    <div>
                      {it.nameAr&&<div style={{fontSize:11,color:"#1A3A5C",direction:"rtl",fontFamily:"'Tahoma','Arial','Times New Roman',serif",fontWeight:700}}>{it.nameAr}</div>}
      ...
      {showDescModal&&descModalIdx!==null&&cart[descModalIdx]&&(()=>{
        const descSettings=LS.get("restopos_description_settings")||{};
        const options=descSettings.options||[];
        const item=cart[descModalIdx];
        const selected=item.descriptions||[];
        function toggleDesc(opt){
          setCart(prev=>prev.map((c,i)=>{
            if(i!==descModalIdx)return c;
            const cur=c.descriptions||[];
            return{...c,descriptions:cur.includes(opt)?cur.filter(x=>x!==opt):[...cur,opt]};
          }));
        }
        return(
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.55)",zIndex:8000,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
            <div style={{background:"#fff",borderRadius:16,padding:24,width:"100%",maxWidth:380,boxShadow:"0 16px 60px rgba(0,0,0,0.3)"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
                <div>
                  <div style={{fontSize:15,fontWeight:800,color:"#1A3A5C"}}>📋 Description</div>
                  <div style={{fontSize:11,color:"#8A9AB0",marginTop:2}}>{item.name}</div>
                </div>
                <button onClick={()=>setShowDescModal(false)} style={{background:"#F0F4F8",border:"none",borderRadius:"50%",width:30,height:30,cursor:"pointer",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center",color:"#5A7A9A"}}>×</button>
              </div>
              {options.length===0?(
                <div style={{textAlign:"center",padding:"20px 0",color:"#94a3b8",fontSize:13}}>No options set yet.<br/><span style={{fontSize:11}}>Add them in Advanced → Description</span></div>
              ):(
                <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:16}}>
                  {options.map(opt=>{
                    const active=selected.includes(opt);
                    return(
                      <button key={opt} onClick={()=>toggleDesc(opt)} style={{padding:"8px 14px",borderRadius:20,border:`2px solid ${active?"#7C3AED":"#E0E8F0"}`,background:active?"#EDE9FE":"#F8FAFF",color:active?"#7C3AED":"#5A7A9A",fontFamily:"inherit",fontSize:12,fontWeight:700,cursor:"pointer"}}>{active?"✓ ":""}{opt}</button>
                    );
                  })}
                </div>
              )}
              {selected.length>0&&<div style={{fontSize:11,color:"#7C3AED",marginBottom:12,padding:"6px 10px",background:"#EDE9FE",borderRadius:8}}>Selected: {selected.join(", ")}</div>}
              <button onClick={()=>setShowDescModal(false)} style={{width:"100%",padding:"11px",background:"linear-gradient(135deg,#7C3AED,#5B21B6)",color:"#fff",border:"none",borderRadius:10,fontSize:14,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>✓ Done</button>
            </div>
          </div>
        );
      })()}
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",borderRight:`1px solid ${C.border}`,background:C.bg,overflow:"hidden"}}>
        <div style={{padding:"8px 12px",background:C.zatcaLight,borderBottom:`1px solid ${C.border}`,display:"flex",gap:8,alignItems:"center"}}>
          ...
          {!editMode
            ...
            : <div style={{display:"flex",gap:6,flexShrink:0}}>
                <button onClick={saveEdit} title="Save layout" style={{height:36,padding:"0 14px",borderRadius:8,border:"none",background:C.success,color:"#fff",cursor:"pointer",fontSize:13,fontWeight:800,fontFamily:"inherit",display:"flex",alignItems:"center",gap:5}}>💾 Save</button>
                <button onClick={cancelEdit} title="Discard changes" style={{height:36,padding:"0 12px",borderRadius:8,border:`1.5px solid ${C.border}`,background:"#fff",color:C.textMid,cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:"inherit"}}>Cancel</button>
              </div>}
        </div>
        {editMode&&<div style={{padding:"8px 12px",background:"#FFF8E8",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:12}}>
          <span style={{fontSize:12,fontWeight:700,color:"#8a6d00",whiteSpace:"nowrap"}}>📐 Box height</span>
          <input type="range" min={44} max={160} step={4} value={draftHeight} onChange={e=>setDraftHeight(parseInt(e.target.value))} style={{flex:1,accentColor:C.primary}}/>
          <span style={{fontSize:12,fontWeight:700,color:C.primary,width:42,textAlign:"right"}}>{draftHeight}px</span>
          <span style={{fontSize:11,color:"#8a6d00",whiteSpace:"nowrap"}}>·  drag items to reorder</span>
        </div>}
        <div style={{display:"flex",gap:4,padding:"8px 12px",overflowX:"auto",borderBottom:`1px solid ${C.border}`,background:"#fff",flexShrink:0}}>
          {cats.map(cat=>{
            ...
            const label=isFav?"★":cat;
            return(
              <button key={cat} onClick={()=>setActiveCat(cat)} title={isFav?"Favourites":cat} style={{padding:isFav?"6px 12px":"6px 14px",borderRadius:20,border:active?`1.5px solid ${dot||C.primary}`:"1px solid transparent",background:active?(dot?dot+"1A":C.primary):C.bg,color:active?(dot||"#fff"):C.textMid,fontFamily:"inherit",fontSize:isFav?15:12,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0,display:"flex",alignItems:"center",gap:6}}>
                {!isFav&&dot&&<span style={{width:9,height:9,borderRadius:"50%",background:dot,flexShrink:0}}/>}
                {label}
              </button>
            );
          })}
          ...
          {filteredItems.map(item=>{
            ...
            return(
              <div key={item.id} role="button"
                draggable={editMode}
                onDragStart={editMode?(()=>setDragId(item.id)):undefined}
                onDragOver={editMode?(e=>{e.preventDefault();}):undefined}
                onDrop={editMode?(e=>{e.preventDefault();handleReorder(item.id);}):undefined}
                onDragEnd={editMode?(()=>setDragId(null)):undefined}
                onClick={editMode?undefined:(()=>addToCart(item))}
                style={{position:"relative",minHeight:effHeight,display:"flex",flexDirection:"column",justifyContent:"center",background:dragging?col+"33":col+"0D",border:`1.5px solid ${col}55`,borderLeft:`5px solid ${col}`,borderRadius:12,padding:effHeight<70?"6px 10px":"12px 10px",cursor:editMode?"grab":"pointer",fontFamily:"inherit",textAlign:"left",opacity:dragging?0.5:1,boxShadow:dragging?"0 4px 14px rgba(0,0,0,0.18)":"none",transition:"min-height .12s,padding .12s"}}>
                {editMode&&<span style={{position:"absolute",top:4,left:6,fontSize:13,color:col,opacity:0.7}}>⠿</span>}
                {!editMode&&<button onClick={e=>{e.stopPropagation();toggleFavourite(item.id);}} title={isFav?"Remove from Favourites":"Add to Favourites"} style={{position:"absolute",top:5,right:5,background:"transparent",border:"none",cursor:"pointer",fontSize:16,lineHeight:1,padding:2,color:isFav?"#F0A500":"#C9CDD6"}}>{isFav?"★":"☆"}</button>}
                <div style={{fontSize:13,fontWeight:700,color:C.text,marginBottom:effHeight<70?2:4,lineHeight:1.3,paddingRight:editMode?2:16,paddingLeft:editMode?14:0}}>{item.name}</div>
                {item.nameAr&&effHeight>=70&&<div style={{fontSize:11,color:C.textLight,direction:"rtl",marginBottom:6}}>{item.nameAr}</div>}
                <div style={{fontSize:14,fontWeight:900,color:col}}>SAR {item.price}</div>
                {item.stock<10&&effHeight>=70&&<div style={{fontSize:10,color:C.danger,fontWeight:600,marginTop:3}}>Low stock</div>}
              </div>
            );
          })}
      ...
      <div style={{width:340,display:"flex",flexDirection:"column",background:"#fff",flexShrink:0}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"8px 14px"}}>
          {cart.length===0?<div style={{textAlign:"center",padding:"40px 0",color:C.textLight}}><div style={{fontSize:32,marginBottom:8}}>🛒</div><div style={{fontSize:13}}>Cart is empty</div></div>:cart.map((item,idx)=>{
            const descSettings=LS.get("restopos_description_settings")||{};
            const descEnabled=descSettings.enabled;
            return(
              <div key={idx}>
                <div onClick={()=>{
                  if(selectedRow===idx&&descEnabled){setDescModalIdx(idx);setShowDescModal(true);}
                  else setSelectedRow(idx===selectedRow?null:idx);
                ...
                {item.descriptions&&item.descriptions.length>0&&(
                  <div style={{fontSize:10,color:"#7C3AED",paddingLeft:10,marginBottom:4,marginTop:-1,fontStyle:"italic"}}>
...
// ═══════════════════════════════════════════════════════════════════
function SalesProgressBar({sales}){
  const pbSettings=LS.get("restopos_progressbar_settings")||{};
  if(!pbSettings.enabled||!pbSettings.goal)return null;
  const goal=pbSettings.goal||1;
  const todaySales=(sales||[]).filter(s=>s.date===TODAY&&!s.isDraft);
  const cardSales=todaySales.filter(s=>s.payMethod==="Card").reduce((sum,s)=>sum+(s.cardAmount||s.total||0),0)
    +todaySales.filter(s=>s.payMethod==="Both").reduce((sum,s)=>sum+(s.cardAmount||0),0);
  const cashSales=todaySales.filter(s=>s.payMethod==="Cash").reduce((sum,s)=>sum+(s.total||0),0)
    +todaySales.filter(s=>s.payMethod==="Both").reduce((sum,s)=>sum+(s.cashAmount||0),0);
  const totalSales=todaySales.reduce((sum,s)=>sum+(s.total||0),0);
  const pct=v=>Math.min(100,Math.round((v/goal)*100));
  const Bar=({value,color,label,icon})=>(
    <div style={{marginBottom:8}}>
      <div style={{display:"flex",justifyContent:"space-between",fontSize:11,fontWeight:700,marginBottom:3}}>
        <span style={{color:"rgba(255,255,255,0.8)"}}>{icon} {label}</span>
        <span style={{color:"#fff"}}>SAR {value.toLocaleString("en-SA",{minimumFractionDigits:2,maximumFractionDigits:2})} <span style={{color:"rgba(255,255,255,0.5)",fontWeight:500}}>/ {pct(value)}%</span></span>
      </div>
      <div style={{height:8,background:"rgba(255,255,255,0.15)",borderRadius:4,overflow:"hidden"}}>
        <div style={{height:"100%",width:pct(value)+"%",background:color,borderRadius:4,transition:"width 0.5s ease"}}/>
      </div>
    </div>
  );
  return(
    <div style={{background:"linear-gradient(135deg,#1A3A5C 0%,#0F2340 100%)",borderRadius:10,padding:"12px 16px",border:"1px solid rgba(255,255,255,0.1)",boxShadow:"0 2px 8px rgba(0,0,0,0.1)",minWidth:220,flex:1}}>
      <div style={{fontSize:11,fontWeight:800,color:"rgba(255,255,255,0.6)",marginBottom:10,letterSpacing:"0.05em"}}>📊 DAILY GOAL — SAR {goal.toLocaleString()}</div>
      <Bar value={cardSales} color="#60A5FA" label="Card Sales" icon="💳"/>
      <Bar value={cashSales} color="#34D399" label="Cash Sales" icon="💵"/>
      <Bar value={totalSales} color="#F0A500" label="Total Sales" icon="📊"/>
    </div>
  );
}
...
function Dashboard({sales,items,license,lang="en"}){
  ...
  return(
    <div>
      {/* TOP ROW: Clock + progress bar + compact license widget */}
      <div style={{display:"flex",gap:14,marginBottom:20,alignItems:"flex-start",flexWrap:"wrap"}}>
        <div style={{flex:"0 0 auto"}}><LiveClock/></div>
        <SalesProgressBar sales={sales}/>
...
// ═══════════════════════════════════════════════════════════════════
const RECEIPT_FONTS=[
  {id:"amiri",label:"Amiri (recommended)",family:"'Tahoma','Arial','Times New Roman',serif",lang:"AR"},
...
function PresetFormatTab({license,company}){
  ...
  const [weightSection,setWeightSection]=useState("weightItems"); // which section the thickness slider controls
  ...
  // defaults per mode
  const DEFAULTS={
    invoice:{usePreset:true,presetStyle:"s1",paperWidth:"80mm",headFont:14,bodyFont:12,totalFont:16,tokenFont:22,lineGap:3,logoSize:60,qrSize:120,headerColor:"#1A6B4A",logoUrl:"",extraInfo:"",extraInfoAr:"",weightItems:700,weightMeta:600,weightToken:900,weightTotal:900,footer:"Thank you — visit again",footerAr:"شكراً لك زيارة مرة أخرى"},
    draft:{usePreset:true,presetStyle:"s1",paperWidth:"80mm",headFont:14,bodyFont:12,totalFont:16,tokenFont:22,lineGap:3,logoSize:60,headerColor:"#1A6B4A",logoUrl:"",extraInfo:"",extraInfoAr:"",weightItems:700,weightMeta:600,weightToken:900,weightTotal:900,footer:"Thank you — visit again",footerAr:"شكراً لك زيارة مرة أخرى"},
    kot:{usePreset:true,presetStyle:"s1",paperWidth:"80mm",headFont:20,bodyFont:16,tokenFont:28,lineGap:4,logoSize:50,logoUrl:"",kotBoxPad:6,weightItems:800,weightMeta:700,weightToken:900,kotTitle:"Telephone Bill"},
  };
  ...
  function save(){
    // Tapping Save ALWAYS enables the preset for this mode (that's the whole point).
    const data={...fmtAll[mode],usePreset:true};
  ...
  function previewHTML(){
    // When the "Telephone in box" toggle is on, preview as a telephone order so
    // the effect is visible live. Otherwise preview the normal sample order.
    const _order=fmt.phoneInBox?{...sampleOrder,billType:"telephone"}:sampleOrder;
    if(mode==="kot")return buildPresetKOT(_order,fmt);
    if(mode==="draft")return buildPresetHTML(_order,prevLicense,null,fmt,null,{draft:true});
    return buildPresetHTML(_order,prevLicense,prevZatca,fmt,previewQR,{draft:false});
  }
    ...
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
        {modeTabs.map(([id,label])=>(
          <button key={id} onClick={()=>{setMode(id);setSavedMsg("");setImgTest({});if(id==="kot"&&weightSection==="weightTotal")setWeightSection("weightItems");}}
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18,alignItems:"start"}} className="preset-grid">
        ...
        <Card>
          ...
          <div style={{border:`1px solid ${C.border}`,borderRadius:10,padding:"14px",marginBottom:16,background:C.bg}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              ...
              {/* business name logic note */}
              <div style={{fontSize:10.5,color:fmt.logoUrl?C.textMid:C.warning,fontWeight:600,lineHeight:1.5,background:fmt.logoUrl?"#f0f7ff":"#fff8e8",border:`1px solid ${fmt.logoUrl?"#cfe2ff":"#ffe0a0"}`,borderRadius:8,padding:"7px 10px"}}>
                {fmt.logoUrl
                  ? "✅ Logo is added — business name is optional. Leave blank to show only the logo, or type a name to show both."
                  : "⚠️ No logo added — business name (English) is required and will be printed at the top. Add a logo above to make it optional."}
              </div>
              {mode!=="kot"&&(
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                  <div>
                    <label style={{fontSize:11,fontWeight:700,color:C.textMid,display:"block",marginBottom:4}}>Extra info (English) — optional</label>
                    <input value={fmt.extraInfo||""} onChange={e=>set("extraInfo",e.target.value)} placeholder="e.g. Branch: Kakkiyah"
                      style={{width:"100%",padding:"8px 11px",border:`1px solid ${C.border}`,borderRadius:8,fontSize:13,fontFamily:"inherit",color:C.text,background:"#fff"}}/>
                  </div>
                  <div>
                    <label style={{fontSize:11,fontWeight:700,color:C.textMid,display:"block",marginBottom:4}}>معلومات إضافية (عربي) — اختياري</label>
                    <input value={fmt.extraInfoAr||""} onChange={e=>set("extraInfoAr",e.target.value)} dir="rtl" placeholder="مثال: فرع الكعكية"
                      style={{width:"100%",padding:"8px 11px",border:`1px solid ${C.border}`,borderRadius:8,fontSize:13,fontFamily:"'Tajawal',sans-serif",color:C.text,background:"#fff"}}/>
                  </div>
                </div>
              )}
              {mode!=="kot"&&<div style={{fontSize:10.5,color:C.textMid,fontWeight:500,marginTop:-2}}>ℹ️ Extra info prints between the business name and the Tel line.</div>}
              {mode!=="kot"&&(
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                  <div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14,marginBottom:16}}>
            ...
            {mode==="kot"&&<Slider label="Token box padding" k="kotBoxPad" min={2} max={20}/>}
          </div>

          {/* ── TEXT THICKNESS (per-section) ── */}
          <div style={{border:`1px solid ${C.border}`,borderRadius:10,padding:"14px",marginBottom:16,background:C.bg}}>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
              <div style={{fontSize:12,fontWeight:800,color:C.text}}>🅱️ Text thickness (boldness)</div>
              <button onClick={()=>setShowWeightGuide(v=>!v)} title="How this works"
                style={{width:20,height:20,borderRadius:"50%",border:`1.5px solid ${C.primary}`,background:"#fff",color:C.primary,fontSize:12,fontWeight:800,cursor:"pointer",lineHeight:1,padding:0}}>?</button>
            </div>
            {showWeightGuide&&<div style={{fontSize:11,color:C.textMid,marginBottom:10,lineHeight:1.5,background:"#fff",border:`1px solid ${C.border}`,borderRadius:8,padding:"8px 10px"}}>
              First pick which part of the bill you want to change (Items, Header info, Token, or Totals), then drag the slider to make just that part thinner or bolder. Each part remembers its own thickness.
            </div>}
            {/* section picker */}
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>
              {(mode==="kot"?[["weightItems","Items"],["weightMeta","Header info"],["weightToken","Token"]]:[["weightItems","Items"],["weightMeta","Header info"],["weightToken","Token"],["weightTotal","Totals"]]).map(([k,label])=>(
                  <button key={k} onClick={()=>setWeightSection(k)}
                    style={{padding:"6px 12px",borderRadius:8,border:`2px solid ${weightSection===k?C.primary:C.border}`,background:weightSection===k?C.primaryLight:"#fff",color:weightSection===k?C.primary:C.textMid,fontFamily:"inherit",fontSize:12,fontWeight:700,cursor:"pointer"}}>{label}</button>
              ))}
            </div>
            {/* the single thickness slider — controls the chosen section */}
            <div style={{display:"flex",flexDirection:"column",gap:4}}>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:12,fontWeight:600,color:C.textMid}}>
                <span>Thickness of: <strong style={{color:C.primary}}>{({weightItems:"Items",weightMeta:"Header info",weightToken:"Token",weightTotal:"Totals"})[weightSection]}</strong></span>
                <span style={{color:C.primary,fontWeight:700}}>{fmt[weightSection]||700}</span>
              </div>
              <input type="range" min={400} max={900} step={100} value={fmt[weightSection]||700} onChange={e=>set(weightSection,parseInt(e.target.value))}
                style={{width:"100%",accentColor:C.primary}}/>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:C.textLight}}>
                <span>Thin</span><span>Normal</span><span>Bold</span><span>Extra Bold</span>
              </div>
            </div>
          </div>
          ...
          {/* ── OPTIONAL LAYOUT EXTRAS (invoice & draft only) ── */}
          {mode!=="kot"&&(
            <div style={{marginBottom:16,padding:"12px 14px",background:C.bg,borderRadius:10,border:`1px solid ${C.border}`}}>
              <div style={{fontSize:13,fontWeight:800,color:C.text,marginBottom:10}}>Layout extras</div>
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {[
                  ["lineAfterTitle","Divider line after “Simplified Tax Invoice”"],
                  ["lineAfterToken","Divider line after Token No"],
                  ["phoneInBox","Put “Telephone” label in a box (phone bills)"]
                ].map(([key,label])=>(
                  <label key={key} style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",fontSize:12.5,color:C.textMid,fontWeight:600}}>
                    <input type="checkbox" checked={!!fmt[key]} onChange={e=>set(key,e.target.checked)} style={{width:18,height:18,cursor:"pointer",accentColor:C.primary}}/>
                    <span>{label}</span>
                  </label>
                ))}
              </div>
        ...
        {/* ── RIGHT: live preview (sticky — stays in view while left scrolls) ── */}
...
function reprintReceipt(sale,license){
  ...
  const itemsHTML=cats.map(cat=>{
    ...
    return `<div style="font-size:9px;font-weight:bold;letter-spacing:0.08em;color:#555;margin-top:6px;margin-bottom:2px;text-transform:uppercase;">${cat}</div>`+
      catItems.map(it=>`<div class="row"><span class="item-name">${it.nameAr?`<span style="font-family:'Tahoma','Arial','Times New Roman',serif;direction:rtl;display:block;font-weight:700;">${it.nameAr}</span>`:""}<span style="display:block">${it.name}</span><small>${it.qty} x SAR ${it.price.toFixed(2)}</small></span><span class="item-amt">SAR ${(it.qty*it.price).toFixed(2)}</span></div>`).join("");
  }).join("");
...
<div style="font-family:'Tahoma','Arial','Times New Roman',serif;font-size:14px;font-weight:bold;text-align:center;direction:rtl;margin-top:3px;">شكراً لزيارتكم</div><br/><br/>
...
function Reports({sales,allSales,items,setSales}){
  ...
  function buildSummaryData(salesArr,dateStr){
    ...
    // Per-category: qty, revenue (incl VAT) and tax portion (VAT)
    const catMap={};
    salesArr.forEach(o=>(o.items||[]).forEach(it=>{
      const cat=it.category||"Items";
      if(!catMap[cat])catMap[cat]={cat,qty:0,revenue:0,tax:0};
    ...
    // Payment split for the totals block
    const cashSale=salesArr.filter(o=>o.payMethod==="Cash").reduce((s,o)=>s+(o.total||0),0);
    const cardSale=salesArr.filter(o=>["Card","Both"].includes(o.payMethod)).reduce((s,o)=>s+(o.total||0),0);
    const creditSale=salesArr.filter(o=>o.payMethod==="Credit").reduce((s,o)=>s+(o.total||0),0);
    // Time range: first sale → now (close)
    const sorted=salesArr.slice().sort((a,b)=>String(a.time).localeCompare(String(b.time)));
    const startTime=sorted.length?(sorted[0].time||""):"";
    return{date:dateStr,orderCount:salesArr.length,revenue,vat,discount,payBreakdown,catList,expenses:expTotal,netRevenue:revenue-expTotal,
      cashSale,cardSale,creditSale,startTime,closeTime:new Date().toLocaleTimeString("en-SA")};
  }
...
${thermal?`
<div class="c" style="direction:rtl;font-family:'Tahoma','Arial','Segoe UI',sans-serif;font-weight:bold;font-size:15px;margin-bottom:3px">${_escHTML(lic.businessNameAr||lic.businessName||"")}</div>
${(lic.addressAr||lic.address)?`<div class="c" style="direction:rtl;font-family:'Tahoma','Arial','Segoe UI',sans-serif;font-size:11px;line-height:1.5;margin-bottom:4px">${_escHTML(lic.addressAr||lic.address)}</div>`:""}
`:`<div class="c" style="font-size:22px;font-weight:900;color:#1A6B4A;margin-bottom:4px">DAILY SUMMARY</div>
<div class="c">${data.date} · ${lic.businessName||"Restaurant"}</div>
<div class="c" style="font-size:10px;color:#666">VAT: ${lic.vatNumber||""} · Printed: ${new Date().toLocaleString("en-SA")}</div>`}
${thermal?(()=>{
  const totalSales=data.revenue;
  const totalSalesTax=data.vat;
  const dateDisp=fmtDate(data.date);
  const startT=data.startTime||"";
  const closeT=data.closeTime||new Date().toLocaleTimeString("en-SA");
  return `
<div class="c" style="font-size:13px;font-weight:700;margin-bottom:2px">${data.dateRange||(dateDisp+" To "+dateDisp)}</div>
<div class="c" style="font-size:11px;margin-bottom:4px">${startT} To ${closeT}</div>
<div style="font-size:11px;margin-bottom:4px">User : ${(LS.get("restopos_currentUser")?.name)||"admin"}</div>
<div class="hr"/>
<div style="display:flex;font-weight:900;font-size:12px;border-bottom:1px solid #000;padding-bottom:2px">
  <span style="flex:1 1 auto">ProductName</span><span style="width:38px;text-align:right">Qty</span><span style="width:54px;text-align:right">Tax</span><span style="width:62px;text-align:right">Amount</span></div>
${data.catList.map(c=>`<div style="margin:5px 0">
  <div style="font-weight:800;font-size:13px">${_escHTML(c.cat)}</div>
  <div style="display:flex;font-size:12px">
    <span style="flex:1 1 auto"></span>
    <span style="width:38px;text-align:right">${c.qty}</span>
    <span style="width:54px;text-align:right">${c.tax.toFixed(2)}</span>
    <span style="width:62px;text-align:right">${c.revenue.toFixed(2)}</span>
  </div></div>`).join("")}
<div class="hr"/>
<div class="row"><span>Total Purchase:</span><span>0.00</span></div>
<div class="row"><span>Total Sales:</span><span>${totalSales.toFixed(2)}</span></div>
<div class="row"><span>Total Sales Tax:</span><span>${totalSalesTax.toFixed(2)}</span></div>
<div class="row"><span>Total Purchase Tax:</span><span>0.00</span></div>
<div class="row"><span>(Sales Tax-Purchase Tax):</span><span>${totalSalesTax.toFixed(2)}</span></div>
<div class="row"><span>Total Card Sales:</span><span>${(data.cardSale||0).toFixed(2)}</span></div>
<div class="row"><span>Total Cash Sale:</span><span>${(data.cashSale||0).toFixed(2)}</span></div>
<div class="row"><span>Total Credit Sale:</span><span>${(data.creditSale||0).toFixed(2)}</span></div>
<div class="row"><span>Total Discount:</span><span>${(data.discount||0).toFixed(2)}</span></div>
<div class="row"><span>Total Payment:</span><span>0.00</span></div>
<div class="row b"><span>Balance:</span><span>${(totalSales-(data.discount||0)).toFixed(2)}</span></div>
${data.expenses>0?`<div class="hr"/><div class="row"><span>Expenses</span><span>-SAR ${data.expenses.toFixed(2)}</span></div><div class="row b"><span>NET REVENUE</span><span>SAR ${data.netRevenue.toFixed(2)}</span></div>`:""}
${data.drafts&&data.drafts.length>0?`<div class="hr"/><div class="b">DRAFT BILLS (${data.drafts.length})</div>${data.drafts.map(d=>`<div class="row"><span>${d.id}</span><span>SAR ${(d.total||0).toFixed(2)}</span></div>`).join("")}`:""}
`;})():`
...
<br/><br/></body></html>`;
  ...
  function handleCloseDay(){
    ...
    const payBreakdown={};
    ["Cash","Card","Both"].forEach(m=>{payBreakdown[m]=todaySales.filter(s=>s.payMethod===m).reduce((s,o)=>s+o.total,0);});
    ...
    // Real open → close time/date range for the printed report
    const openClock=firstSale?firstSale.time:new Date().toLocaleTimeString("en-SA");
    const closeClock=new Date().toLocaleTimeString("en-SA");
    setDaySummaryData({...sd,drafts:todayDrafts,startTime:openClock,closeTime:closeClock,dateRange:fmtDate(TODAY)+" To "+fmtDate(TODAY)});
  ...
  return(<div>
    ...
    {tab==="system"&&<Card style={{maxWidth:560}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        ...
        {window.electronAPI?.isElectron&&<div style={{display:"flex",gap:12,padding:"14px 16px",background:C.primaryLight,borderRadius:10,border:`1.5px solid ${C.primary}`,alignItems:"center"}}>
          <div style={{flex:1}}><div style={{fontSize:14,fontWeight:700,color:C.primary}}>⬇️ Download Latest Update</div><div style={{fontSize:12,color:C.textMid}}>Pulls the newest RestoPOS version. Requires internet — downloads once, then works offline.</div></div>
          <Btn variant="accent" onClick={async()=>{
            if(!window.electronAPI)return;
            if(!navigator.onLine){alert("⚠️ No internet connection. Connect to WiFi first, then try again.");return;}
            if(!confirm("Download and apply the latest RestoPOS update?\n\nThe app will reload with the newest version. Your data is safe."))return;
            setRefreshMsg("Downloading latest update…");
            const res=await window.electronAPI.downloadLatestUpdate();
            if(!res.success){alert("Update failed: "+(res.error||"Unknown error")+"\n\nReverting to current version.");setRefreshMsg("");}
          }}>⬇️ Update Now</Btn>
        </div>}
    ...
    <div style={{flex:1}}>
      ...
      {tab==="update"&&<div>
        <Card style={{marginBottom:16}}>
          <div style={{fontSize:18,fontWeight:800,marginBottom:4}}>🔄 App Update</div>
          <div style={{fontSize:13,color:C.textMid,marginBottom:16}}>Updates are pushed by RestoPOS. Tap the button to download and apply the latest version instantly.</div>
          <div style={{display:"flex",alignItems:"center",gap:14,padding:"14px 16px",background:C.bg,borderRadius:12,border:`1px solid ${C.border}`,marginBottom:14}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontSize:14,fontWeight:800}}>Installed version</div>
              <div style={{fontSize:12,color:C.textMid}}>{APP_VERSION_FULL}</div>
            </div>
            <Badge color={C.success} bg={C.successLight}>✓ Up to date</Badge>
          </div>
          <button onClick={forceUpdate} disabled={updating}
            ...
            {updating?"⏳ Applying update…":"⬇️ Check for Updates & Install"}
          </button>
          {updateMsg&&<div style={{marginTop:12,padding:"10px 14px",background:C.infoLight,border:`1px solid ${C.info}33`,borderRadius:10,fontSize:12.5,color:C.info,fontWeight:600,textAlign:"center"}}>{updateMsg}</div>}
          <div style={{marginTop:12,fontSize:11,color:C.textLight}}>Your data (sales, menu, settings) is safe — updating never deletes it.</div>
        </Card>
        <div style={{fontSize:15,fontWeight:800,marginBottom:12,color:C.text}}>📋 What's new in each version</div>
        {CHANGELOG.map((rel,ri)=>(
          <Card key={rel.version} style={{marginBottom:12,borderLeft:`4px solid ${rel.badgeColor||"#94a3b8"}`}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
              <div style={{flex:1}}>
                <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                  <span style={{fontSize:15,fontWeight:800,fontFamily:"monospace",color:C.text}}>{rel.version}</span>
                  {rel.badge&&<span style={{fontSize:10,fontWeight:800,padding:"2px 8px",borderRadius:20,background:rel.badgeColor+"22",color:rel.badgeColor,border:`1px solid ${rel.badgeColor}44`}}>{rel.badge}</span>}
                </div>
                <div style={{fontSize:11,color:C.textMid,marginTop:2}}>{rel.date}</div>
              </div>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:5}}>
              {rel.changes.map((ch,ci)=>(
                <div key={ci} style={{display:"flex",gap:8,alignItems:"flex-start",padding:"6px 10px",background:ri===0?C.successLight:C.bg,borderRadius:7,fontSize:12.5,color:C.text,lineHeight:1.5}}>
                  <span style={{flexShrink:0}}>{ch.slice(0,2)}</span>
                  <span>{ch.slice(3)}</span>
                </div>
              ))}
            </div>
          </Card>
...
function ShiftManager({sales,currentUser,lang="en"}){
  ...
  function exportShiftPDF(s){
    ...
    ${["Cash","Card","Both"].map(m=>`<div class="row"><span>${m}</span><strong>SAR ${(payBr[m]||0).toFixed(2)}</strong></div>`).join("")}
  ...
  const payMethods=["Cash","Card","Both"];
  return(
    <div>
...
async function printKOTEscPos(cart,orderType,tableId,kotNo,billType){
  const W=32;
  const line=(t)=>escpos.text(t.slice(0,W));
  const _isPhone=billType==="telephone";
  const data=escpos.merge(
    ...
    escpos.doubleSize(false),escpos.bold(false),
    // Telephone line on phone orders (only addition — labels unchanged)
    ...(_isPhone?[escpos.bold(true),escpos.doubleSize(true),escpos.text("TELEPHONE"),escpos.doubleSize(false),escpos.bold(false)]:[]),
    escpos.text("--------------------------------"),
    escpos.alignLeft(),
...
function ThermalPrinterSettings(){
  ...
  // Build a realistic test order using the client's actual license + saved preset
  function _testOrder(){
    const now=new Date();
    return {id:"TEST-001",voucher:"TEST-001",token:"T1",kot:"T1",date:now.toISOString().slice(0,10),time:now.toLocaleTimeString("en-SA"),user:"admin",type:"Takeaway",billType:"normal",payMethod:"Cash",
      items:[{name:"Test Item",nameAr:"صنف تجريبي",qty:1,price:10}],vat:1.30,total:10,discount:0,given:10,change:0};
  }
  function _testLicense(){return LS.get("restopos_license_v2")||{businessName:"RestoPOS Test",businessNameAr:"اختبار",address:"Test Address",vatNumber:"000000000000000",phone:"+966500000000"};}
  async function testBill(){
    setBillStatus("printing");setLog("⏳ Sending test…");
    const order=_testOrder(), lic=_testLicense();
    const fmt=LS.get("restopos_invoice_format")||{};
    // Step 1: if a real ESC/POS serial port is open, use it for native-style raw printing
    if(isPortOpen(_billPort)){
      try{await printReceiptEscPos(order,lic);setBillStatus("connected");setLog("✅ Test receipt printed via ESC/POS. Check your printer.");return;}
      catch(e){console.warn("[ESC/POS test]",e.message);}
    }
    // Step 2: fall back to QZ Tray using the PRESET design
    try{
      let qrImg="";
      try{
        const qrStr=generatePhase1QR({sellerName:lic.businessName||"",vatNumber:lic.vatNumber||"",timestamp:new Date().toISOString(),total:order.total,vatAmount:order.vat});
        qrImg=await makeQRDataURL(qrStr,240);
      }catch(_){}
      const html=buildReceiptHTML(order,lic,null,fmt,qrImg);
      const method=await silentPrintHTML(html,{paperWidth:fmt.paperWidth||"80mm"});
      setBillStatus("connected");setLog("✅ Test receipt printed via "+method+". Check your printer.");
    }catch(e){setBillStatus("error");setLog("❌ Could not print: "+e.message+". Make sure QZ Tray is running, or connect an ESC/POS printer above.");}
  }
  async function testKitchen(){
    setKitchenStatus("printing");setLog("⏳ Sending test…");
    const order={kot:"T1",token:"T1",type:"Dine-in",billType:"normal",table:5,time:new Date().toLocaleTimeString("en-SA"),date:new Date().toISOString().slice(0,10),voucher:"KOT-TEST",
      items:[{qty:1,name:"Test Burger",nameAr:"برجر تجريبي",price:0},{qty:2,name:"French Fries",nameAr:"بطاطس",price:0}]};
    // Step 1: ESC/POS serial if open
    if(isPortOpen(_kitchenPort)){
      try{await printKOTEscPos(order.items,"Dine-in",5,"T1");setKitchenStatus("connected");setLog("✅ KOT printed via ESC/POS.");return;}
      catch(e){console.warn("[ESC/POS KOT test]",e.message);}
    }
    // Step 2: QZ Tray fallback using the PRESET KOT design
    try{
      const kfmt=LS.get("restopos_kot_format")||{};
      const html=buildKOTHtml(order);
      const kp=LS.get("restopos_qz_kitchen_printer")||localStorage.getItem("restopos_qz_kitchen_printer");
      const method=await silentPrintHTML(html,{printer:kp,paperWidth:kfmt.paperWidth||"80mm"});
      setKitchenStatus("connected");setLog("✅ KOT printed via "+method+". Check your kitchen printer.");
    }catch(e){setKitchenStatus("error");setLog("❌ Could not print: "+e.message+". Make sure QZ Tray is running, or connect an ESC/POS printer above.");}
  }
...
<div class="c" style="font-family:'Tahoma','Arial','Segoe UI',sans-serif;direction:rtl">طابعة الفاتورة تعمل!</div>
<div class="hr"/>
...
<div style="font-family:'Tahoma','Arial','Segoe UI',sans-serif;direction:rtl">مطبعة المطبخ تعمل!</div>
<hr/>
...
// ═══════════════════════════════════════════════════════════════════
// DESCRIPTION SETTINGS — Advanced tab: create item modifier options
// ═══════════════════════════════════════════════════════════════════
function DescriptionSettings(){
  const [settings,setSettings]=useState(()=>LS.get("restopos_description_settings")||{enabled:false,options:[]});
  const [newOption,setNewOption]=useState("");
  const [saved,setSaved]=useState(false);
  function save(s){
    LS.set("restopos_description_settings",s);
    setSettings(s);
    setSaved(true);
    setTimeout(()=>setSaved(false),1500);
  }
  function toggleEnabled(){save({...settings,enabled:!settings.enabled});}
  function addOption(){
    const val=newOption.trim();
    if(!val)return;
    if(settings.options.includes(val)){setNewOption("");return;}
    save({...settings,options:[...settings.options,val]});
    setNewOption("");
  }
  function removeOption(opt){save({...settings,options:settings.options.filter(o=>o!==opt)});}
  return(
    <Card>
      <div style={{fontSize:15,fontWeight:800,marginBottom:4}}>📋 Description (Item Modifiers)</div>
      <div style={{fontSize:12,color:C.textMid,marginBottom:16}}>When enabled, the biller can tap a selected cart item to pick modifiers (e.g. No Onion). These print under the item in the KOT — not on the customer invoice.</div>
      {/* Toggle */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 14px",background:settings.enabled?C.primaryLight:C.bg,border:`1px solid ${settings.enabled?C.primary:C.border}`,borderRadius:10,marginBottom:16,cursor:"pointer"}} onClick={toggleEnabled}>
        <div>
          <div style={{fontSize:13,fontWeight:700,color:settings.enabled?C.primary:C.textMid}}>Description popup {settings.enabled?"enabled":"disabled"}</div>
          <div style={{fontSize:11,color:C.textLight}}>Tap a cart item (while selected) to open modifier popup</div>
        </div>
        <div style={{width:42,height:24,borderRadius:12,background:settings.enabled?C.primary:"#cbd5e1",display:"flex",alignItems:"center",padding:"0 3px",transition:"background 0.2s"}}>
          <div style={{width:18,height:18,borderRadius:"50%",background:"#fff",transform:settings.enabled?"translateX(18px)":"translateX(0)",transition:"transform 0.2s",boxShadow:"0 1px 4px rgba(0,0,0,0.2)"}}/>
        </div>
      </div>
      {/* Add options */}
      <div style={{fontSize:12,fontWeight:700,color:C.textMid,marginBottom:8}}>MODIFIER OPTIONS</div>
      <div style={{display:"flex",gap:8,marginBottom:12}}>
        <input value={newOption} onChange={e=>setNewOption(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")addOption();}} placeholder="e.g. No Onion, Extra Spicy…" style={{flex:1,padding:"8px 12px",border:`1px solid ${C.border}`,borderRadius:8,fontSize:13,fontFamily:"inherit",outline:"none"}}/>
        <Btn onClick={addOption} size="sm">+ Add</Btn>
      </div>
      <div style={{display:"flex",flexWrap:"wrap",gap:8,minHeight:40}}>
        {settings.options.length===0&&<div style={{fontSize:12,color:C.textLight,padding:"8px 0"}}>No options yet. Add some above.</div>}
        {settings.options.map(opt=>(
          <div key={opt} style={{display:"flex",alignItems:"center",gap:6,padding:"6px 12px",background:"#EDE9FE",border:"1.5px solid #C4B5FD",borderRadius:20}}>
            <span style={{fontSize:12,fontWeight:700,color:"#7C3AED"}}>{opt}</span>
            <button onClick={()=>removeOption(opt)} style={{background:"none",border:"none",color:"#7C3AED",cursor:"pointer",fontSize:14,lineHeight:1,padding:0,fontWeight:900}}>×</button>
          </div>
        ))}
      </div>
      {saved&&<div style={{marginTop:10,fontSize:12,color:C.success,fontWeight:600}}>✓ Saved</div>}
    </Card>
  );
}

// ═══════════════════════════════════════════════════════════════════
// PROGRESS BAR SETTINGS — Advanced tab: daily sales goal tracker
// ═══════════════════════════════════════════════════════════════════
function ProgressBarSettings(){
  const [settings,setSettings]=useState(()=>LS.get("restopos_progressbar_settings")||{enabled:false,goal:0});
  const [goalInput,setGoalInput]=useState(String(LS.get("restopos_progressbar_settings")?.goal||""));
  const [saved,setSaved]=useState(false);
  function save(s){
    LS.set("restopos_progressbar_settings",s);
    setSettings(s);
    setSaved(true);
    setTimeout(()=>setSaved(false),1500);
  }
  ...
  function saveGoal(){
    ...
    save({...settings,goal:g});
  }
  return(
    <Card>
      <div style={{fontSize:15,fontWeight:800,marginBottom:4}}>📊 Sales Progress Bar</div>
      <div style={{fontSize:12,color:C.textMid,marginBottom:16}}>Shows a live progress bar on the Dashboard next to the clock — tracks Card, Cash, and Total sales vs your daily goal.</div>
      {/* Toggle */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 14px",background:settings.enabled?C.primaryLight:C.bg,border:`1px solid ${settings.enabled?C.primary:C.border}`,borderRadius:10,marginBottom:16,cursor:"pointer"}} onClick={toggleEnabled}>
        <div>
          <div style={{fontSize:13,fontWeight:700,color:settings.enabled?C.primary:C.textMid}}>Progress bar {settings.enabled?"enabled":"disabled"}</div>
          <div style={{fontSize:11,color:C.textLight}}>Shown on Dashboard next to the live clock</div>
        </div>
        <div style={{width:42,height:24,borderRadius:12,background:settings.enabled?C.primary:"#cbd5e1",display:"flex",alignItems:"center",padding:"0 3px",transition:"background 0.2s"}}>
          <div style={{width:18,height:18,borderRadius:"50%",background:"#fff",transform:settings.enabled?"translateX(18px)":"translateX(0)",transition:"transform 0.2s",boxShadow:"0 1px 4px rgba(0,0,0,0.2)"}}/>
        </div>
      ...
      {/* Goal input */}
      <div style={{fontSize:12,fontWeight:700,color:C.textMid,marginBottom:8}}>DAILY SALES GOAL (SAR)</div>
      <div style={{display:"flex",gap:8,alignItems:"center"}}>
        <input value={goalInput} onChange={e=>setGoalInput(e.target.value)} type="number" min="0" placeholder="e.g. 5000" style={{flex:1,padding:"9px 12px",border:`1px solid ${C.border}`,borderRadius:8,fontSize:15,fontWeight:700,fontFamily:"inherit",outline:"none"}}/>
        <Btn onClick={saveGoal}>💾 Save Goal</Btn>
      </div>
      {settings.goal>0&&<div style={{marginTop:8,fontSize:12,color:C.textMid}}>Current goal: <strong>SAR {settings.goal.toLocaleString()}</strong></div>}
      {saved&&<div style={{marginTop:10,fontSize:12,color:C.success,fontWeight:600}}>✓ Saved</div>}
    </Card>
  );
}
...
function AdvancedFeatures({sales,items,setItems,license,company,invoiceFormat,setInvoiceFormat,users,setUsers}){
  ...
  const tabs=[["qztray","🖨️ QZ Tray"],["silentprint","🔇 Silent Printing"],["description","📋 Description"],["progressbar","📊 Progress Bar"],["kitchen","🍽️ Kitchen Printer"],["users","👤 Users"],["kds","🍳 KDS"],["stocktakes","📦 Stock Takes"],["recipes","📋 Recipes"],["giftcards","🎁 Gift Cards"],["delivery","🛵 Delivery"],["locations","🏢 Locations"],["accounting","📤 Accounting"],["reports","📅 Reports"],["printer","🖨️ ESC/POS"],["errorlog","⚠️ Error Log"],["analytics","📉 Analytics"],["audit","🔍 Audit Trail"],["tools","🔧 Tools"]];
  return(
    <div>
      <div style={{display:"flex",gap:6,marginBottom:20,flexWrap:"wrap"}}>
        {tabs.map(([id,lbl])=>(
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    async function flushPendingInvoices(){
      ...
      // Confirm we're actually online. navigator.onLine can be unreliable,
      // so we also try a lightweight reachability check to Firestore (always up).
      let online=navigator.onLine;
      if(online){
        try{
          await fetch("https://firestore.googleapis.com/",{method:"HEAD",mode:"no-cors"});
          online=true; // if fetch didn't throw, we have connectivity
        }catch{
          online=false; // network unreachable
        }
      }
      if(!online||cancelled)return;

      setAutoSyncStatus("syncing");
      console.log(`[AutoSync] Reporting ${queue.length} pending invoice(s) to FATOORA...`);

      let success=0,failed=0;
      for(const q of queue){
        if(cancelled)break;
        const inv=invoiceStorage.getOne(q.invoice_number);
        if(!inv){fatooraQueue.markFailed(q.invoice_number);continue;}
        // Skip if already reported
        if(inv.zatca_reported){fatooraQueue.markSent(q.invoice_number);success++;continue;}
        try{
          await reportToFatoora(inv);
          success++;
          console.log(`[AutoSync] ✅ Reported ${q.invoice_number}`);
        }catch(e){
          failed++;
          console.warn(`[AutoSync] ❌ Failed ${q.invoice_number}: ${e.message}`);
        }
        // Small delay between reports to avoid hammering the service
        await new Promise(r=>setTimeout(r,800));
      }

      if(!cancelled){
        setAutoSyncStatus(failed===0?"done":"error");
        console.log(`[AutoSync] Complete. Success: ${success}, Failed: ${failed}`);
        // Clear the status banner after 6 seconds
        setTimeout(()=>{if(!cancelled)setAutoSyncStatus(null);},6000);
        // Refresh any open invoice views
        try{window.dispatchEvent(new Event("restopos-invoice"));}catch(e){}
      }
    }

    // 1. Trigger when connection just came back
    if(justCameOnline){
      flushPendingInvoices();
    }

    // 2. Also trigger on mount (in case app starts online with pending invoices)
    flushPendingInvoices();

    // 3. Periodic safety check every 2 minutes
    const interval=setInterval(()=>{
      if(navigator.onLine)flushPendingInvoices();
    },120000);
  ...
  const ALL_NAV=[["dashboard","📊","Dashboard",["Admin","Manager"]],["pos","🖥️","POS",["Admin","Manager","Cashier"]],["settings","⚙️","Settings",["Admin"]],["create","➕","Create",["Admin","Manager"]],["transactions","💳","Transactions",["Admin","Manager"]],["financials","🏦","Financials",["Admin","Manager"]],["customers","👥","CRM",["Admin","Manager"]],["reports","📋","Reports",["Admin","Manager"]],["advanced","⚡","Advanced",["Admin","Manager"]],["vat","🧾","VAT",["Admin","Manager"]],["shifts","🔄","Shifts",["Admin","Manager"]],["help","❓","Help",["Admin","Manager","Cashier"]]];
