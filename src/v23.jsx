...
function POS({items,sales,setSales,tables,setTables,promos,license,lang="en"}){
  ...
  const [printBanner,setPrintBanner]=useState(null); // {msg,type} shown for 2sconst [vno,setVno]=useState(()=>LS.get("restopos_vno")||1);const [kotNo,setKotNo]=useState(()=>LS.get("restopos_kot")||1);
  ...
  async function confirmPayment(method,given,change,promo,totalDiscountAmt,finalTotal,finalVat,printAndSave,payInfo,manualDiscountAmt,promoDiscountAmt,isDraft=false,extraData={}){
        ...
        try{
          ...
          setTimeout(()=>{
            try{frame.contentWindow.focus();frame.contentWindow.print();
              setPrintBanner({msg:"✅ Saved & Printed Successfully",type:"success"});
              setTimeout(()=>setPrintBanner(null),2500);
            }catch(e){}
            setTimeout(()=>document.body.removeChild(frame),3000);
          },300);
        }catch(e){setShowReceipt(true);}
      }else{
        setPrintBanner({msg:"✅ Saved & Printed Successfully",type:"success"});
        setTimeout(()=>setPrintBanner(null),2500);
      }
    ...
    // Show save banner
    if(!printAndSave||saveWithoutPrint){
      setPrintBanner({msg:"💾 Invoice Saved Successfully",type:"save"});
      setTimeout(()=>setPrintBanner(null),2000);
    }
    ...
    <div style={{display:"flex",height:"calc(100vh - 52px)",overflow:"hidden"}}>
      ...
      {printBanner&&(
        <div style={{position:"fixed",top:20,left:"50%",transform:"translateX(-50%)",zIndex:9999,
          ...
          background:printBanner.type==="success"?"#1A6B4A":"#2176AE",
...
function Reports({sales,allSales,items,setSales}){
  ...
  const [txFrom,setTxFrom]=useState(()=>{const d=new Date();d.setMonth(d.getMonth()-1);return d.toISOString().slice(0,10);});
