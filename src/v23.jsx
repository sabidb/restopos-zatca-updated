...
function POS({items,sales,setSales,tables,setTables,promos,license,lang="en"}){
  ...
  async function confirmPayment(method,given,change,promo,totalDiscountAmt,finalTotal,finalVat,printAndSave,payInfo,manualDiscountAmt,promoDiscountAmt,isDraft=false,extraData={}){
    ...
    if(!printAndSave){
      ...
      // ── Try 3: Browser print popup (always works) ──────────────
      if(!printed){
        try{
          const printWin=window.open("","_blank","width=320,height=200,left=400,top=300,toolbar=no,menubar=no,scrollbars=no");
          if(printWin){
...
.box{text-align:center;padding:20px;border-radius:12px;background:#fff;box-shadow:0 4px 20px rgba(0,0,0,0.1);}
.spinner{width:32px;height:32px;border:3px solid #e0e8f0;border-top:3px solid #1A6B4A;border-radius:50%;animation:spin 0.8s linear infinite;margin:0 auto 12px;}
@keyframes spin{to{transform:rotate(360deg)}}
</style></head><body><div class="box"><div class="spinner"></div><div style="font-size:14px;font-weight:700;color:#1A3A5C">Printing...</div><div style="font-size:11px;color:#888;margin-top:4px">Page 1 of document</div></div>
