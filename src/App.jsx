...
// ═══════════════════════════════════════════════════════════════════
// DEVICE FINGERPRINT — stable ID per browser/device
// ═══════════════════════════════════════════════════════════════════
function getOrCreateDeviceId(){
  let id=localStorage.getItem("restopos_device_id");
  if(!id){
    id="dev_"+Date.now()+"_"+Math.random().toString(36).slice(2,10);
    localStorage.setItem("restopos_device_id",id);
  }
  return id;
}
function isTrustedDevice(licenseKey){
  const trusted=JSON.parse(localStorage.getItem("restopos_trusted_devices")||"[]");
  const deviceId=getOrCreateDeviceId();
  return trusted.some(d=>d.licenseKey===licenseKey&&d.deviceId===deviceId);
}
function trustDevice(licenseKey){
  const trusted=JSON.parse(localStorage.getItem("restopos_trusted_devices")||"[]");
  const deviceId=getOrCreateDeviceId();
  if(!trusted.some(d=>d.licenseKey===licenseKey&&d.deviceId===deviceId)){
    trusted.push({licenseKey,deviceId,trustedAt:new Date().toISOString(),ua:navigator.userAgent.slice(0,80)});
    localStorage.setItem("restopos_trusted_devices",JSON.stringify(trusted));
  }
}

// ═══════════════════════════════════════════════════════════════════
// FIRESTORE DATA SYNC — saves/restores sales+items+company per license
// ═══════════════════════════════════════════════════════════════════
async function syncDataToFirestore(licenseKey,data){
  ...
  }catch(e){console.warn("[Sync] Firestore write failed:",e.message);}
}
async function restoreDataFromFirestore(licenseKey){
  if(!licenseKey)return null;
  try{
    const ref=doc(db,"client_data",licenseKey);
    const snap=await getDoc(ref);
    if(snap.exists())return snap.data();
  }catch(e){console.warn("[Sync] Firestore read failed:",e.message);}
  return null;
}
...
// ═══════════════════════════════════════════════════════════════════
// DEVICE OTP — 2FA for new devices
// ═══════════════════════════════════════════════════════════════════
function DeviceOTP({license,onVerified,onBack}){
  const [step,setStep]=useState("sending"); // sending | input | verified
  const [code,setCode]=useState("");
  const [sentCode,setSentCode]=useState("");
  const [codeExpiry,setCodeExpiry]=useState(null);
  const [error,setError]=useState("");
  ...
  function startCooldown(secs=60){
    ...
    timerRef.current=setInterval(()=>setResendCooldown(c=>{if(c<=1){clearInterval(timerRef.current);return 0;}return c-1;}),1000);
  }

  async function sendOTP(){
    setLoading(true);setError("");
    const email=license?.email||"";
    if(!email){setError("No email found on this account. Contact support.");setLoading(false);setStep("input");return;}
    const newCode=generateCode();
    const expiry=Date.now()+(10*60*1000);
    setSentCode(newCode);setCodeExpiry(expiry);
    try{
      await sendEmailJS(EMAILJS_RESET_TEMPLATE,{
        to_email:email,
        to_name:license?.ownerName||license?.businessName||"",
        code:newCode
      });
      startCooldown(60);
      setStep("input");
  ...
  function handleVerify(){
    ...
    if(Date.now()>codeExpiry){setError("Code expired. Please request a new one.");setSentCode("");return;}
    if(code.trim()!==sentCode){setError("Incorrect code. Try again.");return;}
    // Trust this device
    trustDevice(license.licenseKey);
    // Register device in Firestore
    try{
      const deviceId=getOrCreateDeviceId();
      const devInfo=getDeviceInfo();
      const q2=query(collection(db,"pending_activations"),where("licenseKey","==",license.licenseKey));
      getDocs(q2).then(snap=>{
        if(!snap.empty){
          const ref=doc(db,"pending_activations",snap.docs[0].id);
          const existing=snap.docs[0].data();
          const devices=existing.trustedDevices||[];
          if(!devices.find(d=>d.deviceId===deviceId)){
            devices.push({deviceId,trustedAt:new Date().toISOString(),ua:devInfo.userAgent,os:devInfo.os,browser:devInfo.browser});
            updateDoc(ref,{trustedDevices:devices});
          }
        }
      });
    }catch(e){console.warn("Device registration failed:",e);}
    setStep("verified");
    setTimeout(()=>onVerified(),800);
  }

  const inp={width:"100%",padding:"14px",background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:10,fontSize:22,color:"#fff",fontFamily:"inherit",textAlign:"center",letterSpacing:"0.3em",fontWeight:800};
      ...
      <div style={{width:"100%",maxWidth:420}}>
        {step==="sending"&&(
          <div style={{textAlign:"center"}}>
            <div style={{width:72,height:72,background:"rgba(240,165,0,0.15)",border:"2px solid rgba(240,165,0,0.4)",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:32,margin:"0 auto 20px"}}>📧</div>
            <div style={{fontSize:20,fontWeight:800,color:"#fff",marginBottom:8}}>Sending verification code…</div>
            <div style={{fontSize:13,color:"rgba(255,255,255,0.45)"}}>We detected a new device. Sending OTP to your email.</div>
          </div>
        )}
        {(step==="input"||step==="verified")&&(
          <div>
            <div style={{textAlign:"center",marginBottom:28}}>
              <div style={{width:72,height:72,background:step==="verified"?"rgba(46,204,113,0.15)":"rgba(240,165,0,0.15)",border:`2px solid ${step==="verified"?"rgba(46,204,113,0.5)":"rgba(240,165,0,0.4)"}`,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:32,margin:"0 auto 16px"}}>
                {step==="verified"?"✅":"🔐"}
              </div>
              <div style={{fontSize:22,fontWeight:900,color:"#fff",marginBottom:6}}>
                {step==="verified"?"Device Verified!":"New Device Detected"}
              </div>
              <div style={{fontSize:13,color:"rgba(255,255,255,0.45)",lineHeight:1.6}}>
                {step==="verified"
                  ? "This device is now trusted. Logging you in…"
                  : <>Enter the 6-digit code sent to<br/><strong style={{color:"#7FFAB5"}}>{license?.email||"your email"}</strong></>
                }
              </div>
            </div>
            {step==="input"&&(
              <div style={{background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.15)",borderRadius:20,padding:28}}>
                <input
                  value={code}
                  ...
                  style={inp}
                />
                {error&&<div style={{marginTop:10,padding:"8px 12px",background:"rgba(217,64,64,0.18)",border:"1px solid rgba(217,64,64,0.4)",borderRadius:8,fontSize:12,color:"#ff8080",textAlign:"center"}}>{error}</div>}
                <button
                  onClick={handleVerify}
                  disabled={loading||code.length!==6}
                  style={{width:"100%",marginTop:16,padding:13,background:code.length===6?"linear-gradient(135deg,#1A6B4A,#134D36)":"#333",color:"#fff",border:"none",borderRadius:12,fontSize:15,fontWeight:800,cursor:code.length===6?"pointer":"not-allowed",fontFamily:"inherit"}}
                >
                  {loading?"Verifying…":"✓ Verify & Trust This Device"}
                </button>
                <div style={{display:"flex",gap:10,marginTop:10}}>
                  <button
                    onClick={()=>{if(resendCooldown>0)return;sendOTP();}}
                    disabled={resendCooldown>0}
                    style={{flex:1,padding:10,background:"transparent",color:resendCooldown>0?"rgba(255,255,255,0.2)":"rgba(255,255,255,0.5)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,fontSize:12,cursor:resendCooldown>0?"not-allowed":"pointer",fontFamily:"inherit"}}
                  >
                    {resendCooldown>0?`⏱ Resend in ${resendCooldown}s`:"🔁 Resend Code"}
                  </button>
                  <button
                    onClick={onBack}
                    style={{flex:1,padding:10,background:"transparent",color:"rgba(255,255,255,0.3)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:10,fontSize:12,cursor:"pointer",fontFamily:"inherit"}}
                  >
                    ← Back
                  </button>
                </div>
                <div style={{marginTop:14,fontSize:11,color:"rgba(255,255,255,0.2)",textAlign:"center"}}>Code expires in 10 minutes · This device will be trusted after verification</div>
              </div>
            )}
          </div>
        )}
...
function OwnerDashboardInline(){
  ...
  return(
    <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",color:DS.text,width:"100%",minHeight:"100vh",background:"linear-gradient(135deg,#0a1020 0%,#0F1929 100%)",padding:0}}>
      ...
      {/* DEVICES */}
      {tab==="devices"&&<div>
        <DCard style={{marginBottom:14}}>
          <div style={{fontSize:13,fontWeight:700,marginBottom:14,color:"#a5b4fc"}}>📱 Trusted Devices (2FA)</div>
          {activations.filter(a=>a.trustedDevices&&a.trustedDevices.length>0).length===0
            ?<div style={{textAlign:"center",padding:20,color:DS.sub,fontSize:12}}>No trusted devices yet. Devices appear here after clients complete 2FA verification.</div>
            :activations.filter(a=>a.trustedDevices&&a.trustedDevices.length>0).map(a=>(
              <div key={a.id} style={{marginBottom:16,padding:12,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10}}>
                <div style={{fontSize:12,fontWeight:700,color:"#fff",marginBottom:8}}>{a.businessName} <span style={{fontSize:10,color:DS.sub,fontFamily:"monospace"}}>({a.licenseKey})</span></div>
                {(a.trustedDevices||[]).map((d,i)=>(
                  <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 8px",background:"rgba(99,102,241,0.08)",border:"1px solid rgba(99,102,241,0.2)",borderRadius:7,marginBottom:5}}>
                    <div style={{flex:1}}>
                      <div style={{fontSize:11,color:DS.text}}>{d.os||"Unknown OS"} · {d.browser||"Unknown Browser"}</div>
                      <div style={{fontSize:10,color:DS.sub,fontFamily:"monospace"}}>{d.deviceId?.slice(0,20)}… · Trusted {d.trustedAt?.slice(0,10)}</div>
                    </div>
                    <button
                      onClick={async()=>{
                        if(!confirm("Revoke this device? The client will need to re-verify with OTP next login."))return;
                        const updated=(a.trustedDevices||[]).filter((_,j)=>j!==i);
                        await updateDoc(doc(db,"pending_activations",a.id),{trustedDevices:updated});
                        setActivations(prev=>prev.map(x=>x.id===a.id?{...x,trustedDevices:updated}:x));
                      }}
                      style={{padding:"4px 10px",background:"rgba(239,68,68,0.15)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:6,color:"#f87171",fontSize:10,fontWeight:700,cursor:"pointer",fontFamily:"inherit",flexShrink:0}}
                    >
                      Revoke
                    </button>
                  </div>
                ...
                <button
                  onClick={async()=>{
                    if(!confirm(`Revoke ALL devices for ${a.businessName}? They will need OTP on next login from any device.`))return;
                    await updateDoc(doc(db,"pending_activations",a.id),{trustedDevices:[]});
                    setActivations(prev=>prev.map(x=>x.id===a.id?{...x,trustedDevices:[]}:x));
                  }}
                  style={{marginTop:4,padding:"5px 12px",background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.25)",borderRadius:6,color:"#f87171",fontSize:10,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}
                >
                  🗑 Revoke All Devices
                </button>
              </div>
        ...
        <DCard>
          <div style={{fontSize:13,fontWeight:700,marginBottom:14,color:"#a5b4fc"}}>📋 Device Registry (Activation Info)</div>
          <DTable
          ...
          />
          {activations.filter(a=>a.deviceId||a.deviceInfo).length===0&&<div style={{textAlign:"center",padding:20,color:DS.sub,fontSize:12}}>No device data yet.</div>}
        </DCard>
      </div>}
...
export default function App(){
  ...
  function setSales(v){_setSales(p=>{const n=typeof v==="function"?v(p):v;LS.set("restopos_sales",n.slice(-500));
    // Sync to Firestore on every sale change (debounced via setTimeout)
    clearTimeout(window._salesSyncTimer);
    window._salesSyncTimer=setTimeout(()=>{
      const lic=LS.get("restopos_license_v2");
      if(lic?.licenseKey)syncDataToFirestore(lic.licenseKey,{sales:n.slice(-500)});
    },3000);
    return n;});}
  ...
  function setItems(v){_setItems(p=>{const n=typeof v==="function"?v(p):v;LS.set("restopos_items",n);
    clearTimeout(window._itemsSyncTimer);
    window._itemsSyncTimer=setTimeout(()=>{
      const lic=LS.get("restopos_license_v2");
      if(lic?.licenseKey)syncDataToFirestore(lic.licenseKey,{items:n});
    },3000);
    if(n.length!==p.length)logActivity(n.length>p.length?"ITEM_ADDED":"ITEM_DELETED",{after:{itemCount:n.length}},currentUser?.role||"System");return n;});}
  ...
  useEffect(()=>{
    ...
    if(saved){
      ...
      if(!creds){setStep("setCredentials");}
        ...
        if(isTrustedDevice(saved.licenseKey)){
          setStep("clientLogin");
        }else{
          setStep("deviceOTP");
        }
      }
      // Restore data from Firestore if localStorage seems empty
      if(saved.licenseKey&&(!LS.get("restopos_sales")||!LS.get("restopos_items"))){
        restoreDataFromFirestore(saved.licenseKey).then(data=>{
          if(data){
            if(data.sales&&data.sales.length>0&&(!LS.get("restopos_sales")||LS.get("restopos_sales").length===0)){
              LS.set("restopos_sales",data.sales);
              _setSales(data.sales);
            }
            if(data.items&&data.items.length>0&&(!LS.get("restopos_items")||LS.get("restopos_items").length===0)){
              LS.set("restopos_items",data.items);
              _setItems(data.items);
            }
            if(data.company){
              LS.set("restopos_company",data.company);
              _setCompany(data.company);
            }
          }
        });
      }
  ...
  if(step==="pendingApproval")return<PendingApprovalScreen license={license} onApproved={()=>setStep("deviceOTP")} onSwitchAccount={handleSwitchAccount}/>;
  if(step==="deviceOTP")return<DeviceOTP license={license} onVerified={()=>setStep("clientLogin")} onBack={()=>setStep("clientLogin")}/>;
