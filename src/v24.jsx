...
const APP_VERSION_FULL="RestoPOS v24 · ZATCA Phase 2";
...
// ═══════════════════════════════════════════════════════════════════
const RECEIPT_FONT_MAP={"courier":"'Courier New',monospace","georgia":"Georgia,serif","trebuchet":"'Trebuchet MS',sans-serif","arial-narrow":"'Arial Narrow',Arial,sans-serif","impact":"Impact,Haettenschweiler,sans-serif","tajawal":"'Tajawal',sans-serif","cairo":"'Cairo',sans-serif","amiri":"'Amiri',serif","scheherazade":"'Scheherazade New',serif","noto-naskh":"'Noto Naskh Arabic',serif"};
...
function buildReceiptHTML(order,license,zatcaInvoice,fmt,qrImgSrc){
  ...
  const fontFamily=RECEIPT_FONT_MAP[fmt.font]||"'Courier New',monospace";
  ...
  // Meta row
  let meta=`<div style="display:flex;justify-content:space-between;font-size:${fontSize-1}px"><span>${_escHTML(order.id||"")}</span><span>${_escHTML(order.date||"")} ${_escHTML(order.time||"")}</span></div>`;
  ...
  if(showCustomer&&order.customer)meta+=`<div style="font-size:${fontSize-2}px">Customer: ${_escHTML(order.customer)}</div>`;
  if(order.note)meta+=`<div style="font-size:${fontSize-2}px;font-style:italic">Note: ${_escHTML(order.note)}</div>`;
  // Items — grouped by category when enabled
  function lineHTML(it){
    const ar=(showArabicName&&it.nameAr)?`<div style="direction:rtl;font-family:'Noto Naskh Arabic','Tajawal',sans-serif;font-size:${fontSize-2}px;color:#555">${_escHTML(it.nameAr)}</div>`:"";
    return `<div style="display:flex;justify-content:space-between;margin:2px 0${boldItems?";font-weight:700":""}"><span style="flex:1;padding-right:4px">${_escHTML(it.name)}<span style="color:#777"> x${it.qty}</span>${ar}</span><span style="white-space:nowrap">SAR ${(it.qty*it.price).toFixed(2)}</span></div>`;
  }
  let itemsHTML="";
  if(showCategories){
    ...
    itemsHTML=cats.map(cat=>{
      ...
      if(!ci.length)return"";
      return `<div style="font-size:${fontSize-3}px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#888;margin:5px 0 2px">${_escHTML(cat)}</div>`+ci.map(lineHTML).join("");
    }).join("");
  }else{
    itemsHTML=items.map(lineHTML).join("");
  }
  // ZATCA QR block — printed on every (non-draft) invoice
  const qrBlock=qrImgSrc
    ? `<div style="text-align:center;margin:8px 0">${zatcaInvoice?`<div style="font-size:${fontSize-3}px;color:#444">Invoice: ${_escHTML(zatcaInvoice.invoice_number||"")}${zatcaInvoice.icv?" · ICV: "+_escHTML(zatcaInvoice.icv):""}</div>`:""}<img src="${qrImgSrc}" style="width:120px;height:120px;display:block;margin:4px auto"/><div style="font-size:${fontSize-3}px;font-weight:700;letter-spacing:.08em">ZATCA PHASE 2 · QR</div><div style="font-size:${fontSize-4}px;color:#777">TLV Base64 · Scan to verify</div></div>`
    : "";
  return `<!DOCTYPE html><html><head><meta charset="UTF-8">
...
body{font-family:${fontFamily};font-size:${fontSize}px;width:${paperWidth};padding:4mm;color:#000;background:#fff;line-height:1.45}
</style></head><body>
${header}
${SEP}
${meta}
...
${itemsHTML}
${SEP}
${order.discount>0?`<div style="display:flex;justify-content:space-between;color:#b00"><span>Discount</span><span>-SAR ${order.discount.toFixed(2)}</span></div>`:""}
${showVat?`<div style="display:flex;justify-content:space-between;font-size:${fontSize-1}px;color:#666"><span>VAT 15% (incl.)</span><span>SAR ${(order.vat||0).toFixed(2)}</span></div>`:""}
<div style="display:flex;justify-content:space-between;font-weight:900;font-size:${totalSize}px;border-top:2px solid #000;padding-top:4px;margin-top:3px"><span>TOTAL</span><span>SAR ${(order.total||0).toFixed(2)}</span></div>
${order.payMethod==="Cash"?`<div style="display:flex;justify-content:space-between;font-size:${fontSize-1}px"><span>Cash Given</span><span>SAR ${(order.given||0).toFixed(2)}</span></div><div style="display:flex;justify-content:space-between;font-size:${fontSize-1}px;font-weight:700"><span>Change</span><span>SAR ${(order.change||0).toFixed(2)}</span></div>`:""}
${SEP}
${qrBlock}
${fmt.website?`<div style="text-align:center;font-size:${fontSize-2}px;color:#666">${_escHTML(fmt.website)}</div>`:""}
${fmt.social?`<div style="text-align:center;font-size:${fontSize-2}px;color:#666">${_escHTML(fmt.social)}</div>`:""}
<div style="text-align:center;font-weight:700;font-size:${fontSize}px;margin-top:4px">${_escHTML(footer)}</div>
${footerAr?`<div style="text-align:center;direction:rtl;font-family:'Noto Naskh Arabic','Tajawal',sans-serif;font-size:${fontSize}px;font-weight:700">${_escHTML(footerAr)}</div>`:""}
<br/><br/>
</body></html>`;
}
...
function POS({items,sales,setSales,tables,setTables,promos,license,lang="en",currentUser=null}){
  ...
  async function confirmPayment(method,given,change,promo,totalDiscountAmt,finalTotal,finalVat,printAndSave,payInfo,manualDiscountAmt,promoDiscountAmt,isDraft=false,extraData={}){
    ...
    if(!printAndSave){
      ...
      // ── Try 3: Browser print — direct window ────────────────────
      if(!printed){
        try{
          const printWin=window.open("","_blank","width=400,height=300,toolbar=no,menubar=no");
          if(printWin){
            ...
            printWin.focus();
            setTimeout(()=>{
              try{printWin.print();}catch(pe){console.warn("[print]",pe);}
              setTimeout(()=>{try{printWin.close();}catch(e){}},2000);
            },500);
            printed=true;
            printMethod="Browser";
          }else{
            // Popup blocked — last resort: show receipt modal
            setShowReceipt(true);
          }
        }catch(e){
...
function Settings({company,setCompany,tables,setTables,license,onClearLicense,onSwitchAccount,pins,setPins,invoiceFormat,setInvoiceFormat,lang="en",onLangChange,sales=[],items=[]}){
  ...
  const TAB_LABELS={"company":"🏢 "+t("Company",lang),"tables":"🪑 "+t("Tables",lang),"printers":"🖨️ "+t("Bill Printer",lang),"invoices":"📄 "+t("Invoices",lang),"backup":"💾 "+t("Backup",lang),"security":"🔐 "+t("Security",lang),"license":"📋 "+t("License",lang),"language":"🌐 "+t("Language",lang)};
  const tabs=[["company","🏢 Company"],["tables","🪑 Tables"],["printers","🖨️ Bill Printer"],["invoices","📄 Invoices"],["backup","💾 Backup"],["security","🔐 Security"],["license","📋 License"],["language","🌐 Language"]];
...
function Help(){
  ...
  const sections=[["guide","🚀","Guide"],["zatca","⬛","ZATCA"],["ai","🤖","AI Help"],["update","🔄","Update"],["upgrade","⬆️","Upgrade"],["live","🆘","Live Help"],["support","📞","Support"]];
...
async function printReceiptEscPos(order,license){
  ...
  // ZATCA QR — scannable, printed natively on the thermal head
  try{
    const qrStr=(order.qr_string)||generatePhase1QR({sellerName:license.businessName||"",vatNumber:license.vatNumber||"",timestamp:new Date().toISOString(),total:order.total||0,vatAmount:order.vat||0});
    parts.push(
      escpos.text(""),
      escpos.alignCenter(),
      escpos.qr(qrStr),
      escpos.text("ZATCA PHASE 2 · QR"),
    );
  }catch(e){}
  parts.push(
    escpos.text(""),
...
function AdvancedFeatures({sales,items,setItems,license,company,invoiceFormat,setInvoiceFormat,users,setUsers}){
  ...
  const tabs=[["qztray","🖨️ QZ Tray"],["invoice","🧾 Invoice Format"],["kitchen","🍽️ Kitchen Printer"],["users","👤 Users"],["kds","🍳 KDS"],["stocktakes","📦 Stock Takes"],["recipes","📋 Recipes"],["giftcards","🎁 Gift Cards"],["delivery","🛵 Delivery"],["locations","🏢 Locations"],["accounting","📤 Accounting"],["reports","📅 Reports"],["printer","🖨️ ESC/POS"],["errorlog","⚠️ Error Log"]];
  return(
    <div>
      <div style={{display:"flex",gap:6,marginBottom:20,flexWrap:"wrap"}}>
        {tabs.map(([id,lbl])=>(
...
export default function App(){
  const [step,setStep]=useState("checking");const [businessData,setBusinessData]=useState(null);const [license,setLicense]=useState(null);const [currentUser,setCurrentUser]=useState(null);const [screen,_setScreen]=useState(()=>{const s=LS.get("restopos_last_screen")||"pos";if(s==="invoices"||s==="backup")return"settings";if(s==="useradmin")return"advanced";return s;});
