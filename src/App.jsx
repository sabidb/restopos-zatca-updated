...
function buildPresetKOT(order,fmt){
  ...
  // Title — "Telephone Bill" style header (kitchen ticket)
  const kotTitle=fmt.kotTitle||"Telephone Bill";
  ...
  const tokenBox=`<div style="margin:5px 0">
    <div style="font-weight:900;font-size:${headFont-2}px;margin-bottom:3px">Token No: ${_escHTML(order.token||order.kot||"")}</div>
    <div style="border:${style==="s4"?"2px":"1px"} solid #000;border-radius:${style==="s2"?"6px":"2px"};padding:6px 8px;display:flex;justify-content:space-between;align-items:center;gap:8px">
      <span style="font-weight:900;font-size:${headFont-2}px">${_escHTML(orderTypeEn)} ${orderTypeEn?"-":""}</span>
      <span style="direction:rtl;font-weight:900;font-size:${headFont-2}px">${_escHTML(orderTypeAr)}</span>
    </div>
  </div>`;
  // Items table header
      ...
      try{
        ...
        try{
          if(isQZConnected()&&_qzKitchenPrinter){
            const kotHTML=buildKOTHtml({kot:newKot2,token:inv.token,voucher:inv.voucher||inv.id,date:inv.date,type:inv.type,table:kTable,time:new Date().toLocaleTimeString("en-SA"),items:kotCart});
...
function PresetFormatTab({license,company}){
  ...
  const DEFAULTS={
    ...
    kot:{usePreset:true,presetStyle:"s1",paperWidth:"80mm",headFont:20,bodyFont:16,lineGap:4,logoSize:50,logoUrl:"",kotTitle:"Telephone Bill"},
  };
    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18,alignItems:"start"}} className="preset-grid">
        ...
        <Card>
          ...
          {/* KOT title (KOT mode only) */}
          {mode==="kot"&&(
            <div style={{marginBottom:16}}>
              <label style={{fontSize:12,fontWeight:700,color:C.textMid,display:"block",marginBottom:4}}>Kitchen ticket title</label>
              <input value={fmt.kotTitle||""} onChange={e=>set("kotTitle",e.target.value)} placeholder="Telephone Bill" style={{width:"100%",padding:"9px 12px",border:`1px solid ${C.border}`,borderRadius:8,fontSize:13,fontFamily:"inherit",color:C.text,background:"#fff"}}/>
            </div>
          )}
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    const onStorage=(e)=>{if(!e||e.key===TOKEN_KEY)setDailyToken(getDailyToken());};
    window.addEventListener("restopos-token",onTok);
    window.addEventListener("storage",onStorage);
    return()=>{window.removeEventListener("restopos-token",onTok);window.removeEventListener("storage",onStorage);};
  },[]);
